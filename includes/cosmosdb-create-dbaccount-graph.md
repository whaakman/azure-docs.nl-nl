...
2. Klik in het menu links op **Nieuw**, klik op **Databases** en klik vervolgens op **Azure Cosmos DB**.
   ...
   ![Schermopname van Azure Portal waarbij Meer services en Azure Cosmos DB zijn gemarkeerd](./media/cosmosdb-create-dbaccount-graph/create-nosql-db-databases-json-tutorial-1.png)
...
3. Geef op de blade **Nieuw account** de gewenste configuratie op voor het Azure Cosmos DB-account. 
    ...
    In deze Quick Start programmeren we met de Graph API, daarom kiest u bij het invullen van het formulier voor **Gremlin (grafiek)**. Maar als u documentgegevens uit een catalogus-app hebt, sleutelwaardegegevens (tabelgegevens), of gegevens die zijn gemigreerd uit een MongoDB-app, moet u er rekening mee houden dat Azure Cosmos DB een zeer beschikbare, globaal gedistribueerd databaseserviceplatform kan bieden voor alle bedrijfskritische toepassingen.
    ...
    ![Schermopname van de blade Nieuwe Azure Cosmos DB](./media/cosmosdb-create-dbaccount-graph/create-nosql-db-databases-json-tutorial-2.png)
    ...
    App Service-plan voor Graph API|Maak nieuwe - standaard S1|Selecteer de rekenresource om Gremlin Graph-query's uit te voeren.

4. Klik op **Maken** om het account te maken.
5. Klik op de werkbalk op **Meldingen** om het implementatieproces te bewaken.

    ![De melding Implementatie is gestart](./media/cosmosdb-create-dbaccount-graph/azure-documentdb-nosql-notification.png)

6.  Als de implementatie is voltooid, opent u het nieuwe account via de tegel Alle resources. 

    ![DocumentDB-account op de tegel Alle resources](./media/cosmosdb-create-dbaccount-graph/azure-documentdb-all-resources.png)