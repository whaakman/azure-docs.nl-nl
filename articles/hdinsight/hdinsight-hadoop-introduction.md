---
title: Informatie over Azure HDInsight, de Hadoop-technologiestack en clusters | Microsoft Docs
...
# Informatie over Azure HDInsight, de Hadoop-technologiestack en Hadoop-clusters
...
Azure HDInsight is een cloud-distributie van de Hadoop-onderdelen van het **Hortonworks Data Platform (HDP)**. [Apache Hadoop](http://hadoop.apache.org/) was het originele open-source framework voor gedistribueerde verwerking en analyse van sets met big data in clusters van computers. 

HDInsight vergemakkelijkt het toepassen van Hadoop technologieën door:

*    Minder installatie- en configuratietaken. Zie [Set up clusters in HDInsight with Hadoop, Spark, Kafka, and more](hdinsight-hadoop-provision-linux-clusters.md) (Clusters instellen in HDInsight met Hadoop, Spark, Kafka en meer).
*    Hoge beschikbaarheid en betrouwbaarheid. Zie [Availability and reliability of Hadoop clusters in HDInsight](hdinsight-high-availability-linux.md) (Beschikbaarheid en betrouwbaarheid van Hadoop-clusters in HDInsight).
*    Beveiliging en governance via integratie met Active Directory. Zie [An introduction to Hadoop security with domain-joined HDInsight clusters (Preview)](hdinsight-domain-joined-introduction.md) (Een inleiding tot Hadoop-beveiliging met aan een domein gekoppelde HDInsight-clusters (Preview)).
*    Dynamisch schalen zonder onderbreking van taken
*   Onderdeelupdates en huidige versies. Zie [What are the Hadoop components and versions available with HDInsight?][component-versioning] (Welke Hadoop-onderdelen en -versies zijn beschikbaar met HDInsight?).
*   Integratie met andere Azure-services, waaronder [web-apps](https://docs.microsoft.com/azure/app-service-web/) en [SQL Database](https://docs.microsoft.com/azure/sql-database/).

De Hadoop-technologiestack omvat verwante software en hulpprogramma's, waaronder Apache Hive, HBase, Spark en Kafka om er maar een paar te noemen. Zie de [pagina met Azure-functies voor HDInsight](https://azure.microsoft.com/services/hdinsight/) voor meer informatie over Hadoop in HDInsight.

<a id="what-is-a-hadoop-cluster-and-when-do-you-use-it" class="xliff"></a>

## Wat is een Hadoop-cluster en wanneer gebruikt u het?
De term *Hadoop* verwijst ook naar een type cluster met deze kenmerken:

* HDFS (Hadoop Distributed File System)
* YARN voor taakplanning en resourcebeheer
* MapReduce voor parallelle verwerking
  
Hadoop-clusters worden het meest gebruikt voor batchverwerking van opgeslagen gegevens. Andere soorten clusters in HDInsight hebben extra functies, zoals snellere verwerking in het geheugen of verwerking voor streaming berichtenwachtrijen. Zie [Clustertypen in HDInsight](#overview) voor meer informatie.
...
Bij big data kan het gaan om historische gegevens (opgeslagen gegevens), maar ook om realtime gegevens (gestreamd vanuit de bron). Big data worden verzameld in steeds sneller groeiende volumes en in een steeds groter wordend aantal indelingen.

## <a name="overview"></a>Clustertypen in HDInsight
HDInsight is een clouddistributie van de snelgroeiende Apache Hadoop-technologiestack in Microsoft Azure voor big data-analyse. Het omvat specifieke clustertypen en opties voor clusteraanpassing, zoals het toevoegen van onderdelen, hulpprogramma's en talen.

<a id="spark-kafka-interactive-hive-hbase-customized-and-other-cluster-types" class="xliff"></a>

### Clusters van Spark, Kafka, Interactive Hive, HBase, aangepaste clusters en andere clustertypen
HDInsight biedt de volgende clustertypen:

* **[Apache Hadoop](https://wiki.apache.org/hadoop)**: maakt gebruik van [HDFS](#hdfs), [YARN](#yarn) voor resourcebeheer en een eenvoudig [MapReduce](#mapreduce)-programmeermodel om gegevens parallel te verwerken en te analyseren.
...
### Voorbeeldscripts voor het aanpassen van clusters
...
## Onderdelen en hulpprogramma's in HDInsight-clusters
...
## Microsoft Business Intelligence en HDInsight
