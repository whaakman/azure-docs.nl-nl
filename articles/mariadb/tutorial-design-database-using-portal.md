---
title: 'Zelfstudie: een Azure Database for MariaDB ontwerpen met Azure Portal'
description: In deze zelfstudie wordt uitgelegd hoe u een Azure Database voor MariaDB-server en -database maakt en beheert via Azure Portal.
author: ajlam
ms.author: andrela
editor: jasonwhowell
services: mariadb
...
ms.locfileid: "46991008"
---
# <a name="tutorial-design-an-azure-database-for-mariadb-database-using-the-azure-portal"></a>Zelfstudie: een Azure Database for MariaDB-database ontwerpen met Azure Portal
Azure Database voor MariaDB is een beheerde service waarmee u MySQL-databases met hoge beschikbaarheid in de cloud kunt uitvoeren, beheren en schalen. Met behulp van Azure Portal kunt u eenvoudig uw server beheren en een database ontwerpen.
...
Een Azure Database voor MariaDB-server wordt gemaakt met een gedefinieerde set reken- en opslagresources <!--[compute and storage resources](./concepts-compute-unit-and-storage.md)-->. De server wordt gemaakt in een [Azure-resourcegroep](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-overview).
...
2. Typ **Azure Database for MariaDB** in het zoekvak om de service te vinden.
   ...
   ![Navigeren naar MySQL](./media/tutorial-design-database-using-portal/1-Navigate-to-mariadb.png)

3. Klik op de tegel **Azure Database voor MariaDB** en klik vervolgens op **Maken**. Vul het Azure Database for MariaDB-formulier in.
...
4. Klik op **Create**. Na een minuut of twee wordt een nieuwe Azure-database voor de MariaDB-server uitgevoerd in de cloud. Klik op de knop **Meldingen** op de werkbalk om het implementatieproces te volgen.

## <a name="configure-firewall"></a>Firewall configureren
Databases van Azure Database voor MariaDB worden beveiligd door een firewall. Standaard worden alle verbindingen met de server en de databases op de server geweigerd. Voordat u voor het eerst verbinding maakt met Azure Database voor MariaDB moet u de firewall configureren en het IP-adres (of IP-adresbereik) van het openbare netwerk van de clientcomputer toevoegen.
...
> [!TIP]
> Azure Database voor MariaDB-servers communiceren via poort 3306. Als u verbinding probeert te maken vanuit een bedrijfsnetwerk, wordt uitgaand verkeer via poort 3306 mogelijk niet toegestaan door de firewall van uw netwerk. In dat geval kunt u geen verbinding maken met uw Azure Database voor MariaDB-server, tenzij de IT-afdeling poort 3306 openstelt.

## <a name="get-connection-information"></a>Verbindingsgegevens ophalen
Ga naar Azure Portal om de volledig gekwalificeerde **servernaam** en **aanmeldingsnaam van de serverbeheerder** op te halen voor uw Azure-Database voor MariaDB-server. U gebuikt de volledig gekwalificeerde servernaam om verbinding met uw server te maken via het opdrachtregelprogramma mysql. 

1. Klik in [Azure Portal](https://portal.azure.com/) op **Alle resources** in het menu aan de linkerkant, typ de naam en zoek naar uw Azure Database voor MariaDB-server. Selecteer de naam van de server om de details te bekijken.
...
U weet nu hoe u verbinding kunt maken met de Azure Database voor MariaDB-database en dus is het tijd om enkele eenvoudige taken uit te voeren:
...
Stel dat u per ongeluk een belangrijke databasetabel hebt verwijderd en dat de gegevens niet gemakkelijk kunnen worden hersteld. Azure Database voor MariaDB biedt gelukkig de mogelijkheid om de server te herstellen naar een eerder tijdstip, door het maken van een kopie van de databases op de nieuwe server. U kunt deze nieuwe server dan gebruiken om de verwijderde gegevens te herstellen. Met de volgende stappen wordt de voorbeeldserver hersteld naar een punt voordat de tabel is toegevoegd.

1. Zoek uw Azure Database voor MariaDB-server in Azure Portal. Klik op de pagina **Overzicht** op **Herstellen** op de werkbalk. De gelijknamige pagina wordt geopend.
...
3. Klik op **OK** om de server te herstellen naar een eerder tijdstip <!--[restore to a point in time](./howto-restore-server-portal.md)-->, voordat de tabel werd toegevoegd. Als u een server herstelt, wordt er een nieuwe kopie van de server gemaakt, vanaf het tijdstip dat u opgeeft. 
