...
### <a name="batchlabs-optional"></a>BatchLabs (optioneel)
[BatchLabs][github_batchlabs] is een gratis, uitgebreid, zelfstandig clienthulpprogramma voor het maken en bewaken van en opsporen van fouten in Azure Batch-toepassingen. Hoewel het niet is vereist om deze zelfstudie te voltooien, kan het nuttig zijn bij de ontwikkeling en foutopsporing van uw Batch-oplossingen.
...
We gebruiken in de toepassing de verwijzing `blobClient` en geven deze als een parameter door aan verschillende methoden. Een voorbeeld hiervan bevindt zich in het codeblok dat onmiddellijk na het bovenstaande blok volgt, waar we `CreateContainerIfNotExistAsync` aanroepen om de containers daadwerkelijk te maken.
...
Zodra de toepassing en gegevensbestanden naar het Storage-account zijn geüpload met Azure Storage-API's, worden er door *DotNetTutorial* aanroepen naar de Batch-service gedaan met API's uit de Batch .NET-bibliotheek. De code maakt eerst een [BatchClient][net_batchclient]:
...
Daarna wordt in het voorbeeld met behulp van een aanroep naar `CreatePoolIfNotExistsAsync` een pool van rekenknooppunten in het Batch-account gemaakt. `CreatePoolIfNotExistsAsync` gebruikt de methode [BatchClient.PoolOperations.CreatePool][net_pool_create] om een nieuwe pool te maken in de Batch-service:
...
* Bekijk de [opmerkingen bij de release](https://github.com/Azure/azure-sdk-for-net/blob/psSdkJson6/src/SDKs/Batch/DataPlane/changelog.md#azurebatch-release-notes) van Batch .NET voor de meest recente wijzigingen in de bibliotheek.
