...
|ignoreRoleInstanceStatus|booleaans|Optioneel. Wanneer de waarde van dit kenmerk is ingesteld op `true`, wordt de status van een service genegeerd en het eindpunt wordt niet door de load balancer verwijderd. De waarde instelt op `true` nuttig voor het opsporen van bezet exemplaren van een service. De standaardwaarde is `false`. **Opmerking:** een eindpunt kan nog steeds verkeer ontvangen zelfs wanneer de functie niet gereed is.|  
