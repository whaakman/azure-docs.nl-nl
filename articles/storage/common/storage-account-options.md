...
Azure Storage biedt drie verschillende accountopties, met verschillende prijzen en functies. Het is belangrijk dat gebruikers rekening houden met deze verschillen om te bepalen welke optie het beste is voor hun toepassingen.  De drie verschillende opties zijn:

* **General Purpose v2-accounts (GPv2)** bieden alle nieuwste functies en ondersteuning voor blobs, bestanden, wachtrijen en tabellen. Momenteel bevatten deze nieuwste functies laaginstelling op blobniveau, archiefopslag, hogere limieten voor opslagaccounts en opslaggebeurtenissen. Prijsmodel biedt de laagste prijzen per GB en concurrerende transactieprijzen in de branche.

* **Blob Storage**-accounts bieden de nieuwste functies voor blok-blobs, maar ondersteunen alleen blok-blobs.  Prijzen zijn vergelijkbaar met die van General Purpose v2. We raden de meeste gebruikers aan om General Purpose v2 te gebruiken in plaats van Blob Storage-accounts.
...
**Blob Storage-accounts** ondersteunen dezelfde Blok Blob-functies als GPv2, maar zijn beperkt tot het alleen ondersteunen van blok-blobs. Klanten moeten de prijsverschillen tussen Blob Storage-accounts en GPv2 goed bekijken en overwegen om te upgraden naar GPv2. Houd er rekening mee dat deze upgrade niet ongedaan kan worden gemaakt.
...
**General Purpose v1 (GPv1)** zijn de oudste opslagaccounts en het enige type dat gebruikt kan worden in het klassieke implementatiemodel. Functies zoals Cool- en Archive-opslag zijn niet beschikbaar in GPv1. GPv1 heeft over het algemeen hogere kosten voor GB-opslag, maar lagere transactiekosten dan GPv2- of Blob Storage-accounts.
...
Voor toepassingen waarvoor alleen de opslag van blok- of toevoeg-blobs is vereist, wordt aangeraden gebruik te maken van GPv2-accounts. Zo profiteert u maximaal van het gedifferentieerde prijsmodel voor gelaagde opslag. We begrijpen dat dit onder bepaalde omstandigheden, waar GPv1-opslagaccounts beter werken, wellicht niet mogelijk is. Bijvoorbeeld:
...
* U gebruikt omvangrijke transacties of geo-replicatie bandbreedte, die beiden meer kosten in GPv2- en Blob Storage-accounts dan in GPv1, en u hebt niet genoeg opslagruimte om te kunnen profiteren van de lagere kosten van GB-opslag.
...
* Een GPv1- of Blob Storage-account converteren naar een GPv2-opslagaccount.
...
5. Selecteer **General Purpose v2** in de vervolgkeuzelijst Soort account.
...
3. Klik in de blade Instellingen op **Configuratie**.

4. Klik onder Soort account op **Upgrade**.

5. Een nieuwe blade verschijnt rechts ter bevestiging. Typ bij Upgrade bevestigen de naam van uw account. 
...
> [!NOTE]
> Blob Storage-accounts geven het service-eindpunt van de tabel alleen weer voor het opslaan en openen van de metrische gegevens voor dat account. GPv1 ZRS-opslagaccounts bieden geen ondersteuning voor metrische gegevens.
...
> [!NOTE]
> Als u een algemeen opslagaccount hebt waarin u pagina-blobs en virtuele-machineschijven of wachtrijen, bestanden of tabellen, hebt opgeslagen naast blok- en toevoegblobgegevens, is dit schattingsproces niet van toepassing. Dit komt omdat de capaciteitsgegevens geen onderscheid maken tussen blok-blobs en andere typen en geen capaciteitsgegevens voor andere datatypen geven. Als u deze typen gebruikt, is een alternatieve methode om te kijken naar de hoeveelheden op uw meest recente factuur.
