---
title: Overzicht van uw Azure Import/Export-taken | MicrosoftDocs
description: Informatie over het overzicht van de Azure Import/Export-service-taken in een abonnement.
author: muralikk
services: storage
ms.service: storage
ms.topic: article
ms.date: 01/23/2017
ms.author: muralikk
ms.subservice: common
ms.openlocfilehash: 0ae9e7fa76c8ecbb724cf0f494e648df989dff30
ms.sourcegitcommit: 3102f886aa962842303c8753fe8fa5324a52834a
ms.translationtype: MT
ms.contentlocale: nl-NL
ms.lasthandoff: 04/23/2019
ms.locfileid: "61478688"
---
# <a name="enumerating-jobs-in-the-azure-importexport-service"></a>Taken in de Azure Import/Export-service opsommen
Aanroepen voor het inventariseren van alle taken in een abonnement, de [lijst met taken](/rest/api/storageimportexport/jobs) bewerking. `List Jobs` retourneert een lijst van taken, evenals de volgende kenmerken:

-   Het type van de taak (importeren of exporteren)

-   De huidige taakstatus

-   De taak gekoppeld opslagaccount

## <a name="next-steps"></a>Volgende stappen

* [Met behulp van de Import/Export-service REST-API](storage-import-export-using-the-rest-api.md)
