...
2. Voer de volgende opdracht in om gegevens kopiëren vanuit het *csv*-account.
   ...
   ```
   * Vervang de waarde van de tijdelijke aanduiding `<csv-folder-path>` door het mappad naar het *csv*-bestand (met uitzondering van de naam van het bestand).
...
Voeg aan het notitieblok dat u eerder hebt gemaakt een nieuwe cel toe en plak de volgende code in die cel. Vervang de waarde van de tijdelijke aanduiding `storage-account-name` in dit codefragment door de naam van de map waarin u het CSV-bestand hebt opgeslagen.
...
Plak de volgende code in een nieuwe cel om een lijst op te halen met CSV-bestanden die zijn geüpload via AzCopy. Vervang de waarde van de tijdelijke aanduiding `<csv-folder-path>` door dezelfde waarde voor die tijdelijke aanduiding die u eerder hebt gebruikt.
...
* Vervang de waarde van de tijdelijke plaatsaanduiding `<your-csv-file-name` door de naam van uw *csv*-bestand.
