...
>[!IMPORTANT]
>Als u PolyBase om gegevens te laden vanuit ADLS Gen2 in SQL DW, wanneer ADLS Gen2 met-verificatie beheerde gebruikt, zorg ervoor dat u ook de stappen #1 en 2 in # [deze richtlijnen](../sql-database/sql-database-vnet-service-endpoint-rule-overview.md#impact-of-using-vnet-service-endpoints-with-azure-storage) uw SQL Database-server registreren bij Azure Active Directory (AAD) en toewijzen van opslag-Blob gegevens Inzender RBAC-rol met uw SQL-Database-server. de rest wordt afgehandeld door ADF. Als uw ADLS-Gen2 is geconfigureerd met VNet-service-eindpunt, voor het gebruik van PolyBase om gegevens te laden, moet u verificatie van de beheerde identiteit.
