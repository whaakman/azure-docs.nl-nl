...
Op basis van een voorwaarde van toepassing is één waarde of de andere. 'Overige' als NULL beschouwd als het is niet opgegeven. Zowel de waarden moeten compatibel zijn (numerieke, tekenreeks...) * ``iif(custType == 'Premium', 10, 4.5)``
* ``iif(amount > 100, 'High')``
...
Rechts vult de tekenreeks die door de opgegeven opvulling totdat deze van een bepaalde periode is. Als de tekenreeks is gelijk aan of groter dan de lengte geen is * ``rpad('great', 10, '-') -> 'great-----'`` 
* ``rpad('great', 4, '-') -> 'great'`` 
* ``rpad('great', 8, '<>') -> 'great<><'`` 
*********************************
...
Links vult de tekenreeks die door de opgegeven opvulling totdat deze van een bepaalde periode is. Als de tekenreeks is gelijk aan of groter dan de lengte geen is * ``lpad('great', 10, '-') -> '-----great'`` 
* ``lpad('great', 4, '-') -> 'great'`` 
...
Het patroon is een tekenreeks die is letterlijk, met uitzondering van de volgende speciale tekens: _ komt overeen met Eén willekeurig teken in de invoer (vergelijkbaar met. in reguliere expressies posix) % komt overeen met nul of meer tekens in de invoer (vergelijkbaar met. * in posix reguliere expressies).
...
Converteert een primitief gegevenstype naar een tekenreeks. Voor de getallen en datum kan een indeling die worden opgegeven. Als u niets opgeeft de systeemstandaard wordt opgehaald. Java decimale indeling wordt gebruikt voor getallen. Standaarddatumnotatie is jjjj-MM-dd * ``toString(10) -> '10'``
...
Retourneert altijd een waarde true. De functie syntax(true()) gebruiken als u de naam van een kolom met de naam 'true' * ``isDiscounted == true()``
* ``isDiscounted() == true``
*********************************
<code>false</code>
==============================
<code><b>false() => boolean</b></code><br/><br/>
Retourneert altijd een waarde false. De functie syntax(false()) gebruiken als u de naam van een kolom met de naam 'false' * ``isDiscounted == false()``
* ``isDiscounted() == false``
*********************************
<code>toBoolean</code>
==============================
...
Converteert een tekenreeks naar een datum een optionele datumnotatie gegeven. Als de datumnotatie wordt weggelaten, worden de combinaties van de volgende geaccepteerd. [ yyyy, yyyy-[M]M, yyyy-[M]M-[d]d, yyyy-[M]M-[d]d, yyyy-[M]M-[d]d, yyyy-[M]M-[d]dT* ] * ``toDate('2012-8-8') -> 2012-8-8``
* ``toDate('12/12/2012', 'MM/dd/yyyy') -> 2012-12-12``
*********************************
<code>toTimestamp</code>
==============================
<code><b>toTimestamp(<i>&lt;string&gt;</i> : any, [<i>&lt;timestamp format&gt;</i> : string], [<i>&lt;time zone&gt;</i> : string]) => timestamp</b></code><br/><br/>
Converteert een tekenreeks naar een datum een optionele tijdstempelnotatie gegeven. Als de tijdstempel wordt weggelaten de standaard-patroon jjjj-[M] M-[d] d uu: mm: [. f...] wordt gebruikt * ``toTimestamp('2016-12-31 00:12:00') -> 2012-8-8T00:12:00``
...
Berekent de SHA-2-samenvatting van de set kolommen van verschillende primitieve gegevenstypen gegeven een bitlengte die alleen van waarden 0(256) zijn kan, 224, 256, 384, 512. Het kan worden gebruikt voor het berekenen van een vingerafdruk voor een rij * ``sha2(256, 'gunchus', 8.2, 'bojjus', true, toDate('2010-4-4')) -> 'd3b2bff62c3a00e9370b1ac85e428e661a7df73959fa1a96ae136599e9ee20fd'``
*********************************
<code>crc32</code>
==============================
...
Controleert of als de rij is gemarkeerd voor invoegen. Transformaties die meer dan één invoerstroom kunnen de index (1-indeling) van de stroom doorgeven. De standaardwaarde voor de index van de stream is een * ``isInsert() -> true``
* ``isInsert(1) -> false``
*********************************
<code>isUpdate</code>
==============================
<code><b>isUpdate([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Controleert of als de rij is gemarkeerd voor update. Transformaties die meer dan één invoerstroom kunnen de index (1-indeling) van de stroom doorgeven. De standaardwaarde voor de index van de stream is een * ``isUpdate() -> true``
* ``isUpdate(1) -> false``
*********************************
<code>isDelete</code>
==============================
<code><b>isDelete([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Controleert of als de rij is gemarkeerd voor verwijderen. Transformaties die meer dan één invoerstroom kunnen de index (1-indeling) van de stroom doorgeven. De standaardwaarde voor de index van de stream is een * ``isDelete() -> true``
* ``isDelete(1) -> false``
*********************************
<code>isMatch</code>
==============================
<code><b>isMatch([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Controleert of als de rij is afgestemd op opzoeken. Transformaties die meer dan één invoerstroom kunnen de index (1-indeling) van de stroom doorgeven. Standaardwaarde voor de stream-index is 1 * ``isMatch() -> true``
* ``isMatch(1) -> false``
*********************************
<code>isError</code>
==============================
<code><b>isError([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Hiermee wordt gecontroleerd of de rij is gemarkeerd als fout. Transformaties die meer dan één invoerstroom kunnen de index (1-indeling) van de stroom doorgeven. De standaardwaarde voor de index van de stream is een * ``isError() -> true``
* ``isError(1) -> false``
*********************************
<code>isIgnore</code>
==============================
<code><b>isIgnore([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Hiermee wordt gecontroleerd of de rij is gemarkeerd om te worden genegeerd. Transformaties nemen meer dan één invoerstroom kunnen de index (1-indeling) van de stroom doorgeven. De standaardwaarde voor de index van de stream is een * ``isIgnore() -> true``
* ``isIgnore(1) -> false``
*********************************
<code>sum</code>
==============================
...
Hiermee haalt u de eerste waarde van de kolomgroep van een. Als de tweede parameter null-waarden is negeren wordt weggelaten, wordt ervan uitgegaan moet false zijn * ``first(sales) -> 12233.23``
...
Hiermee haalt u de waarde van de eerste parameter geëvalueerd n rijen voor de huidige rij. De tweede parameter is het aantal rijen om terug te kijken. De standaardwaarde is een. Als er niet zo veel rijen wordt waarde null geretourneerd als er een standaardwaarde is opgegeven * ``lag(amount, 2) -> 60``
* ``lag(amount, 2000, 100) -> 100``
*********************************
<code>lead</code>
==============================
<code><b>lead(<i>&lt;value&gt;</i> : any, [<i>&lt;number of rows to look after&gt;</i> : number], [<i>&lt;default value&gt;</i> : any]) => any</b></code><br/><br/>
Hiermee haalt de waarde van de eerste parameter geëvalueerd n rijen na de huidige rij. De tweede parameter is het aantal rijen dat u wilt kijken ernaar uit. De standaardwaarde is een. Als er niet zo veel rijen wordt waarde null geretourneerd als er een standaardwaarde is opgegeven * ``lead(amount, 2) -> 60``
