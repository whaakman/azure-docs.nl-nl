---
title: "Gegevens kopiëren van Shopify met behulp van Azure Data Factory (bèta) | Microsoft Docs"
...
---
# <a name="copy-data-from-shopify-using-azure-data-factory-beta"></a>Gegevens kopiëren van Shopify met behulp van Azure Data Factory (bèta)
...
U kunt gegevens uit Shopify kopiëren naar een ondersteunde sink-gegevensarchief. Zie voor een lijst van opgeslagen gegevens die worden ondersteund als bronnen/put door met de kopieerbewerking de [ondersteunde gegevensarchieven](copy-activity-overview.md#supported-data-stores-and-formats) tabel.
...
De volgende secties bevatten informatie over de eigenschappen die worden gebruikt voor het definiëren van Data Factory-entiteiten specifieke naar Shopify-connector.
...
| host | Het eindpunt van de server Shopify. (dat wil zeggen, mystore.myshopify.com)  | Ja |
| accessToken | Het toegangstoken API die kan worden gebruikt voor toegang tot Shopify van gegevens. Het token verloopt niet als het offlinemodus. U kunt kiezen voor dit veld markeren als een SecureString veilig opslaan in ADF of wachtwoord worden opgeslagen in Azure Sleutelkluis en de kopieeractiviteit pull daar bij het uitvoeren van de gegevens opnieuw te kopiëren: meer informatie kunt [referenties opgeslagen in de Sleutelkluis](store-credentials-in-key-vault.md). | Ja |
...
Zie voor een volledige lijst met secties en de eigenschappen die beschikbaar zijn voor het definiëren van gegevenssets, de [gegevenssets](concepts-datasets-linked-services.md) artikel. Deze sectie bevat een lijst met eigenschappen die ondersteund worden door Shopify dataset.

Stel de eigenschap type van de gegevensset om gegevens te kopiëren van Shopify, **ShopifyObject**. Er is geen aanvullende typespecifieke-eigenschap in dit type dataset.
...
Zie voor een volledige lijst met secties en de eigenschappen die beschikbaar zijn voor het definiëren van activiteiten, de [pijplijnen](concepts-pipelines-activities.md) artikel. Deze sectie bevat een lijst met eigenschappen die ondersteund worden door Shopify bron.

### <a name="shopifysource-as-source"></a>ShopifySource als bron

Om gegevens te kopiëren van Shopify, stelt u het brontype in de kopieerbewerking naar **ShopifySource**. De volgende eigenschappen worden ondersteund in de kopieerbewerking **bron** sectie:
