...
| Azure Blob Storage | Ja | Gebruik {date} en {time} tokens van de gebeurtenis velden in het patroon pad. Kies de notatie van datum, zoals jjjj/MM/DD, MM-DD-JJJJ, MM-DD-JJJJ. HH wordt gebruikt voor de tijdnotatie. Als onderdeel van de [preview](https://aka.ms/ASAPreview), blob uitvoer kan worden gepartitioneerd door een kenmerk met één aangepaste gebeurtenis {fieldname}. | De invoer partitionering voor volgt [volledig worden query's](stream-analytics-scale-jobs.md). | 
