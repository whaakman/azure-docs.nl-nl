...
- **Secondary site**: In the secondary site, there are one or more Hyper-V host servers running target VMs to which you replicate the primary VMs. These host servers are located in a VMM private cloud. The cloud can be on the primary server (if you only have a single VMM server) or on a secondary VMM server.
...
Here's what you need in Azure to deploy this scenario:
...
Here's what you need in the primary and secondary on-premises sites to deploy this scenario:
...
- If you want to set up network mapping during Site Recovery deployment here's what you need:
...
7. You fail over and recover as follows:
...
4. Select **Yes** to indicate you're using VMM to manage the Hyper-V hosts, and select **Yes** if you have a secondary VMM server. If you're deploying replication between clouds on a single VMM server you click **No**. Then click **OK**.
...
You can use bandwidth weight settings or limit traffic by bits per secondary. If you're using a cluster you need to do this on all cluster node. For more information see:
...
>[AZURE.NOTE] You can also enable protection for virtual machines in the VMM console. Click **Enable Protection** on the toolbar in the virtual machine properties > **Azure Site Recovery** tab.
...
2. In **Test Failover**, select **None**. With this option you test that the VM fails over as expected but the replicated VM won't be connected to any network.
