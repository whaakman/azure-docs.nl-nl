...
4. In **instellingen**, kunt u eventueel target-site-instellingen configureren:
    ...
    - **Abonnement van doel**: Het doelabonnement dat wordt gebruikt voor herstel na noodgevallen. Standaard is het doelabonnement niet hetzelfde als het bronabonnement.
    ...
    - **Doelbeschikbaarheidssets**: Azure Site Recovery maakt standaard een nieuwe beschikbaarheidsset in de doelregio met het achtervoegsel 'asr' voor de virtuele machines die deel uitmaken van een beschikbaarheidsset in de regio van de gegevensbron met de naam. Als de beschikbaarheidsset die wordt gemaakt met Azure Site Recovery, al bestaat, wordt deze opnieuw gebruikt.
    - **Doel van de beschikbaarheidszones**: Standaard wordt in Site Recovery de dezelfde zone-nummer als de regio van de gegevensbron in de doelregio toegewezen als de doelregio ondersteuning voor beschikbaarheidszones biedt.
...
1. Klik op **aanpassen:** replicatie-instellingen wijzigen.
   ...
   - Alle machines in een replicatiegroep hebben gedeelde crash-consistente en app-consistente herstelpunten bij een failover. Inschakelen van multi-VM-consistentie kan invloed hebben op prestaties van de werkbelasting (zoals het CPU-intensieve is) en moet alleen worden gebruikt als machines dezelfde werkbelasting worden uitgevoerd en u consistentie tussen meerdere computers. Bijvoorbeeld, als een toepassing 2 virtuele machines van de sql- en 2 webservers heeft vervolgens u moet alleen sql virtuele machines toevoegen als onderdeel van de replicatiegroep.
   - U kunt op maximaal 16 virtuele machines in een replicatiegroep.
