---
title: Een testfailover voor Hyper-V-replicatie (met System Center VMM) uitvoeren naar Azure | Microsoft Docs
description: Geeft een overzicht van de stappen die u nodig hebt voor een testfailover uitgevoerd voor Hyper-V-machines in VMM-clouds repliceren naar Azure met behulp van de Azure Site Recovery-service.
...
---
# <a name="step-11-run-a-test-failover-for-hyper-v-replication-with-vmm-to-azure"></a>Stap 11: Een testfailover voor Hyper-V-replicatie (met VMM) uitvoeren naar Azure

Nadat u [replicatie inschakelen voor Hyper-V-machines](vmm-to-azure-walkthrough-enable-replication.md), gebruik dit artikel voor een testfailover van on-premises Hyper-V virtuele machines in System Center Virtual Machine Manager (VMM)-clouds naar Azure, wordt beheerd met behulp van de [Azure Site Recovery](site-recovery-overview.md) service in de Azure portal.
...
- - Als het opslagaccount dat u voor replicatie gebruikt zijn versleuteld met versleuteling van opslag-Service, kunnen beheerde schijven kunnen niet worden gemaakt tijdens de failover. In dit geval niet een gebruik van beheerde schijven, inschakelen of schakel de beveiliging voor de virtuele machine en opnieuw inschakelen voor gebruik van een opslagaccount dat geen versleuteling ingeschakeld. [Meer informatie](../virtual-machines/windows/managed-disks-overview.md#managed-disks-and-encryption).
...
- [Meer informatie over failback](site-recovery-failback-from-azure-to-hyper-v.md), zodat een failback- en virtuele Azure-machines repliceren naar de primaire lokale VMM-cloud.
