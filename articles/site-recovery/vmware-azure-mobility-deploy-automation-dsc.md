...
ms.locfileid: "29812850"
---
# <a name="deploy-the-mobility-service-with-azure-automation-dsc"></a>De Mobility-service implementeren met behulp van Azure Automation DSC

Implementeren van de Mobility-service op virtuele VMware-machines en fysieke servers die u wilt repliceren naar Azure met behulp van [Azure Site Recovery](site-recovery-overview.md).

In dit artikel wordt beschreven hoe de service op Windows-machines met behulp van Azure Automation Desired State Configuration (DSC), om ervoor te zorgen dat implementeren:


## <a name="prerequisites"></a>Vereisten

* Een opslagplaats voor het opslaan van de vereiste instellingen
* Een opslagplaats voor het opslaan van de vereiste wachtwoordzin te registreren met de beheerserver. Een unieke wachtwoordzin wordt gegenereerd voor een specifieke configuratie-server. 
* Windows Management Framework (WMF) 5.0 moet worden geïnstalleerd op de machines die u wilt inschakelen voor beveiliging. Dit is een vereiste voor Automation DSC.
Als u wilt gebruiken DSC voor Windows-machines waarvoor WMF 4.0 is geïnstalleerd, raadpleegt u [DSC gebruiken in niet-verbonden omgevingen](## Use DSC in disconnected environments).
 

## <a name="extract-binaries"></a>Binaire bestanden extraheren

De Mobility-service kan worden geïnstalleerd via de opdrachtregel en verschillende argumenten. U moet ophalen van de binaire bestanden (nadat ze het uitpakken van uw installatie) en deze opslaan op een locatie waar u ze met behulp van een DSC-configuratie kunt terugvinden.

1. De bestanden die u nodig hebt om deze installatie wilt uitpakken, blader naar de volgende map op de beheerserver: **\Microsoft Azure Site Recovery\home\svsystems\pushinstallsvc\repository**
2. In deze map, Controleer of er een MSI-bestand met de naam: **Microsoft ASR_UA_version_Windows_GA_date_Release.exe**.
3. De volgende opdracht gebruiken om op te halen van het installatieprogramma: **.\Microsoft-ASR_UA_9.1.0.0_Windows_GA_02May2016_release.exe /q /x:C:\Users\Administrator\Desktop\Mobility_Service\Extract**
...
## <a name="store-the-passphrase"></a>Sla de wachtwoordzin

U moet bepalen waar u deze gecomprimeerde map geplaatst. U kunt een Azure storage-account gebruiken zoals later, voor het opslaan van de wachtwoordzin op die u nodig hebt voor de installatie. De agent wordt vervolgens geregistreerd met de beheerserver als onderdeel van het proces.

1. Sla de wachtwoordzin op die u hebt verkregen tijdens de implementatie van de configuratie-server in een tekstbestand - passphrase.txt.
2. Plaats de gecomprimeerde map en de wachtwoordzin in een specifieke container in Azure storage-account.
...
## <a name="create-the-dsc-configuration"></a>Maken van de DSC-configuratie
...
## <a name="upload-to-automation-dsc"></a>Uploaden naar Automation DSC
...
1. Aanmelden bij uw Automation-account, blader naar **activa** > **Modules**, en klik op **bladeren galerie**.
2. Zoeken naar de module en importeer ze in uw account.
3. Wanneer u klaar bent, gaat u naar de computer waar u de Azure Resource Manager-modules geïnstalleerd hebt en gaat u verder met het importeren van de zojuist gemaakte DSC-configuratie.

## <a name="import-cmdlets"></a>Cmdlets voor importeren

1. In PowerShell, moet u zich aanmelden bij uw Azure-abonnement.
2. Wijzig de cmdlets voor overeenstemming met uw omgeving en uw Automation-accountgegevens in een variabele vastleggen:

    ```powershell
    $AAAccount = Get-AzureRmAutomationAccount -ResourceGroupName 'KNOMS' -Name 'KNOMSAA'
    ```

3. Uploaden van de configuratie van Automation DSC met behulp van de volgende cmdlet:
...
1. De Automation DSC-configuratie compileren zodat u beginnen kunt met knooppunten te registreren. U bereiken die door de volgende cmdlet:
...
2. Nadat u de configuratie compileren, kunt u de informatie over de taak ophalen met behulp van PowerShell (Get-AzureRmAutomationDscCompilationJob) of met behulp van de [Azure-portal](https://portal.azure.com/).

    ![Taak ophalen](./media/vmware-azure-mobility-deploy-automation-dsc/retrieve-job.png)

U hebt nu gepubliceerd en DSC-configuratie geüpload naar Automation DSC.

## <a name="onboard-machines-to-automation-dsc"></a>Ingebouwde machines Automation DSC


1. Zorg ervoor dat uw Windows-machines worden bijgewerkt met de nieuwste versie van WMF. U kunt downloaden en installeren van de juiste versie voor uw platform van de [Downloadcentrum](https://www.microsoft.com/download/details.aspx?id=50395).
2. Maak een metaconfig voor DSC die u op de knooppunten worden toegepast. Als u wilt dit is gelukt, moet u voor het ophalen van de eindpunt-URL en de primaire sleutel voor uw geselecteerde Automation-account in Azure. U vindt deze waarden onder **sleutels** op de **alle instellingen** blade voor het Automation-account.
    ...
    ![Sleutelwaarden](./media/vmware-azure-mobility-deploy-automation-dsc/key-values.png)

In dit voorbeeld hebt u een fysieke Windows Server 2012 R2-server die u beveiligen wilt met behulp van Site Recovery.

## <a name="check-for-any-pending-file-rename-operations"></a>Controleren of er in behandeling zijnde bestand operations wijzigen
...
1. Voer de volgende cmdlet om te controleren of er geen opnieuw opstarten in behandeling op de server:

    ```powershell
    Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\' | Select-Object -Property PendingFileRenameOperations
    ```
2. Als u dit leeg ziet, bent u OK om door te gaan. Als dat niet het geval is, moet u dit oplossen door de server opnieuw opstarten tijdens een onderhoudsvenster.
3. De configuratie toepassen op de server, start PowerShell Integrated Scripting Environment (ISE) en voer het volgende script. Dit is in wezen een DSC lokale configuratie waarmee de lokale Configuration Manager-engine registreren bij de Automation DSC-service en de specifieke configuratie (ASRMobilityService.localhost) op te halen.
...
1. Nadat u hebt dit script uitvoert, moet het knooppunt eerst registreren bij Automation DSC.

    ![Knooppunt-registratie in voortgang](./media/vmware-azure-mobility-deploy-automation-dsc/register-node.png)

2. Als u terug naar de Azure-portal gaat, ziet u dat het nieuw ingeschreven knooppunt nu is gepubliceerd in de portal.

    ![Geregistreerde knooppunt in de portal](./media/vmware-azure-mobility-deploy-automation-dsc/registered-node.png)

3. U kunt de volgende PowerShell-cmdlet om te controleren of het knooppunt juist is geregistreerd kunt uitvoeren op de server:

    ```powershell
    Get-DscLocalConfigurationManager
    ```

4. Nadat de configuratie is opgehaald en toegepast op de server, kunt u dit controleren door de volgende cmdlet:
...
![Uitvoer](./media/vmware-azure-mobility-deploy-automation-dsc/successful-config.png)
...
![Geslaagde implementatie](./media/vmware-azure-mobility-deploy-automation-dsc/successful-install.png)
...
Nadat u de Mobility-service-agents te implementeren, kunt u [replicatie inschakelen](vmware-azure-tutorial.md) voor de virtuele machines.
