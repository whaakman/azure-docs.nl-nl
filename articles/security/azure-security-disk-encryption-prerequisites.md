...
Azure Disk Encryption is beschikbaar op virtuele machines die voldoen aan deze minimale geheugenvereisten:

| Virtuele Machine | Minimumvereiste geheugen |
|--|--|
| Virtuele Windows-machines | 2 GB |
| Linux-VM's bij het versleutelen van alleen gegevensvolumes| 2 GB |
| Linux-VM's bij het versleutelen van zowel gegevens als besturingssysteemvolumes, en het gebruik van het bestandssysteem hoofdpartitie (/) 4GB is of minder | 8 GB |
| Linux-VM's bij het versleutelen van zowel gegevens als besturingssysteemvolumes, en waarbij het gebruik van het bestandssysteem hoofdpartitie (/) is groter dan 4GB | Het gebruik van het bestandssysteem hoofdmap * 2. Bijvoorbeeld vereist een hoofdmap gebruik van het bestandssysteem van 16 GB ten minste 32GB aan RAM-geheugen |

Zodra het versleutelingsproces voor OS-schijf voltooid op virtuele Linux-machines is, kan de virtuele machine worden geconfigureerd om te worden uitgevoerd met minder geheugen. 

> [!NOTE]
> Schijfversleuteling voor Linux-besturingssysteem is niet beschikbaar voor [Virtual Machine Scale Sets](../virtual-machine-scale-sets/index.yml).
