...
Azure Redis-Cache kan ook worden gebruikt als een archief van de structuur in-memory-gegevens, een gedistribueerde, niet-relationele database en een berichtenbroker. De prestaties van toepassingen worden verbeterd door gebruik te maken van de snelle gegevensdoorvoer met lage latentie van de Redis-engine.

Azure Redis-Cache hebt dat u toegang tot een beveiligde, toegewezen Redis-cache. Azure Redis-Cache wordt beheerd door Microsoft, die worden gehost in Azure, en die toegankelijk is voor elke toepassing binnen of buiten Azure.

## <a name="using-azure-cache-for-redis"></a>Met behulp van Azure Cache voor Redis
...
| Laag | Description |
|---|---|
Basis | Een cache met één knooppunt. Deze laag biedt ondersteuning voor meerdere geheugengrootten (250 MB - 53 GB). Dit is een ideale laag voor het ontwikkelen/testen en voor niet-essentiële werkbelastingen. De Basic-laag heeft geen SLA (Service Level Agreement) |
...
| Omschrijving | Premium | Standard | Basis |
| ------------------- | :-----: | :------: | :---: |
