...
Als u bijvoorbeeld ondersteuning voor de `New-AzRedisCache` cmdlet, type:

    Get-Help New-AzRedisCache -Detailed

### <a name="how-to-connect-to-other-clouds"></a>Verbinding maken met andere clouds
Standaard de Azure-omgeving is `AzureCloud`, die staat voor de globale Azure-cloud-exemplaar. Gebruiken voor verbinding met een ander exemplaar, de `Connect-AzAccount` opdracht met de `-Environment` of -`EnvironmentName` opdrachtregelparameter met de gewenste omgeving of de naam van de omgeving.

Als u wilt zien van de lijst met beschikbare omgevingen, de `Get-AzEnvironment` cmdlet.
...
Nieuwe Azure-Cache voor Redis-exemplaren worden gemaakt met behulp van de [New-AzRedisCache](https://docs.microsoft.com/powershell/module/az.rediscache/new-azrediscache) cmdlet.

> [!IMPORTANT]
> De eerste keer dat u een Azure-Cache maken voor Redis in een abonnement met behulp van de Azure portal, de portal registreert de `Microsoft.Cache` naamruimte voor dat abonnement. Als u probeert te maken van de eerste Azure-Cache voor Redis in een abonnement met behulp van PowerShell, moet u eerst registreren die naamruimte met de volgende opdracht. anders cmdlets zoals `New-AzRedisCache` en `Get-AzRedisCache` mislukken.
> 
> `Register-AzResourceProvider -ProviderNamespace "Microsoft.Cache"`
> 
> 

Voor een overzicht van de parameters en hun beschrijvingen voor `New-AzRedisCache`, voer de volgende opdracht uit.
...
De `databases` instelling alleen tijdens het maken van de cache kan worden geconfigureerd. Het volgende voorbeeld wordt een premium P3 (26 GB)-cache met 48 databases met behulp van de [New-AzRedisCache](https://docs.microsoft.com/powershell/module/az.rediscache/New-azRedisCache) cmdlet.

    New-AzRedisCache -ResourceGroupName myGroup -Name mycache -Location "North Central US" -Sku Premium -Size P3 -RedisConfiguration @{"databases" = "48"}

Voor meer informatie over de `databases` eigenschap, Zie [standaard Azure Cache voor configuratie van Redis-server](cache-configure.md#default-redis-server-configuration). Voor meer informatie over het maken van een cache met de [New-AzRedisCache](https://docs.microsoft.com/powershell/module/az.rediscache/new-azrediscache) cmdlet, Zie de vorige te maken van een Azure-Cache voor Redis-sectie.

## <a name="to-update-an-azure-cache-for-redis"></a>Bijwerken van een Azure-Cache voor Redis
Azure voor instanties van Redis-Cache zijn bijgewerkt met de [Set AzRedisCache](https://docs.microsoft.com/powershell/module/az.rediscache/Set-azRedisCache) cmdlet.

Voor een overzicht van de parameters en hun beschrijvingen voor `Set-AzRedisCache`, voer de volgende opdracht uit.
...
`Set-AzRedisCache` kan worden gebruikt om de schaal van een Azure-Cache voor Redis-exemplaar als de `Size`, `Sku`, of `ShardCount` eigenschappen worden gewijzigd. 
...
Nadat u deze opdracht is uitgevoerd, de status van de cache wordt geretourneerd (vergelijkbaar met aanroepen `Get-AzRedisCache`). Houd er rekening mee dat de `ProvisioningState` is `Scaling`.
...
U kunt informatie over het gebruik van een cache ophalen de [Get-AzRedisCache](https://docs.microsoft.com/powershell/module/az.rediscache/get-azrediscache) cmdlet.

Voor een overzicht van de parameters en hun beschrijvingen voor `Get-AzRedisCache`, voer de volgende opdracht uit.
...
Uitvoeren als u wilt retourneren informatie over alle caches in het huidige abonnement, `Get-AzRedisCache` zonder parameters.

    Get-AzRedisCache

Uitvoeren als u wilt retourneren informatie over alle caches in een specifieke resourcegroep, `Get-AzRedisCache` met de `ResourceGroupName` parameter.

    Get-AzRedisCache -ResourceGroupName myGroup

Uitvoeren om informatie te krijgen over een specifieke cache, `Get-AzRedisCache` met de `Name` parameter met de naam van de cache en de `ResourceGroupName` parameter met de resourcegroep met deze cache.
...
Als u wilt de toegangssleutels voor uw cache ophalen, kunt u de [Get-AzRedisCacheKey](https://docs.microsoft.com/powershell/module/az.rediscache/Get-azRedisCacheKey) cmdlet.

Voor een overzicht van de parameters en hun beschrijvingen voor `Get-AzRedisCacheKey`, voer de volgende opdracht uit.
...
Als wilt ophalen van de sleutels voor uw cache, roept de `Get-AzRedisCacheKey` cmdlet en geef de naam van de cache met de naam van de resourcegroep met de cache.
...
Als u wilt genereren van de toegangssleutels voor uw cache, kunt u de [New-AzRedisCacheKey](https://docs.microsoft.com/powershell/module/az.rediscache/New-azRedisCacheKey) cmdlet.

Voor een overzicht van de parameters en hun beschrijvingen voor `New-AzRedisCacheKey`, voer de volgende opdracht uit.
...
Als u wilt genereren van de primaire of secundaire sleutel voor uw cache, Roep de `New-AzRedisCacheKey` cmdlet en geef de naam, resourcegroep, en opgeven of `Primary` of `Secondary` voor de `KeyType` parameter. In het volgende voorbeeld wordt de secundaire toegangssleutel voor een cache opnieuw gegenereerd.
...
Als u wilt verwijderen van een Azure-Cache voor Redis, gebruikt u de [Remove-AzRedisCache](https://docs.microsoft.com/powershell/module/az.rediscache/remove-azrediscache) cmdlet.

Voor een overzicht van de parameters en hun beschrijvingen voor `Remove-AzRedisCache`, voer de volgende opdracht uit.
...
U kunt gegevens importeren in een Azure-Cache voor het gebruik van Redis-exemplaar de `Import-AzRedisCache` cmdlet.
...
Voor een overzicht van de parameters en hun beschrijvingen voor `Import-AzRedisCache`, voer de volgende opdracht uit.
...
U kunt gegevens exporteren uit een Azure-Cache voor het gebruik van Redis-exemplaar de `Export-AzRedisCache` cmdlet.
...
Voor een overzicht van de parameters en hun beschrijvingen voor `Export-AzRedisCache`, voer de volgende opdracht uit.
...
U kunt uw Azure-Cache voor het gebruik van Redis-exemplaar opnieuw de `Reset-AzRedisCache` cmdlet.
...
Voor een overzicht van de parameters en hun beschrijvingen voor `Reset-AzRedisCache`, voer de volgende opdracht uit.
...
* [Azure Cache voor Redis-cmdlet-documentatie op MSDN](https://docs.microsoft.com/powershell/module/az.rediscache)
