...
IoT Edge-runtime is een verzameling van programma's die een apparaat in een IoT Edge-apparaat omzetten. Gezamenlijk de IoT Edge-runtime-onderdelen voor het ontvangen van code worden uitgevoerd aan de rand IoT Edge-apparaten inschakelen en kennis van de resultaten. 
...
IoT Edge hub kunt bepalen of deze verbonden met IoT Hub. Als de verbinding verbroken wordt, IoT Edge hub-berichten of dubbele updates lokaal wordt opgeslagen. Zodra een verbinding opnieuw tot stand is gebracht, worden deze gesynchroniseerd met alle gegevens. De locatie die wordt gebruikt voor deze tijdelijke cache wordt bepaald door een eigenschap van de moduledubbel van de IoT Edge hub. De grootte van de cache wordt niet beperkt tot, en zullen groeien zolang het apparaat heeft opslagcapaciteit. Zie voor meer informatie, [offlinemogelijkheden](offline-capabilities.md).

### <a name="module-communication"></a>Module-communicatie

IoT Edge hub vergemakkelijkt de communicatie van de module-module. Met behulp van IoT Edge houdt hub als een berichtenbroker modules onafhankelijk van elkaar. Modules hoeft alleen te geven van de invoer waarop ze berichten en de uitvoer waaraan ze berichten schrijven accepteren. Een ontwikkelaar van de oplossing kan samen te voegen deze invoer en uitvoer zodat de modules verwerken van gegevens in de volgorde die specifiek zijn voor die oplossing. 
...
De ontwikkelaar van de oplossing is verantwoordelijk voor het opgeven van de regels die bepalen hoe IoT Edge hub wordt doorgegeven berichten tussen modules. Regels voor doorsturen zijn gedefinieerd in de cloud en verlaagd met de IoT Edge hub in de apparaatdubbel. Dezelfde syntaxis voor routes voor IoT Hub wordt gebruikt voor het definiëren van routes tussen modules in Azure IoT Edge. Zie voor meer informatie, [meer informatie over het implementeren van modules en routes instellen in IoT Edge](module-composition.md).   
...
* **restartPolicy** : de manier waarop de IoT Edge-agent opnieuw wordt opgestart een module. Mogelijke waarden:
   * `never` : De IoT Edge-agent wordt nooit de module opnieuw gestart.
   * `on-failure` -Als de module vastloopt, de IoT Edge-agent opnieuw worden opgestart. Als de module nu foutloos wordt afgesloten, de IoT Edge-agent niet opnieuw wordt opgestart deze.
   * `on-unhealthy` -Als de module vastloopt of als slecht beschouwd, de IoT Edge agent start deze opnieuw op.
   * `always` -Als de module vastloopt, wordt beschouwd als niet in orde of op geen enkele manier worden afgesloten, wordt het door de IoT Edge-agent opnieuw. 
...
De IoT Edge-agent verzendt runtimereactie naar IoT Hub. Hier volgt een lijst van mogelijke reacties:
  ...
  * 417 - het apparaat beschikt niet over de configuratie van een implementatie instellen.
...
Zie voor meer informatie, [meer informatie over het implementeren van modules en routes instellen in IoT Edge](module-composition.md).   
