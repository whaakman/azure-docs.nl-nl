---
title: Client testen om te valideren vooraf een Azure-SaaS-toepassing | Microsoft Docs
description: Het maken van een client testen voor het vooraf valideren van een Azure-SaaS-toepassing.
...
---
# <a name="create-a-self-test-client-to-pre-validate-an-azure-saas-application"></a>Een client testen voor het vooraf valideren van een Azure-SaaS-toepassing maken
...
De API heeft de volgende structuur.
...
|          DSN-naam          |         De DNS-naam van de virtuele machine die u gaat testen.          |
|          Gebruiker          |          Gebruikersnaam voor verbinding met en aanmelden bij de virtuele machine.         |
|           Wachtwoord         |         Het wachtwoord voor verbinding met en aanmelden bij de virtuele machine.          |
|          OS          |         Het besturingssysteem van de virtuele machine, 'Linux' of 'Windows'.          |
|         PortNo           |         Het open poortnummer om verbinding te maken met de virtuele machine. Het poortnummer dat is gewoonlijk "22" voor Linux en '5986' voor Windows.          |
...
De volgende Schermafbeelding toont een voorbeeld van JSON de resultaten weergegeven in een online JSON-viewer.
...
Voer de volgende stappen uit op de API-aanroep met cURL:
...
3. Selecteer op de navigatiebalk links **alle services** en selecteer vervolgens **Azure Active Directory**.
   ...
   In de volgende stappen, moet u wellicht de tenantnaam (of de mapnaam) of de tenant-ID (of map-ID).
...
20. Maken van een geheime sleutel door te selecteren van de sleutel **beschrijving** tekstvak. Configureer de volgende velden:
    ...
    - Onder **waarde**, Kopieer de sleutel.
     ...
     >[!Important]
     >U niet mogelijk om te zien van de sleutelwaarde nadat u het formulier sleutels hebt afgesloten.
...
Nadat u uw Azure-SaaS-toepassing met succes hebt getest, kunt u [de aanbieding publiceren](./cpp-publish-offer.md)