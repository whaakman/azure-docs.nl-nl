...
In Azure Search, gebruikt u de `$count`, `$top`, en `$skip` parameters om deze waarden te retourneren. Het volgende voorbeeld wordt een voorbeeld van een aanvraag voor totaal aantal treffers op een index met de naam 'online catalogus,' geretourneerd als `@odata.count`:
...
In Azure Search, zou u `$select` en een [zoeken-API-aanvraag](https://docs.microsoft.com/rest/api/searchservice/search-documents) voor het implementeren van deze ervaring.
