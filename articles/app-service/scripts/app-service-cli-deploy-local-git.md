...
Met dit voorbeeldscript wordt er een web-app gemaakt in App Service, inclusief de bijbehorende resources, en wordt vervolgens de code van de web-app geïmplementeerd in een lokale Git-opslagplaats.
...
| [`az webapp deployment user set`](/cli/azure/webapp/deployment/user?view=azure-cli-latest#az_webapp_deployment_user_set) | Hiermee stelt u de implementatiereferenties voor App Service in op accountniveau. |
| [`az webapp deployment source config-local-git`](/cli/azure/webapp/deployment/source?view=azure-cli-latest#az_webapp_deployment_source_config_local_git) | Hiermee maakt u een configuratie voor broncodebeheer voor een lokale Git-opslagplaats. |
