...
# <a name="get-started-with-windows-containers-in-a-kubernetes-cluster"></a>Aan de slag met Windows-containers in een Kubernetes-cluster
...
3. Typ het volgende om de toepassing te starten:
...
4. Typ het volgende als u de implementatie van de service wilt controleren (dit duurt ongeveer 30 seconden):

  ```
  kubectl get pods
  ```

5. Wanneer de service wordt uitgevoerd, typt u het volgende als u de interne en externe IP-adressen van de service wilt bekijken:
...
6. Wanneer het externe IP-adres beschikbaar is gemaakt, kunt u de service via uw webbrowser bereiken.
