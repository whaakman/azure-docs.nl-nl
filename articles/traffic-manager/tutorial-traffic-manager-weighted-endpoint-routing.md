---
title: 'Zelfstudie: Verkeer routeren naar gewogen eindpunten met behulp van Azure Traffic Manager | Microsoft Docs'
...
ms.locfileid: "49649431"
---
# <a name="tutorial-control-traffic-routing-with-weighted-endpoints-using-traffic-manager"></a>Zelfstudie: Verkeersroutering met gewogen eindpunten beheren met behulp van Traffic Manager 

In deze zelfstudie wordt beschreven hoe u Traffic Manager gebruikt om de routering van gebruikersverkeer tussen eindpunten te beheren via de methode voor gewogen routering. In deze routeringsmethode wijst u aan elk eindpunt in de Traffic Manager-profielconfiguratie een bepaald gewicht toe. Gebruikersverkeer wordt vervolgens gerouteerd op basis van het gewicht dat aan elk eindpunt is toegewezen. Het gewicht is een geheel getal tussen 1 en 1000. Hoe hoger het gewicht dat is toegewezen aan een eindpunt, hoe hoger de prioriteit van het eindpunt.
...
- twee virtuele machines voor het testen van de Traffic Manager - Een VM in **VS-Oost** en de tweede VM in **Europa - west**. De test-VM’s worden gebruikt om te illustreren hoe gebruikersverkeer via Traffic Manager naar een website wordt gerouteerd met een bijbehorend eindpunt waaraan een hoger gewicht is toegewezen.
...
In deze sectie maakt u een VM - *mVMEastUS*. U gaat deze VM gebruiken om te testen hoe verkeer via Traffic Manager naar het website-eindpunt wordt gerouteerd waaraan een hoger gewicht is toegewezen.
...
6. Selecteer onder **Maken** in het **Overzicht** **Maken** om de implementatie van de VM te starten.
8. Het maken van de virtuele machine duurt een paar minuten. Ga pas verder met de overige stappen als de VM is gemaakt.
...
2. Voer in  **Traffic Manager-profiel maken** de volgende gegevens in of selecteer deze, accepteer de standaardwaarden voor de overige instellingen en selecteer **Maken**:
    ...
    | Resourcegroep          | Selecteer **Bestaande** en selecteer *myResourceGroupTM1*. |
    |        |   |
...
Voeg de twee VM’s toe waarop de IIS-servers worden uitgevoerd- *myIISVMEastUS*  & *myIISVMWEurope* - om gebruikersverkeer naar deze VM’s te routeren.
...
4. Herhaal stap 2 en 3 om nog een eindpunt met de naam *myWestEuropeEndpoint* toe te voegen voor het openbare IP-adres *myIISVMWEurope-ip* dat is gekoppeld aan de IIS-server-VM met de naam *myIISVMWEurope*, en voer **25** in bij **Gewicht**. 
...
Voer de volgende stappen uit om de Traffic Manager in actie te zien:
...
4. Selecteer **OK**.
5. Er wordt mogelijk een certificaatwaarschuwing weergegeven tijdens het aanmelden. Als u de waarschuwing ontvangt, selecteert u **Ja** of **Doorgaan** om door te gaan met de verbinding. 
6. Typ in een webbrowser op de VM *myVMEastUS* de DNS-naam van uw Traffic Manager-profiel om uw website weer te geven. U wordt naar de website gerouteerd die wordt gehost op de IIS-server *myIISVMEastUS*, omdat hieraan een hoger gewicht is toegewezen (**100**) in vergelijking met de IIS-server *myIISVMWEurope* (waaraan het lager eindpuntgewicht van **25** is toegewezen).
...
Verwijder de resourcegroepen die u in deze zelfstudie hebt gemaakt, wanneer u ze niet meer nodig hebt. Selecteer daarvoor de resourcegroep (**ResourceGroupTM1** of **ResourceGroupTM2**) en selecteer vervolgens **Verwijderen**.

## <a name="next-steps"></a>Volgende stappen

> [!div class="nextstepaction"]
> [Verkeer routeren naar specifieke eindpunten op basis van de geografische locatie van een gebruiker](traffic-manager-configure-geographic-routing-method.md)
