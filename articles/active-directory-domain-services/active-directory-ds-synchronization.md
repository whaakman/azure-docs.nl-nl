...
Als u terugschrijven configureert, worden de veranderingen in uw Azure AD-directory gesynchroniseerd terug naar uw lokale Active Directory. Bijvoorbeeld, als u uw wachtwoord met behulp van Azure AD selfservice voor wachtwoordherstel management wijzigt, het gewijzigde wachtwoord wordt bijgewerkt in uw on-premises AD-domein.
...
Accounts van gebruikers, groepslidmaatschappen en referentie-hashes worden gesynchroniseerd vanuit uw Azure AD-tenant aan uw Azure AD Domain Services beheerd domein. Dit synchronisatieproces is automatisch. U hoeft niet te configureren, bewaken of beheren van dit synchronisatieproces. Initiële synchronisatie kan enkele uren tot duren enkele dagen duren afhankelijk van het aantal objecten in uw Azure AD-directory. Het duurt ongeveer 20-30 minuten voor wijzigingen die zijn aangebracht in Azure AD in uw beheerde domein worden bijgewerkt nadat de initiële synchronisatie is voltooid. Deze synchronisatie-interval is van toepassing op wachtwoord te wijzigen of wijzigingen in kenmerken die zijn aangebracht in Azure AD.
...
|:--- |:--- |
| displayName |displayName |
| displayName |SAMAccountName (soms mogelijk automatisch gegenereerd) |
| mail |mail |
| mailNickname |msDS-AzureADMailNickname |
| object-id |msDS-AzureADObjectId |
| onPremiseSecurityIdentifier |SID-geschiedenis |
...
## <a name="password-hash-synchronization-and-security-considerations"></a>Wachtwoord-hash overwegingen voor synchronisatie en beveiliging
Wanneer u Azure AD Domain Services inschakelt, wordt er in uw Azure AD-directory genereert en slaat wachtwoord-hashes in NTLM en Kerberos-compatibel indelingen. 

Voor bestaande cloud gebruikersaccounts, omdat Azure AD nooit hun wachtwoorden met leesbare tekst opgeslagen kunnen niet deze hashes automatisch worden gegenereerd. Daarom moet [cloud-gebruikers hun wachtwoord opnieuw instellen/wijzigen](active-directory-ds-getting-started-password-sync.md) om hun wachtwoord-hashes worden gegenereerd en opgeslagen in Azure AD. Voor elk cloud-gebruikersaccount gemaakt in Azure AD nadat Azure AD Domain Services is ingeschakeld, worden de wachtwoord-hashes gegenereerd en opgeslagen in de indelingen van NTLM en Kerberos-compatibel. 

Voor de accounts van gebruikers synchroniseren met on-premises AD dat met behulp van Azure AD Connect-synchronisatie, moet u [Azure AD Connect om te synchroniseren van de wachtwoord-hashes in de indelingen van NTLM en Kerberos-compatibel configureren](active-directory-ds-getting-started-password-sync-synced-tenant.md).

De NTLM en Kerberos-compatibele wachtwoord-hashes worden altijd opgeslagen in een versleutelde manier in Azure AD. Deze hashes worden versleuteld zodat alleen Azure AD Domain Services heeft toegang tot de ontsleutelingssleutels. Er zijn geen andere services of onderdelen in Azure AD heeft toegang tot de ontsleutelingssleutels. De versleutelingssleutels zijn uniek per Azure AD-tenant. Azure AD Domain Services synchroniseert de wachtwoord-hashes naar de domeincontrollers voor uw beheerde domein. Deze wachtwoordhashes zijn opgeslagen en beveiligd op deze domeincontrollers die vergelijkbaar is met hoe wachtwoorden worden opgeslagen en beveiligd op Windows Server AD-domeincontrollers. De schijven voor deze beheerde domeincontrollers zijn versleuteld in rust.
