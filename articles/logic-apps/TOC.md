# Overzicht
## [Overzicht van logische apps](logic-apps-what-are-logic-apps.md)
## [Lijst van connectors](../connectors/apis-list.md)
## [Enterprise Integration Pack](logic-apps-enterprise-integration-overview.md)

# Aan de slag
## [Een logische app maken](logic-apps-create-a-logic-app.md)
## [Functies van logische apps gebruiken](logic-apps-use-logic-app-features.md)
## [Overzicht van sjablonen](logic-apps-use-logic-app-templates.md)
## [Een app maken met een sjabloon](logic-apps-arm-provision.md)
## [Voorbeelden en scenario's](logic-apps-examples-and-scenarios.md)
## [Scenario voor Service Bus](logic-apps-scenario-function-sb-trigger.md)
## [B2B-verwerking](logic-apps-enterprise-integration-b2b.md)
## [XML-verwerking](logic-apps-enterprise-integration-xml.md)

# Procedures
## Ontwikkelen
### [Een connector maken](logic-apps-create-api-app.md)
### [Bestandssysteemconnector gebruiken](logic-apps-using-file-connector.md)
### [SAP-connector gebruiken](logic-apps-using-sap-connector.md)
### [Visual Studio-hulpprogramma's](logic-apps-deploy-from-vs.md)
### [Azure-functies gebruiken](logic-apps-azure-functions.md)
### [Logische apps als aanroepbare eindpunten](logic-apps-http-endpoint.md)
### [Lussen, bereiken en debatching](logic-apps-loops-and-scopes.md)
### [Uw aangepaste API gebruiken](logic-apps-custom-hosted-api.md)
### [Afhandeling van uitzonderingen](logic-apps-exception-handling.md)
### [Scenario voor afhandeling van uitzonderingen](logic-apps-scenario-error-and-exception-handling.md)
### [Afhandeling van inhoudstypen](logic-apps-content-type.md)
### [Werkstroom-definitie](logic-apps-author-definitions.md)
### [Tips voor probleemoplossing](logic-apps-diagnosing-failures.md)
### [Limieten en configuratie](logic-apps-limits-and-config.md)


## Enterprise Integration Pack (EIP) gebruiken
### [Overzicht van integratieaccounts](logic-apps-enterprise-integration-create-integration-account.md)
### [Overeenkomsten](logic-apps-enterprise-integration-agreements.md)
### [B2B-verwerking](logic-apps-enterprise-integration-b2b.md)
### [XML-verwerking](logic-apps-enterprise-integration-xml.md)
### [Verwerking van platte bestanden](logic-apps-enterprise-integration-flatfile.md)
### [XSLT-toewijzingen toevoegen](logic-apps-enterprise-integration-maps.md)
### [XML transformeren](logic-apps-enterprise-integration-transform.md)
### [XML valideren](logic-apps-enterprise-integration-xml-validation.md)
### [Certificaten gebruiken](logic-apps-enterprise-integration-certificates.md)
### [Partners toevoegen](logic-apps-enterprise-integration-partners.md)
### [Schema's toevoegen](logic-apps-enterprise-integration-schemas.md)
### [Metagegevens opslaan](logic-apps-enterprise-integration-metadata.md)
### [AS2-integratie](logic-apps-enterprise-integration-as2.md)
### [AS2-codering](logic-apps-enterprise-integration-as2-encode.md)
### [AS2-decodering](logic-apps-enterprise-integration-as2-decode.md)
### [EDIFACT-integratie](logic-apps-enterprise-integration-edifact.md)
### [EDIFACT-codering](logic-apps-enterprise-integration-edifact-encode.md)
### [EDIFACT-decodering](logic-apps-enterprise-integration-edifact-decode.md)
### [X12-integratie](logic-apps-enterprise-integration-x12.md)
### [X12-codering](logic-apps-enterprise-integration-x12-encode.md)
### [X12-decodering](logic-apps-enterprise-integration-x12-decode.md)


## Gateway gebruiken
### [On-premises verbinden](logic-apps-gateway-connection.md)
### [Installeren](logic-apps-gateway-install.md)

## Automatiseren
### [Automation-sjabloon](logic-apps-create-deploy-template.md)
### [Publiceren vanuit VS](logic-apps-deploy-from-vs.md)

## Beheren
### [Visual Studio Cloud Explorer gebruiken](logic-apps-manage-from-vs.md)
### [Uw logische app beveiligen](logic-apps-securing-a-logic-app.md)
### [Logische apps controleren](logic-apps-monitor-your-logic-apps.md)
### [B2B-berichten controleren](logic-apps-monitor-b2b-message.md)
#### [Volgschema voor AS2](logic-apps-track-integration-account-as2-tracking-schemas.md)
#### [Volgschema voor X12](logic-apps-track-integration-account-x12-tracking-schema.md)
#### [Aangepaste volgschema's](logic-apps-track-integration-account-custom-tracking-schema.md)
#### [B2B-berichten bijhouden](logic-apps-track-b2b-messages-omsportal.md)
#### [Query uitvoeren in OMS-portal](logic-apps-track-b2b-messages-omsportal-query-filter-control-number.md)

### [Integratieaccounts](logic-apps-enterprise-integration-accounts.md)
### [Prijzen](logic-apps-pricing.md)

# Naslaginformatie
## [PowerShell](/powershell/resourcemanager/azurerm.logicapp/v2.3.0/azurerm.logicapp)
## Schema-geschiedenis
### [Algemene beschikbaarheid](logic-apps-schema-2016-04-01.md)
### [Preview](logic-apps-schema-2015-08-01.md)
## [REST](/rest/api/logic/)

# Bronnen
## [Prijzen](https://azure.microsoft.com/pricing/details/logic-apps/)  
## [MSDN-forum](https://social.msdn.microsoft.com/Forums/azure/en-US/home?forum=azurelogicapps)
## [Stack Overflow](http://stackoverflow.com/questions/tagged/azure-logic-apps)
## [Service-updates](https://azure.microsoft.com/updates/?product=logic-apps)


<!--HONumber=Jan17_HO3-->


