---
title: Openbare Standard Load Balancer maken - Azure CLI | Microsoft Docs
...
---
# <a name="create-a-standard-load-balancer-to-load-balance-vms-using-azure-cli-20"></a>Standard Load Balancer maken om taken van VM's te verdelen via Azure CLI 2.0
...
In deze sectie wordt beschreven hoe u de volgende onderdelen van de load balancer kunt maken en configureren.
  ...
  - een back-end IP-pool waar de front-eindpool het netwerkverkeer op de load balancer heen stuurt.
  - een statustest die de status van de back-end-VM-exemplaren vaststelt.
  - een load balancer-regel die bepaalt hoe het verkeer over de VM's wordt gedefinieerd.
...
### <a name="create-the-load-balancer-rule"></a>De load balancer-regel maken
...
Maak een beschikbaarheidsset met [az vm availabilityset create](/cli/azure/network/nic#az_network_availabilityset_create)
...
U kunt een cloud-init-configuratiebestand maken om NGINX te installeren en een Node.js-app 'Hello World' uit te voeren op een virtuele Linux-machine. Maak in uw huidige shell een bestand met de naam cloud-init.txt en plak de volgende configuratie in de shell. Controleer of u het hele cloud-init-bestand correct kopieert, met name de eerste regel:
