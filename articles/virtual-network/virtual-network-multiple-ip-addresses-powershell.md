...
9. <a name="os"></a>Manually add all the secondary private IP addresses (IP addresses with *False* in the **Primary** column from the output in the previous step) to the TCP/IP configuration in the operating system. The private IP address assigned to *IPConfig-1* in step 5 is automatically assigned to the operating system via Azure DHCP, because it's the *Primary* configuration.
   ...
   6. Make sure you are the root user. If you are not, you can do this by using the following command:
      
           sudo -i 
   7. Update the configuration file of the network interface (assuming ‘eth0’). 
   ...
   8. Open the file: vi *filename*.
      
       You should see the following lines at the end of the file:
      
           auto eth0
           iface eth0 inet dhcp
   9. Add the following lines after the lines that exist in this file:
      
           iface eth0 inet static
           address <your private IP address here>
   10. Save the file by using the following command:
       
            :wq
   11. Reset the network interface with the following command: 
       ...
       > 
       > 
   12. Verify the IP address is added to the network interface with the following command:
   ...
   14. Make sure you are the root user. If you are not, you can do this by using the following command:
       
            sudo -i
   15. Enter your password and follow instructions as prompted. Once you are the root user, navigate to the network scripts folder with the following command:
       
            cd /etc/sysconfig/network-scripts
   16. List the related ifcfg files using the following command:
       
            ls ifcfg-*
       
        You should see *ifcfg-eth0* as one of the files.
   17. Copy the *ifcfg-eth0* file and name it *ifcfg-eth0:0* with the following command:
       
            cp ifcfg-eth0 ifcfg-eth0:0
   18. Edit the *ifcfg-eth0:0* file with the following command:
       
            vi ifcfg-eth1
   19. Change the device to the appropriate name in the file; *eth0:0* in this case, with the following command:
       
            DEVICE=eth0:0
   20. Change the *IPADDR = YourPrivateIPAddress* line to reflect the IP address.
   21. Save the file with the following command:
       
            (:wq)
   22. Restart the network services and make sure the changes are successful by running the following commands:
