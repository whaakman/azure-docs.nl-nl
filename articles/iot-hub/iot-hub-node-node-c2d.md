...
Azure IoT Hub is een volledig beheerde service waarmee stabiele en veilige tweerichtingscommunicatie tussen miljoenen apparaten inschakelen en een back-end oplossing. De [aan de slag met IoT Hub] zelfstudie ziet hoe:

* Een iothub maken.
* Inrichten van een apparaat-id in een IoT-hub.
* De code is een gesimuleerde apparaattoepassing dat apparaat-naar-cloud-berichten verzendt.
...
1. Maak een lege map met de naam `sendcloudtodevicemessage`. Maak in de map `sendcloudtodevicemessage` een package.json-bestand door achter de opdrachtprompt de volgende opdracht op te geven. Accepteer alle standaardwaarden:

    ```cmd/sh
    npm init
    ```

1. Bij de opdrachtprompt in de `sendcloudtodevicemessage` map, voer de volgende opdracht voor het installeren van de **azure-iothub** pakket:

    ```cmd/sh
    npm install azure-iothub --save
    ```

1. Maak met een teksteditor, een **SendCloudToDeviceMessage.js** bestand de `sendcloudtodevicemessage` map.

1. Voeg de volgende `require` instructies aan het begin van de **SendCloudToDeviceMessage.js** bestand:
...
1. Voeg de volgende code naar **SendCloudToDeviceMessage.js** bestand. Vervang de `{iot hub connection string}` aanduidingswaarde met de IoT Hub-verbindingsreeks voor de hub die u hebt gemaakt in de [aan de slag met IoT Hub] zelfstudie. Vervang de `{device id}` aanduiding voor items met de apparaat-ID van het apparaat dat u hebt toegevoegd in de [aan de slag met IoT Hub] zelfstudie:
...
1. Voeg de volgende functie levering Feedbackberichten naar de console afdrukken:
...
1. Opslaan en sluiten **SendCloudToDeviceMessage.js** bestand.
...
1. Bij een opdrachtprompt in de `sendcloudtodevicemessage` map de volgende opdracht om een cloud naar apparaat verzenden en wacht tot de bevestiging van feedback:
