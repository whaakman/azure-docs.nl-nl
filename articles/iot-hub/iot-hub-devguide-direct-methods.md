...
Elke methode apparaten is gericht op één apparaat. [Taken] [ lnk-devguide-jobs] bieden een manier om directe methoden aanroepen op meerdere apparaten en plannen van aanroepen van de methode voor niet-verbonden apparaten.
...
Raadpleeg [Cloud-naar-apparaat communicatie richtlijnen] [ lnk-c2d-guidance] bij twijfel tussen het gebruik van gewenste eigenschappen directe methoden of cloud-naar-apparaat-berichten.

## <a name="method-lifecycle"></a>Methode levenscyclus
Directe methoden zijn geïmplementeerd op het apparaat en mogelijk nul of meer van de invoervermeldingen in de nettolading van de methode correct wordt gestart. Aanroepen van een rechtstreekse methode via een gerichte service-URI (`{iot hub}/twins/{device id}/methods/`). Een apparaat ontvangt directe methoden via een apparaat-specifieke MQTT-onderwerp (`$iothub/methods/POST/{method name}/`) of via AMQP-koppelingen (`IoThub-methodname` en `IoThub-status` toepassingseigenschappen). 
...
Rechtstreekse methodeaanroepen op een apparaat zijn HTTPS-aanroepen die bestaan uit:
...
De back-end-app ontvangt een reactie die bestaat uit:
...
Aanroepen directe methoden met een module-ID wordt ondersteund in de C#-voorbeeld SDK (beschikbaar [hier](https://www.nuget.org/packages/Microsoft.Azure.Devices/1.16.0-preview-004)).
...
### <a name="amqp"></a>AMQP
#### <a name="method-invocation"></a>Aanroepen van de methode
Het apparaat ontvangt de directe methodeaanvragen door een koppeling ontvangen op adres maken `amqps://{hostname}:5671/devices/{deviceId}/methods/deviceBound`

Het AMQP-bericht wordt ontvangen op de koppeling ontvangen dat de aanvraag methode vertegenwoordigt. Het bevat het volgende:
...
* De correlatie-ID-eigenschap, die de aanvraag-ID die wordt doorgegeven in van de methode aanvraagbericht bevat
* Een eigenschap van de toepassing met de naam `IoThub-status`, waarin de gebruiker opgegeven methodestatus
* De hoofdtekst van het AMQP-bericht met de methode reactie als JSON
...
* [Bandbreedtebeperking en quota's] [ lnk-quotas] beschrijving van de quota die van toepassing zijn en het beperkingsgedrag u kunt verwachten wanneer u een IoT-Hub.
...
* [IoT Hub-querytaal voor apparaatdubbels, taken en berichtroutering] [ lnk-query] beschrijving van de IoT Hub-querytaal kunt u gegevens ophalen uit IoT Hub over uw apparaatdubbels en taken.
