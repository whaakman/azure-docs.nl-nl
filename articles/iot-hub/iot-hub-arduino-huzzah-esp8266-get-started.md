...
* Draadloos netwerk voor Feather HUZZAH ESP8266 verbinden.
...
1. Ubuntu afmelden en vervolgens weer aanmelden om de wijziging wordt weergegeven.
...
Installeer het pakket voor Feather HUZZAH ESP8266 in de Visual Studio-Code:

1. Open de map waarin de voorbeeldtoepassing is opgeslagen.
1. Open het bestand app.ino in de app-map in de Visual Studio-Code.
...
1. Type **Arduino** en selecteer **Arduino: bord Manager**.
1. In de **Arduino-bord Manager** tabblad **extra URL's**.
...
1. Klik op **vernieuwen pakket indexen**. Nadat het vernieuwen is voltooid, zoeken naar **esp8266**.
...
1. Voer `F1`, typt u **Arduino** en selecteer **Arduino: bord Config**.
1. Klik in het vak voor **bord geselecteerd:** en het type **esp8266**en selecteer vervolgens **Adafruit HUZZAH ESP8266 (esp8266)**.
...
1. Zoek naar de volgende bibliotheek de naam van één voor één. Voor elke bibliotheek die u vindt, klikt u op **installeren**.
   * `AzureIoTHub`
   * `AzureIoTUtility`
...
1. Voer `F1`, typt u **Arduino** en selecteer **Arduino: uploaden** te bouwen en implementeren van de voorbeeldtoepassing om te Feather HUZZAH ESP8266.
...
1. In het venster van de seriële monitor, ziet u de twee lijsten in de vervolgkeuzelijst in de rechterbenedenhoek.
