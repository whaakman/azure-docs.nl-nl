...
3. Pak de doos uit. Controleer of de inhoud van de doos bestaat uit de volgende items:
    - Een enkele behuizing gegevens in het Edge-apparaat
    - Twee netsnoeren
...
3. De back-end van de trein betrekken voor de seats op de verticale rack moer totdat het volledig en de vergrendeling op klikt in plaats. Herhaal deze stappen om te plaatsen en de front-apparaat op de verticale rack moer seat.
...
De volgende procedures wordt uitgelegd hoe uw gegevens in het Edge-apparaat voor energiebeheer en netwerk-kabel.

Voordat u begint met de bekabeling van uw apparaat, zorgt u ervoor dat u over de volgende zaken beschikt:

- Uw gegevens in het Edge-fysieke apparaat, uitgepakt, en het rack is gekoppeld.
- Twee netsnoeren.
...
> - De gegevens in het Edge-apparaat moet worden verbonden met het netwerk van datacenters, zodat deze gegevens uit de bronservers data kan opnemen.

Op uw gegevens in het Edge-apparaat:

- Het voorpaneel heeft schijfstations en een / uit-knop.

    - Er zijn 10 schijf sleuven vooraan in uw apparaat.
    - Sleuf 0 heeft een 240 GB SATA-schijf die wordt gebruikt als een besturingssysteemschijf. Sleuf 1 is leeg en sleuven 2 tot en met 9 zijn dat nvme SSD's gebruikt als gegevensschijven.
- Het vlak van het vorige bevat redundante voeding aanbod eenheden (PSUs).
- Het vlak van het vorige heeft zes netwerkinterfaces:
...
- Het vlak van het vorige heeft twee netwerkkaarten die overeenkomt met de 6-poorten:
...
1. Identificeer de verschillende poorten op het vlak van het vorige van uw apparaat.

    ![Back-gegevenslaag van een bekabelde apparaat](./media/data-box-edge-deploy-install/backplane-cabled.png)

2. Zoek de sleuven voor de schijf en de / uit-knop op de voorgrond van het apparaat.

    ![Front vlak van een apparaat](./media/data-box-edge-deploy-install/device-front-plane-labeled-1.png)
...
5. Druk op de knop om in te schakelen op het apparaat.
...
7. Verbind één of meer interfaces, bijvoorbeeld PORT 2, PORT 3, PORT 4, PORT 5 of PORT 6 aan het netwerk van het datacenter of internet.
    ...
    - Gebruik de kabels SFP + koperen voor de 25-10-GbE-netwerkinterfaces.
