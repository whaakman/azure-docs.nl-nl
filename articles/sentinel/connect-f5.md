---
title: Verzamelen van gegevens in Azure Sentinel Preview F5 | Microsoft Docs
description: Meer informatie over het verzamelen van gegevens in Azure Sentinel F5.
services: sentinel
...
U kunt Azure Sentinel verbinden met een apparaat F5 door op te slaan van de logboekbestanden als Syslog CEF. De integratie met Azure Sentinel kunt u gemakkelijk uitvoeren op analytics en query's voor gegevens van het logboekbestand van F5. Zie voor meer informatie over hoe Azure Sentinel gegevens op CEF neemt. [CEF verbinding maken met apparaten](connect-common-event-format.md).

> [!NOTE]
> - Gegevens worden opgeslagen in de geografische locatie van de werkruimte waarop u werkt met Azure Sentinel.

## <a name="step-1-connect-your-f5-appliance-using-an-agent"></a>Stap 1: Verbinding maken met uw F5-apparaat met behulp van een agent

Als u wilt verbinden met uw apparaat F5 Sentinel van Azure, moet u een agent op een specifieke virtuele machine (VM of on-premises) voor de ondersteuning van de communicatie tussen het apparaat en de Azure-Sentinel implementeren. U kunt de agent deploly automatisch of handmatig. Automatische implementatie is alleen beschikbaar als uw toegewezen machine is een nieuwe virtuele machine die u in Azure maken wilt. 
...
## <a name="step-2-forward-f5-logs-to-the-syslog-agent"></a>Stap 2: F5-logboeken doorsturen naar de Syslog-agent

F5 voor het doorsturen van Syslog-berichten in CEF-indeling naar uw Azure-werkruimte via de Syslog-agent configureren:

Ga naar F5 [configureren Application Security logboekregistratie](https://aka.ms/asi-syslog-f5-forwarding), en volg de instructies voor het instellen van externe logboekregistratie, met de volgende richtlijnen:
  - Stel de **type externe opslag** naar **CEF**.
  - Stel de **Protocol** naar **UDP**.
  - Stel de **IP-adres** naar het IP-adres van de Syslog-server.
  - Stel de **poortnummer** naar **514**, of de poort u instellen dat de agent moet worden gebruikt.
  - Stel de **faciliteit** naar de map die u in de Syslog-agent instelt (standaard de agent wordt ingesteld op **local4**).
  - U kunt instellen dat de **maximale grootte van de Query-tekenreeks** aan de grootte die u in de agent instelt.
...
In dit document hebt u geleerd hoe u verbinding maken met F5 toestellen Azure Sentinel. Zie voor meer informatie over Azure Sentinel, de volgende artikelen:
