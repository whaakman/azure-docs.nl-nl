---
title: Aan de slag met Azure Mobile Services voor HTML/JavaScript-apps | Microsoft Docs
...
4. Klik onder **Uw app downloaden en uitvoeren** op **Downloaden**.
    ...
    Hiermee downloadt u de websitebestanden voor de *takenlijst*-voorbeeldtoepassing die is verbonden met uw mobiele service. Sla het gecomprimeerde bestand op uw lokale computer op en noteer de opslaglocatie.
5. Controleer op het tabblad **Configureren** of `localhost` al wordt vermeld in de lijst **Aanvragen van hostnamen toestaan** onder **Cross-origin-resource delen (CORS)**. Als dat niet het geval is, typt u `localhost` in het veld **Hostnaam** en klikt u vervolgens op **Opslaan**.
   ...
   > [!IMPORTANT]
   > Als u de Quick Start-app op een andere webserver dan localhost implementeert, moet u de hostnaam van de webserver toevoegen aan de lijst **Aanvragen van hostnamen toestaan**. Zie voor meer informatie [Cross-origin-resources delen](http://msdn.microsoft.com/library/windowsazure/dn155871.aspx).
   > 
   > 
...
1. Blader naar de locatie waar u de gecomprimeerde projectbestanden hebt opgeslagen, vouw de bestanden op uw computer uit en start een van de volgende opdrachtbestanden vanuit de **server**-submap.
   ...
   * **launch-linux.sh** (Linux-computers)
     ...
     > [!NOTE]
     > Typ op een Windows-computer `R` wanneer PowerShell u vraagt te bevestigen dat u het script wilt uitvoeren. Mogelijk waarschuwt uw webbrowser u het script niet uit te voeren omdat het is gedownload van internet. Als dit gebeurt, moet u aangeven dat de browser moet verdergaan met het laden van het script.
     > 
     > 
...
3. Typ in de app zinvolle tekst, zoals *Voltooi de zelfstudie* in **Nieuwe taak invoeren** en klik op **Toevoegen**.
   ...
   > [!NOTE]
   > U kunt de code die toegang heeft tot uw mobiele service bekijken om gegevens op te vragen en in te voegen. Deze code vindt u in het bestand page.js.
   > 
   > 
