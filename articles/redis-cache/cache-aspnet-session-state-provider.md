...
Het NuGet-pakket downloadt en voegt de vereiste assembly-verwijzingen en voegt de volgende sectie in het bestand web.config. In deze sectie bevat de vereiste configuratie voor uw ASP.NET-toepassing op de Redis-Cache Sessiestatusprovider gebruiken.
...
Zodra deze stappen worden uitgevoerd, wordt uw toepassing is geconfigureerd voor het gebruik van de Redis-Cache Sessiestatusprovider. Als u sessiestatus in uw toepassing gebruikt, wordt deze opgeslagen in een Azure Redis-Cache-exemplaar.
...
Bekijk de [Cacheprovider voor ASP.NET voor Azure Redis Cache](cache-aspnet-output-cache-provider.md).
