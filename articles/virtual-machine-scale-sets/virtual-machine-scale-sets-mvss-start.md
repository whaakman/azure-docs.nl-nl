...
GitHub gebruiken om te controleren van onze minimale levensvatbaar scale set-sjabloon [azuredeploy.json](https://raw.githubusercontent.com/gatneil/mvss/minimum-viable-scale-set/azuredeploy.json).

In deze zelfstudie bekijken we de diff (`git diff master minimum-viable-scale-set`) ingesteld voor het maken van de minimale levensvatbaar schaal sjabloon stuk door stuk.

## <a name="define-schema-and-contentversion"></a>$Schema en contentVersion definiëren
Eerst definiëren we `$schema` en `contentVersion` in de sjabloon. De `$schema` element definieert de versie van de sjabloontaal en wordt gebruikt voor syntaxismarkering Visual Studio en vergelijkbare validatie-functies. De `contentVersion` element wordt niet gebruikt door Azure. In plaats daarvan kunt u de sjabloonversie.
...
Resource Manager-sjablonen kunnen u definiëren variabelen moet later in de sjabloon worden gebruikt. Het voorbeeld gebruikt niet alle variabelen zodat we het JSON-object hebt leeg.
...
Geef een locatie voor het virtuele netwerk, gebruiken we een [Resource Manager sjabloonfunctie](../azure-resource-manager/resource-group-template-functions.md). Deze functie moet tussen aanhalingstekens en vierkante haken als volgt: `"[<template-function>]"`. In dit geval gebruiken we de `resourceGroup` functie. Dit heeft geen argumenten en retourneert een JSON-object met metagegevens over de resourcegroep die deze implementatie wordt geïmplementeerd. De resourcegroep is ingesteld door de gebruiker op het moment van implementatie. We vervolgens index in dit JSON-object met `.location` de locatie van de JSON-object ophalen.
...
Elke Resource Manager-bron heeft zijn eigen `properties` sectie voor specifieke configuraties voor de resource. In dit geval wordt opgegeven dat het virtuele netwerk één subnet met het particuliere IP-adresbereik moet hebben `10.0.0.0/16`. Een schaalset zich altijd in één subnet. Dit kan geen subnetten omvatten.
...
De schaal ingesteld moeten weten welk besturingssysteem moet worden geplaatst op de virtuele machines. Hier maken we de virtuele machines met een volledig patches Ubuntu 16.04 TNS-installatiekopie.
...
De schaalaanpassingsset implementeert meerdere virtuele machines. In plaats van elke VM-naam geven, geven we `computerNamePrefix`. De schaalaanpassingsset voegt een index samen met het voorvoegsel voor elke virtuele machine, zodat de VM-namen hebben de vorm `<computerNamePrefix>_<auto-generated-index>`.

In het volgende fragment gebruiken we de parameters uit voordat u de gebruikersnaam voor de beheerder en het wachtwoord instellen voor alle VM's in de schaalset. We doen dit met de `parameters` sjabloonfunctie. Deze functie omvat een tekenreeks die Hiermee geeft u op welke parameter om te verwijzen naar en levert de waarde voor deze parameter.
...
Tot slot moet de netwerkconfiguratie voor de virtuele machines in de schaalset opgeven. In dit geval hoeft er alleen te geven de ID van het subnet eerder hebt gemaakt. Hiermee wordt de schaal is ingesteld op de netwerkinterfaces in dit subnet plaatsen schrijfopdrachten.
