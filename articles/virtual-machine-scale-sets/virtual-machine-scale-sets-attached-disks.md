...
Een andere manier om een schaalset te maken met gekoppelde gegevensschijven, is om een schaalset te definiëren in een Azure Resource Manager-sjabloon, een _dataDisks_-sectie op te nemen in het _storageProfile_ en de sjabloon te implementeren. De schijf van 50 GB en 100 GB in het vorige voorbeeld wordt gedefinieerd als in het volgende sjabloonvoorbeeld:
...
U kunt een gegevensschijf toevoegen aan een virtuele-machineschaalset met behulp van de Azure CLI-opdracht _az vmss disk attach_. Geef een LUN op die nog niet in gebruik is. In het volgende CLI-voorbeeld wordt een station van 50 GB aan LUN 3 toegevoegd:
...
U kunt een gegevensschijf verwijderen uit een virtuele-machineschaalset met behulp van de Azure CLI-opdracht _az vmss disk detach_. Met de volgende opdracht wordt bijvoorbeeld de schijf die is gedefinieerd op LUN 2 verwijderd:
```bash
