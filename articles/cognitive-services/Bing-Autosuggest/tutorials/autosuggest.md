---
title: Ophaalt voor Automatische suggestie van Bing voor Automatische suggestie API (cognitieve Azure-Services) gebruiken | Microsoft Docs
description: Toont hoe u Automatische suggestie van Bing.
services: cognitive-services
author: v-jaswel
manager: kamrani
ms.assetid: 2575A80C-FC74-4631-AE5D-8101CF2591D3
ms.service: cognitive-services
ms.component: bing-autosuggest
ms.topic: article
ms.date: 09/28/2017
ms.author: v-jaswel
ms.openlocfilehash: f66af61ed06f4c6a7fe36cd44aa7781b71bb18ea
ms.sourcegitcommit: 95d9a6acf29405a533db943b1688612980374272
ms.translationtype: MT
ms.contentlocale: nl-NL
ms.lasthandoff: 06/23/2018
ms.locfileid: "35345475"
---
# <a name="build-a-web-page-autosuggest-client"></a>Een automatische suggestie van webpagina-client maken

In deze zelfstudie hebt wij een webpagina waarmee gebruikers kunnen de API voor Automatische suggestie van Bing query bouwen.

In deze handleiding ontdekt u hoe u:

> [!div class="checklist"]
> - Een eenvoudige query maken die aan de API voor Automatische suggestie van Bing
> - Weergave van queryresultaten

## <a name="prerequisites"></a>Vereisten

Als u wilt volgen samen met de zelfstudie, moet u een abonnementssleutel voor de API voor Automatische suggestie van Bing. Als u nog geen hebt, [aanmelden voor een gratis proefversie](https://azure.microsoft.com/try/cognitive-services/?api=autosuggest-api).

## <a name="create-a-new-web-page"></a>Maak een nieuwe webpagina

Open een teksteditor. Maak een nieuw bestand met de naam, bijvoorbeeld autosuggest.html.

## <a name="html-header"></a>HTML-header

De gegevens van de HTML-header toevoegen en de sectie script als volgt beginnen.

```html
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Bing Autosuggest</title>

<style type="text/css">
    html, body, div, p, h1, h2 {font-family: Verdana, "Lucida Sans", sans-serif;}

    html, body, div, p  {font-weight: normal;}
    h1, h2 {font-weight: bold;}
    sup {font-weight: normal;}

    html, body, div, p  {font-size: 12px;}
    h1 {font-size: 20px;}
    h2 {font-size: 16px;}
    h1, h2 {clear: left;}

    img#logo {float: right;
</style>

<script type="text/javascript">
```

## <a name="getsubscriptionkey-function"></a>de functie getSubscriptionKey

De functie getSubscriptionKey retourneert de Automatische suggestie van Bing-API-sleutel. Deze ophaalt van lokale opslag (dat wil zeggen, een cookie) of de gebruiker gevraagd om, indien nodig.

De functie getSubscriptionKey beginnen en de cookienaam als volgt declareren.

```html
getSubscriptionKey = function() {

    var COOKIE = "bing-autosuggest-api-key";   // name used to store API key in key/value storage
```

De hulpfunctie findCookie retourneert de waarde van de opgegeven cookie; Als de cookie niet gevonden is, wordt er een lege tekenreeks.

```html
    function findCookie(name) {
        var cookies = document.cookie.split(";");
        for (var i = 0; i < cookies.length; i++) {
            var keyvalue = cookies[i].split("=");
            if (keyvalue[0].trim() === name) {
                return keyvalue[1];
            }
        }
        return "";
        }
```

De hulpfunctie getSubscriptionKeyCookie vraagt de gebruiker om de waarde van de Automatische suggestie van Bing-API-sleutel en retourneert de waarde van de sleutel.

```html
    function getSubscriptionKeyCookie() {
        var key = findCookie(COOKIE);
        while (key.length !== 32) {
            key = prompt("Enter Bing Autosuggest API subscription key:", "").trim();
            var expiry = new Date();
            expiry.setFullYear(expiry.getFullYear() + 2);
            document.cookie = COOKIE + "=" + key.trim() + "; expires=" + expiry.toUTCString();
        }
        return key;
    }
```

De hulpfunctie getSubscriptionKeyLocalStorage probeert eerst de Automatische suggestie van Bing-API-sleutel ophalen door het opzoeken van de juiste cookie. Als de cookie niet wordt gevonden, wordt de gebruiker de sleutelwaarde gevraagd. Vervolgens wordt de sleutelwaarde.

```html
    function getSubscriptionKeyLocalStorage() {
        var key = localStorage.getItem(COOKIE) || "";
        while (key.length !== 32)
            key = prompt("Enter Bing Autosuggest API subscription key:", "").trim();
        localStorage.setItem(COOKIE, key)
        return key;
    }
```

De hulpfunctie getSubscriptionKey reageert op één parameter **ongeldig**. Als **ongeldig** is **true**, getSubscriptionKey verwijdert de cookie die voor Automatische suggestie van Bing-API-sleutel bevat. Als **ongeldig** is **false**, getSubscriptionKey retourneert de waarde van de Automatische suggestie van Bing-API-sleutel.

```html
    function getSubscriptionKey(invalidate) {
        if (invalidate) {
            try {
                localStorage.removeItem(COOKIE);
            } catch (e) {
                document.cookie = COOKIE + "=";
            }
        } else {
            try {
                return getSubscriptionKeyLocalStorage();
            } catch (e) {
                return getSubscriptionKeyCookie();
            }
        }
    }
```

De getSubscriptionKey Help-functie worden geretourneerd als het resultaat van de buitenste getSubscriptionKey-functie. Sluit de definitie van de buitenste getSubscriptionKey-functie.

```html
    return getSubscriptionKey;

}();
```

## <a name="helper-functions"></a>Help-functies

De pre-Help-functie retourneert de opgegeven tekst vooraf opgemaakt met de [pre](https://www.w3schools.com/tags/tag_pre.asp) HTML-tag.

```html
function pre(text) {
    return "<pre>" + text.replace(/&/g, "&amp;").replace(/</g, "&lt;") + "</pre>"
}
```

De opgegeven resultaten van de Bing voor Automatische suggestie API, met een JSON worden weergegeven in de functie renderSearchResults afdrukken.

```html
function renderSearchResults(results) {
    document.getElementById("results").innerHTML = pre(JSON.stringify(results, null, 2));
}
```

De functie renderErrorMessage bevat het foutbericht opgegeven en de foutcode.

```html
function renderErrorMessage(message, code) {
    if (code)
        document.getElementById("results").innerHTML = "<pre>Status " + code + ": " + message + "</pre>";
    else
        document.getElementById("results").innerHTML = "<pre>" + message + "</pre>";
}
```

## <a name="bingautosuggest-function"></a>de functie bingAutosuggest

De functie bingAutosuggest wordt telkens wanneer die de gebruiker krijgt de tekst in het HTML-formulierveld aangeroepen.
Deze twee parameters zijn vereist: de inhoud van het HTML-formulier veld en de sleutel voor Automatische suggestie van Bing-API.

```html
function bingAutosuggest(query, key) {
```

Geef het voor Automatische suggestie van Bing-API-eindpunt en een XMLHttpRequest-object die zullen worden gebruikt voor het verzenden van aanvragen naar het eindpunt declareren.

```html
    var endpoint = "https://api.cognitive.microsoft.com/bing/v7.0/Suggestions";

    var request = new XMLHttpRequest();

    try {
        request.open("GET", endpoint + "?q=" + encodeURIComponent(query));
    }
    catch (e) {
        renderErrorMessage("Bad request");
        return false;
    }
```

Stel de **Ocp-Apim-Subscription-Key** header met de waarde van de Automatische suggestie van Bing-API-sleutel.

```html
    request.setRequestHeader("Ocp-Apim-Subscription-Key", key);
```

Het antwoord van het eindpunt worden verwerkt. Als de status is 200 (OK), geef de resultaten; anders wordt de informatie over de fout weergegeven.

```html
    request.addEventListener("load", function() {
        if (this.status === 200) {
            renderSearchResults(JSON.parse(this.responseText));
        }
        else {
            if (this.status === 401) getSubscriptionKey(true);
            renderErrorMessage(this.statusText, this.status);
        }
    });
```

Ook mogelijke foutgebeurtenissen uit het object XMLHttpRequest verwerken.

```html
    request.addEventListener("error", function() {
        renderErrorMessage("Network error");
    });

    request.addEventListener("abort", function() {
        renderErrorMessage("Request aborted");
    });
```

Verzend de aanvraag. De functie bingAutosuggest, sluit de **script** label, en de **head** label.

```html
    request.send();
    return false;
}
// --></script>

</head>
```

## <a name="html-body"></a>De hoofdtekst HTML

Wanneer de webpagina wordt geladen, zorg er dan voor dat we hebben de Automatische suggestie van Bing-API-sleutel, vraagt de gebruiker voor deze indien nodig.

```html
<body onload="document.forms.bing.query.focus(); getSubscriptionKey();">
```

De Bing-logo weergeven.

```html
<img id="logo" align=base src="data:image/png;base64,">
```

Maak een HTML-formulier met een tekstveld. Verwerken de **oninput** gebeurtenis en de functie bingAutosuggest en het doorgeven van de inhoud van het tekstveld en de sleutel voor Automatische suggestie van Bing-API-aanroep.

```html
<form name="bing" oninput="return bingAutosuggest(this.query.value, getSubscriptionKey())">
    <h2>Autosuggest</h2>
    <input type="text" name="query" size="80" placeholder="Autosuggest" autocomplete=off>
</form>
```

De HTML-code toevoegen **div** code die we gebruiken om de resultaten weer te geven. We eerder gedefinieerde JavaScript verwijst naar deze **div** label.

```html
<h2>Results</h2>
<div id="results">
<p>None yet.</p>

</div>

</body>
</html>
```

Sla het bestand op.

## <a name="display-results"></a>Resultaten weer te geven

Open de webpagina in uw browser. Voer uw abonnementssleutel voor Automatische suggestie van Bing-API bij de prompt. Voer een query (bijvoorbeeld ' voeren') in de **Automatische suggestie** in het tekstvak. Terwijl u typt, wordt de webpagina die automatisch bijgewerkt om de Automatische suggestie resultaten weer te geven.

```json
{
  "_type": "Suggestions",
  "queryContext": {
    "originalQuery": "sail"
  },
  "suggestionGroups": [
    {
      "name": "Web",
      "searchSuggestions": [
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=vheQSvKZylM3dlX_B9bQ8-hQEsEJo8zDD2y7H1nsBjE&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailor%2bbrinkley%2bcook%26FORM%3dUSBAPI&p=DevEx,5003.1",
          "displayText": "sailor brinkley cook",
          "query": "sailor brinkley cook",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=EStLqAfxGCa44Ur3jEMXBv-Qp-lXUSFJbkBfnUdKKDg&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailor%2bbrinkley%26FORM%3dUSBAPI&p=DevEx,5004.1",
          "displayText": "sailor brinkley",
          "query": "sailor brinkley",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=gvtP9TS9NwhajSapY2Se6y1eCbP2fq_GiP2n-cxi6OY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailrite%26FORM%3dUSBAPI&p=DevEx,5005.1",
          "displayText": "sailrite",
          "query": "sailrite",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=c0QOA_j6swCZJy9FxqOwke2KslJE7ZRmMooGClAuCpY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailboats%2bfor%2bsale%26FORM%3dUSBAPI&p=DevEx,5006.1",
          "displayText": "sailboats for sale",
          "query": "sailboats for sale",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=mnMdREUH20SepmHQH1zlh9Hy_w7jpOlZFm3KG2R_BoA&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailing%2banarchy%26FORM%3dUSBAPI&p=DevEx,5007.1",
          "displayText": "sailing anarchy",
          "query": "sailing anarchy",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=0udadFl0gCTKCp0QmzQTXS3_y08iO8FpwsoKPHPS6kw&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailboatdata%26FORM%3dUSBAPI&p=DevEx,5008.1",
          "displayText": "sailboatdata",
          "query": "sailboatdata",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=BTS0G6AakxntIl9rmbDXtk1n6rQpsZZ99aQ7ClE7dTY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsail%2bsand%2bpoint%26FORM%3dUSBAPI&p=DevEx,5009.1",
          "displayText": "sail sand point",
          "query": "sail sand point",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=quBMwmKlGwqC5wAU0K7n416plhWcR8zQCi7r-Fw9Y0w&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailflow%26FORM%3dUSBAPI&p=DevEx,5010.1",
          "displayText": "sailflow",
          "query": "sailflow",
          "searchKind": "WebSearch"
        }
      ]
    }
  ]
}
```

## <a name="next-steps"></a>Volgende stappen

> [!div class="nextstepaction"]
> [Vereisten voor gebruik en weergeven](../UseAndDisplayRequirements.md)