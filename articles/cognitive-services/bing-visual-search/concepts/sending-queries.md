...
- Uploaden van een afbeelding (in binaire indeling).

## <a name="bing-visual-search-requests"></a>Bing visuele zoekopdrachten-aanvragen

Als u Visual Search een token van de installatiekopie of een URL verzendt, ziet het volgende fragment u de JSON-object dat u in de hoofdtekst van het bericht opnemen moet:
...
Het `imageInfo`-object moet het veld `url` of `imageInsightsToken` bevatten, maar niet beide. Stel de `url` veld naar de URL van een installatiekopie van een Internet toegankelijk is. De maximale ondersteunde afbeeldingsgrootte is 1 MB.
...
Het veld `cropArea` is optioneel. Het gebied bijsnijden Hiermee geeft u de linkerbovenhoek en de rechterbenedenhoek van een gebied van belang zijn. Geef waarden op in het bereik 0,0 t/m 1,0. De waarden zijn een percentage van de totale breedte of hoogte. In het bovenstaande voorbeeld wordt bijvoorbeeld de rechterhelft van de afbeelding gemarkeerd als het interessegebied. Neem dit op als u de inzichtaanvraag wilt beperken tot het interessegebied.
...
| <a name="market" />BingAPIs-market te versnellen    | Antwoordheader.<br /><br /> De markt die wordt gebruikt door de aanvraag. De notatie is \<languageCode\>-\<countryCode\>. Bijvoorbeeld: nl-NL.  |
...
| <a name="useragent" />Gebruikersagent  | Optionele aanvraagheader.<br /><br /> De gebruikersagent waarvan de aanvraag afkomstig is. Bing gebruikt de user-agent om mobiele gebruikers een geoptimaliseerde ervaring te bieden. Hoewel dit optioneel is, wordt u aangeraden deze header altijd op te geven.<br /><br /> De user-agent moet dezelfde tekenreeks zijn die elke veelgebruikte browser verzendt. Zie voor meer informatie over gebruikersagenten [RFC 2616](https://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html).<br /><br /> Hier volgen enkele voorbeelden van user-agent-tekenreeksen.<br /><ul><li>Windows Phone&mdash;Mozilla/5.0 (compatibel; MSIE 10.0; Windows Phone 8.0; Trident/6.0; IEMobile/10.0; ARM; Touch; NOKIA; Lumia 822)<br /><br /></li><li>Android&mdash;Mozilla/5.0 (Linux; U; Android 2.3.5; nl-nl; SCH-I500 Build/GINGERBREAD) AppleWebKit/533.1 (KHTML; like Gecko) Version/4.0 Mobile Safari/533.1<br /><br /></li><li>iPhone&mdash;Mozilla/5.0 (iPhone; CPU iPhone OS 6_1 like Mac OS X) AppleWebKit/536.26 (KHTML; zoals Gecko) Mobile/10B142 iPhone4;1 BingWeb/3.03.1428.20120423<br /><br /></li><li>PC&mdash;Mozilla/5.0 (Windows NT 6.3; WOW64; Trident/7.0; Touch; rv:11.0) zoals Gecko<br /><br /></li><li>iPad&mdash;Mozilla/5.0 (iPad; CPU OS 7_0 zoals Mac OS X) AppleWebKit/537.51.1 (KHTML, zoals Gecko) Version/7.0 Mobile/11A465 Safari/9537.53</li></ul>      |
...
Elke aanvraag moet bevatten de `Content-Type` header. De header moet worden ingesteld op: `multipart/form-data; boundary=\<boundary string\>`, waarbij \<grens tekenreeks\> is een unieke, ondoorzichtige tekenreeks die de grens van de gegevens identificeert. Bijvoorbeeld `boundary=boundary_1234-abcd`.

Als u Visual Search een token van de installatiekopie of een URL verzendt, het volgende fragment toont de gegevens dat moet u in de hoofdtekst van het bericht opnemen. De gegevens moet bevatten de `Content-Disposition` kop- en u moet instellen de `name` parameter 'knowledgeRequest'. Voor meer informatie over de `imageInfo` object, raadpleegt u de aanvraag.
...
Als u een lokale installatiekopie uploadt, het volgende fragment toont de gegevens dat moet u in de hoofdtekst van het bericht opnemen. De gegevens moet bevatten de `Content-Disposition` header. De parameter `name` moet worden ingesteld op "image" en de parameter `filename` kan op een willekeurige tekenreeks worden ingesteld. De `Content-Type` header kan worden ingesteld op een installatiekopie van de meest gebruikte mime-type. De inhoud van het formulier is de binaire gegevens van de installatiekopie. De maximale afbeeldingsgrootte die u kunt uploaden is 1 MB. De grootste zijde van de afbeelding (breedte of hoogte) moet 1500 pixels of minder zijn.
...
Het volgende codefragment laat zien hoe de regio van belang van geüploade afbeeldingen op te geven:
...
Het volgende fragment toont een aanvraag van de volledige installatiekopie inzichten die een token van de installatiekopie en de regio van belang zijn doorgegeven. Bij het ophalen van het insights-token van een vorige aanroep aan /images/search:
...
Als er Inzichten beschikbaar zijn voor de afbeelding, bevat het antwoord een of meer `tags` die de Inzichten bevatten. De `image` veld bevat de insights-token voor de afbeelding:
...
Zie voor een lijst van de standaard-inzichten, [insights Standaardtag](../default-insights-tag.md).

De overige tags bevatten andere inzichten die van belang kunnen zijn voor de gebruiker. Als de afbeelding bijvoorbeeld tekst bevat, kan een van de tags het inzicht TextResults bevatten, dat de herkende tekst bevat. Of, als Bing een entiteit (dat wil zeggen, een persoon, plaats of ding) in de afbeelding herkent, een van de labels kan worden geïdentificeerd door de entiteit. Visual Search retourneert ook een diverse reeks termen (tags) die zijn afgeleid van de ingevoerde afbeelding. Deze tags kunnen gebruikers om te verkennen concepten gevonden in de afbeelding. Als de invoerafbeelding bijvoorbeeld een beroemde atleet bevat, kan een van de tags Sport zijn, die koppelingen naar afbeeldingen van sport bevat.
...
Als de afbeelding tekst bevat die door de service wordt herkend, bevat een van de tags het inzicht TextResults (actie). Van het inzicht `displayName` bevat de herkende tekst:
...
Omdat het veld `displayName` van de tag ##TextRecognition bevat, moet u dit niet gebruiken als categorietitel in de UX. Dat geldt voor elke weergavenaam die begint met ##. In plaats daarvan gebruikt u de weergavenaam van de actie.
...
- [Zelfstudie: Een visuele zoekopdrachten één pagina web-app maken](../tutorial-bing-visual-search-single-page-app.md)
