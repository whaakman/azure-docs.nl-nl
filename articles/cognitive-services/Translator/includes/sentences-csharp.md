...
Maak in de klasse `Program` een functie met de naam `BreakSentence()`. Deze functie heeft vier argumenten: `subscriptionKey`, `host`, `route` `inputText`en.
...
`Main`U ziet dat in, u `subscriptionKey`declareert `host` `route`,, en de tekst die moet worden geëvalueerd `breakSentenceText`.
