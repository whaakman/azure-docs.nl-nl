...
Maak in `Program` de-klasse een asynchrone functie met `TransliterateTextRequest()`de naam. Deze functie heeft vier argumenten: `subscriptionKey`, `host`, `route` `inputText`en.
...
`Main`U ziet dat in, u `subscriptionKey`declareert `host` `route`,, en het script dat moet worden transliter `textToTransliterate`.
