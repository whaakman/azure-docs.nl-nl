...
Maak nu de URL. De URL is gebouwd met behulp van de methoden `Parse()` en `Query()`. U ziet dat de parameters worden toegevoegd met de methode `Add()`. In dit voorbeeld is sprake van transliteratie van Japans naar het Latijnse alfabet.
