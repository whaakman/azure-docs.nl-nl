---
title: Ophaalt spellen controleren met behulp van Bing spellen controleren API (Microsoft cognitieve Services in Azure) | Microsoft Docs
description: Laat zien hoe spellingcontrole van Bing gebruiken.
services: cognitive-services
author: v-jaswel
manager: kamrani
ms.assetid: 2575A80C-FC74-4631-AE5D-8101CF2591D3
ms.service: cognitive-services
ms.component: bing-spell-check
ms.topic: article
ms.date: 09/28/2017
ms.author: v-jaswel
ms.openlocfilehash: 4e4cdbb8a3d6ab01888d8f273083155c33eb06c1
ms.sourcegitcommit: 95d9a6acf29405a533db943b1688612980374272
ms.translationtype: MT
ms.contentlocale: nl-NL
ms.lasthandoff: 06/23/2018
ms.locfileid: "35344735"
---
# <a name="build-a-web-page-spell-check-client"></a>Een client webpagina spellingcontrole bouwen

In deze zelfstudie hebt wij een webpagina waarmee gebruikers kunnen de Bing-API voor het controleren van spellen query bouwen.

In deze handleiding ontdekt u hoe u:

> [!div class="checklist"]
> - Een eenvoudige query maken die aan de Bing-API voor het controleren van spellen
> - Weergave van queryresultaten

## <a name="prerequisites"></a>Vereisten

Als u wilt volgen samen met de zelfstudie, moet u een abonnementssleutel voor de Bing-API voor het controleren van spellen. Als u nog geen hebt, [aanmelden voor een gratis proefversie](https://azure.microsoft.com/try/cognitive-services/?api=spellcheck-api).

## <a name="create-a-new-web-page"></a>Maak een nieuwe webpagina

Open een teksteditor. Maak een nieuw bestand met de naam, bijvoorbeeld spellcheck.html.

## <a name="add-html-header"></a>HTML-koptekst toevoegen

De gegevens van de HTML-header toevoegen en de sectie script als volgt beginnen.

```html
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8"> 
    <title>Bing Spell Check</title>

<style type="text/css">
    html, body, div, p, h1, h2 {font-family: Verdana, "Lucida Sans", sans-serif;}

    html, body, div, p  {font-weight: normal;}
    h1, h2 {font-weight: bold;}
    sup {font-weight: normal;}

    html, body, div, p  {font-size: 12px;}
    h1 {font-size: 20px;}
    h2 {font-size: 16px;}
    h1, h2 {clear: left;}

    img#logo {float: right;
</style>

<script type="text/javascript">
```

## <a name="getsubscriptionkey-function"></a>de functie getSubscriptionKey

De functie getSubscriptionKey retourneert de Bing spellen controleren API-sleutel. Deze ophaalt van lokale opslag (dat wil zeggen, een cookie) of de gebruiker gevraagd om, indien nodig.

De functie getSubscriptionKey beginnen en de cookienaam als volgt declareren.

```html
getSubscriptionKey = function() {

    var COOKIE = "bing-spell-check-api-key";   // name used to store API key in key/value storage
```

De hulpfunctie findCookie retourneert de waarde van de opgegeven cookie; Als de cookie niet gevonden is, wordt er een lege tekenreeks.

```html
    function findCookie(name) {
        var cookies = document.cookie.split(";");
        for (var i = 0; i < cookies.length; i++) {
            var keyvalue = cookies[i].split("=");
            if (keyvalue[0].trim() === name) {
                return keyvalue[1];
            }
        }
        return "";
        }
```

De hulpfunctie getSubscriptionKeyCookie vraagt de gebruiker om de waarde van de Bing spellen controleren API-sleutel en retourneert de waarde van de sleutel.

```html
    function getSubscriptionKeyCookie() {
        var key = findCookie(COOKIE);
        while (key.length !== 32) {
            key = prompt("Enter Bing Spell Check API subscription key:", "").trim();
            var expiry = new Date();
            expiry.setFullYear(expiry.getFullYear() + 2);
            document.cookie = COOKIE + "=" + key.trim() + "; expires=" + expiry.toUTCString();
        }
        return key;
    }
```

De hulpfunctie getSubscriptionKeyLocalStorage probeert eerst de Bing spellen controleren API-sleutel ophalen door het opzoeken van de juiste cookie. Als de cookie niet wordt gevonden, wordt de gebruiker de sleutelwaarde gevraagd. Vervolgens wordt de sleutelwaarde.

```html
    function getSubscriptionKeyLocalStorage() {
        var key = localStorage.getItem(COOKIE) || "";
        while (key.length !== 32)
            key = prompt("Enter Bing Spell Check API subscription key:", "").trim();
        localStorage.setItem(COOKIE, key)
        return key;
    }
```

De hulpfunctie getSubscriptionKey reageert op één parameter **ongeldig**. Als **ongeldig** is **true**, getSubscriptionKey verwijdert de cookie die de Bing spellen controleren API-sleutel bevat. Als **ongeldig** is **false**, getSubscriptionKey retourneert de waarde van de Bing spellen controleren API-sleutel.

```html
    function getSubscriptionKey(invalidate) {
        if (invalidate) {
            try {
                localStorage.removeItem(COOKIE);
            } catch (e) {
                document.cookie = COOKIE + "=";
            }
        } else {
            try {
                return getSubscriptionKeyLocalStorage();
            } catch (e) {
                return getSubscriptionKeyCookie();
            }
        }
    }
```

De getSubscriptionKey Help-functie worden geretourneerd als het resultaat van de buitenste getSubscriptionKey-functie. Sluit de definitie van de buitenste getSubscriptionKey-functie.

```html
    return getSubscriptionKey;

}();
```

## <a name="helper-functions"></a>Help-functies

De pre-Help-functie retourneert de opgegeven tekst vooraf opgemaakt met de [pre](https://www.w3schools.com/tags/tag_pre.asp) HTML-tag.

```html
function pre(text) {
    return "<pre>" + text.replace(/&/g, "&amp;").replace(/</g, "&lt;") + "</pre>"
}
```

De opgegeven resultaten van de Bing spellen controleren API, met een JSON worden weergegeven in de functie renderSearchResults afdrukken.

```html
function renderSearchResults(results) {
    document.getElementById("results").innerHTML = pre(JSON.stringify(results, null, 2));
}
```

De functie renderErrorMessage bevat het foutbericht opgegeven en de foutcode.

```html
function renderErrorMessage(message, code) {
    if (code)
        document.getElementById("results").innerHTML = "<pre>Status " + code + ": " + message + "</pre>";
    else
        document.getElementById("results").innerHTML = "<pre>" + message + "</pre>";
}
```

## <a name="bingspellcheck-function"></a>de functie bingSpellCheck

De functie bingSpellCheck wordt telkens wanneer die de gebruiker krijgt de tekst in het HTML-formulierveld aangeroepen.
Deze twee parameters zijn vereist: de inhoud van het HTML-formulier veld en de spelling controleren Bing-API-sleutel.

```html
function bingSpellCheck(query, key) {
```

Geef het Bing spellen controleren API-eindpunt en een object XMLHttpRequest, die zullen worden gebruikt voor het verzenden van aanvragen naar het eindpunt declareren.

```html
    var endpoint = "https://api.cognitive.microsoft.com/bing/v7.0/spellcheck/";

    var request = new XMLHttpRequest();

    try {
        request.open("GET", endpoint + "?mode=proof&mkt=en-US&text=" + encodeURIComponent(query));
    }
    catch (e) {
        renderErrorMessage("Bad request");
        return false;
    }
```

Stel de **Ocp-Apim-Subscription-Key** header met de waarde van de Bing spellen controleren API-sleutel.

```html
    request.setRequestHeader("Ocp-Apim-Subscription-Key", key);
```

Het antwoord van het eindpunt worden verwerkt. Als de status is 200 (OK), geef de resultaten; anders wordt de informatie over de fout weergegeven.

```html
    request.addEventListener("load", function() {
        if (this.status === 200) {
            renderSearchResults(JSON.parse(this.responseText));
        }
        else {
            if (this.status === 401) getSubscriptionKey(true);
            renderErrorMessage(this.statusText, this.status);
        }
    });
```

Ook mogelijke foutgebeurtenissen uit het object XMLHttpRequest verwerken.

```html
    request.addEventListener("error", function() {
        renderErrorMessage("Network error");
    });

    request.addEventListener("abort", function() {
        renderErrorMessage("Request aborted");
    });
```

Verzend de aanvraag. De functie bingSpellCheck, sluit de **script** label, en de **head** label.

```html
    request.send();
    return false;
}
// --></script>

</head>
```

## <a name="html-body"></a>De hoofdtekst HTML

Wanneer de webpagina wordt geladen, zorg er dan voor dat we hebben de Bing spellen controleren API-sleutel, vraagt de gebruiker voor deze indien nodig.

```html
<body onload="document.forms.bing.query.focus(); getSubscriptionKey();">
```

De Bing-logo weergeven.

```html
<img id="logo" align=base src="data:image/png;base64,">
```

Maak een HTML-formulier met een tekstveld. Verwerken de **onsubmit** gebeurtenis en de aanroep van de functie bingSpellCheck, de inhoud van het tekstveld en de spelling controleren Bing-API-sleutel wordt doorgegeven.

```html
<form name="bing" onsubmit="return bingSpellCheck(this.query.value, getSubscriptionKey())">
    <h2>Spell Check</h2>
    <input type="text" name="query" size="80" placeholder="Spell Check" autocomplete=off>
</form>
```

De HTML-code toevoegen **div** code die we gebruiken om de resultaten weer te geven. We eerder gedefinieerde JavaScript verwijst naar deze **div** label.

```html
<h2>Results</h2>
<div id="results">
<p>None yet.</p>

</div>

</body>
</html>
```

Sla het bestand op.

## <a name="display-results"></a>Resultaten weer te geven

Open de webpagina in uw browser. Voer uw sleutel spellen controleren Bing-API-abonnement bij de prompt. Voer een query (bijvoorbeeld ' holle, wlrd!') in de **spellingcontrole** tekstvak in en druk op **Enter**. De webpagina worden vervolgens de queryresultaten weergegeven.

```json
{
  "_type": "SpellCheck",
  "flaggedTokens": [
    {
      "offset": 0,
      "token": "Hollo",
      "type": "UnknownToken",
      "suggestions": [
        {
          "suggestion": "Hello",
          "score": 0.856629936217145
        },
        {
          "suggestion": "Hollow",
          "score": 0.816717853225633
        }
      ]
    },
    {
      "offset": 7,
      "token": "wlrd",
      "type": "UnknownToken",
      "suggestions": [
        {
          "suggestion": "world",
          "score": 0.856629936217145
        }
      ]
    }
  ]
}
```

## <a name="next-steps"></a>Volgende stappen

> [!div class="nextstepaction"]
> [Vereisten voor gebruik en weergeven](../UseAndDisplayRequirements.md)
