---
title: Azure SQL-logische servers en individuele databases | Microsoft Docs
description: Meer informatie over logische Azure SQL Database-server en één database-concepten en hun resources.
...
ms.locfileid: "39091793"
---
# <a name="azure-sql-database-logical-servers-and-single-databases-and-their-resources"></a>Azure SQL Database-logische servers en individuele databases en hun resources
...
> [!TIP]
> Zie voor namen van geldige resource-groep en de server, [Naming Conventions](https://docs.microsoft.com/azure/architecture/best-practices/naming-conventions).
>
...
- Geeft het bereik voor management-beleidsregels die van toepassing op de betreffende databases: aanmeldingen, firewall, controleren, bedreigingen detecteren, enzovoort. 
...
- Een individuele database gemaakt binnen een [Azure-resourcegroep](../azure-resource-manager/resource-group-overview.md) met een [gecombineerde set met reken- en opslagresources](sql-database-service-tiers-dtu.md) of een [onafhankelijk schalen van reken- en opslagresources](sql-database-service-tiers-vcore.md). Een Azure SQL database is gekoppeld aan logische Azure SQL Database-server, die wordt gemaakt binnen een specifieke Azure-regio.
- Een database gemaakt als onderdeel van een [pool van databases](sql-database-elastic-pool.md) binnen een [Azure-resourcegroep](../azure-resource-manager/resource-group-overview.md) met een [gecombineerde set met reken- en -resources (DTU-indeling)](sql-database-service-tiers-dtu.md) of een [onafhankelijk schalen van reken- en opslagresources (vCore-gebaseerde)](sql-database-service-tiers-vcore.md) die worden gedeeld tussen alle databases in de groep. Een Azure SQL database is gekoppeld aan logische Azure SQL Database-server, die wordt gemaakt binnen een specifieke Azure-regio.

> [!IMPORTANT]
> SQL Database Managed Instance, momenteel in openbare preview is een [exemplaar van een SQL-server](sql-database-managed-instance.md) (een Managed Instance) wordt gemaakt binnen een [Azure-resourcegroep](../azure-resource-manager/resource-group-overview.md) met een gedefinieerde set met reken- en storage bronnen voor alle databases op die server-exemplaar. Een beheerd exemplaar bevat zowel systeem en gebruikersdatabases. Beheerd exemplaar is ontworpen om in te schakelen database lift-and-shift naar een volledig beheerde PaaS zonder het ontwerp van de toepassing. Managed Instance biedt extra compatibiliteit met het on-premises SQL Server-programmeermodel en biedt ondersteuning voor de grote meerderheid van SQL Server-functies en bijbehorende tools en services. Zie [SQL Database Managed Instance](sql-database-managed-instance.md) voor meer informatie. De rest van dit artikel geldt niet voor beheerd exemplaar.

## <a name="tds-and-tcpip-connections"></a>TDS- en TCP/IP-verbindingen

Microsoft Azure SQL Database biedt ondersteuning voor tabelgegevens stream (TDS Tabular)-protocol clientversie 7.3 of hoger en kunnen alleen versleutelde TCP/IP-verbindingen.

## <a name="azure-sql-databases-protected-by-sql-database-firewall"></a>Azure SQL databases hebt beschermd met SQL Database-firewall

Om uw gegevens te beveiligen een [SQL Database-firewall](sql-database-firewall-configure.md) voorkomt alle toegang tot uw database-server of een van de databases van buiten uw verbinding met de server rechtstreeks via de verbinding van uw Azure-abonnement. Aanvullende als connectiviteit wilt inschakelen, moet u [maken van een of meer firewallregels](sql-database-firewall-configure.md#creating-and-managing-firewall-rules). Zie voor het maken en beheren van elastische pools, [elastische pools](sql-database-elastic-pool.md).
