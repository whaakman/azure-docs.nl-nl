...
## <a name="whatis-the-new-vcore-based-purchasing-model-for-azure-sql-database"></a>Het nieuwe vCore gebaseerde aankopen model voor Azure SQL Database Whatis?

De nieuwe aankoopmodel is in aanvulling op het bestaande model op basis van DTU. Het model op basis van vCore is ontworpen om klanten flexibiliteit, beheer, transparantie en een eenvoudige manier om te vertalen lokale werklastvereisten naar de cloud. Ook kunnen klanten scale rekenkracht en opslag op basis van hun werkbelasting behoeften. Individuele database en met het model vCore elastische pool-opties zijn ook in aanmerking voor van 30 procent besparingen met het [Azure hybride gebruik voordeel voor SQL Server](../virtual-machines/windows/hybrid-use-benefit-licensing.md). Zie [Servicelagen](sql-database-service-tiers.md) voor meer informatie over het DTU-aankoopmodel zowel op basis van vCore aankoopmodel.
...
## <a name="how-should-i-choose-between-the-dtu-based-purchasing-model-vs-the-vcore-based-purchasing-model"></a>Hoe moet ik kiezen tussen de vs aankopen model op basis van DTU vCore gebaseerde aankoopmodel? 
...
Vandaag de dag bieden we SQL Server-klanten met Software Assurance license mobility voordelen waardoor hernieuwde toewijzing van de licenties voor servers van derden die worden gedeeld. Deze vergoeding kan worden gebruikt op Azure IaaS en AWS EC2.
...
U kunt het bewaren van back-up configureren voor PITR tussen 7 en 35 dagen. De back-ups opslag brengt afzonderlijk op basis van de werkelijke opslagverbruik als deze de opslaghoeveelheid gelijk aan de maximale grootte overschrijdt. In voorbeeld van standaard is de bewaarperiode PITR ingesteld op 7 dagen. In veel gevallen is de maximale grootte voor het opslaan van 7 dagen van back-ups voldoende.
...
Facturen SQL-Database op een voorspelbare per uur tarief op basis van de [aanschaffen van een model](sql-database-service-tiers.md). Werkelijke gebruik wordt berekend en pro rato per uur uitgevoerd, zodat uw factuur mogelijk breuken van een uur weer. Als een database voor 12 uur in een maand bestaat, ziet uw factuur u bijvoorbeeld informatie over het gebruik van 0,5 dagen. 
...
Zie voor meer informatie [prijzen SQL Database](https://azure.microsoft.com/pricing/details/sql-database/) en [Servicelagen](sql-database-service-tiers.md).
