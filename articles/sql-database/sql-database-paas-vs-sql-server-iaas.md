...
- [Azure SQL Database](https://azure.microsoft.com/services/sql-database/): Een volledig beheerde SQL database-engine, op basis van de nieuwste stabiele Enterprise-editie van SQL Server. Dit is een relationele database-as-a-service (DBaaS) die wordt gehost in de Azure-cloud die in de branchecategorie van valt *Platform-as-a-Service (PaaS)*. [SQL-database](sql-database-technical-overview.md) is gebouwd op gestandaardiseerde hardware en software die eigendom is van, en wordt gehost en beheerd door Microsoft. Met SQL Database, kunt u ingebouwde functies en functionaliteiten die uitgebreide configuratie in SQL Server vereist. Als u SQL Database gebruikt, betaalt u per gebruik met opties voor het omhoog schalen voor meer vermogen zonder onderbrekingen. SQL-Database heeft extra functies die niet beschikbaar in SQL Server, zoals ingebouwde intelligentie en het beheer zijn. Azure SQL Database biedt verschillende implementatieopties:
  - U kunt een individuele database te implementeren een [SQL Database-server](sql-database-servers.md). Een zelfstandige en gepoolde databases met SQL Database-server biedt de meeste functies van SQL Server-database-scoped van. Deze optie is geoptimaliseerd voor het ontwikkelen van nieuwe toepassingen cloudgegevens moderne toepassingen.
...
| SQL Server op VM | Azure SQL-Database (beheerd exemplaar) | Azure SQL-Database (individuele database of elastische pool) |
| --- | --- | --- |
...
| | Azure SQL Database<br>SQL Database-servers, elastische pools en individuele databases | Azure SQL Database<br>Beheerd exemplaar |Virtuele Azure-machine<br>SQL Server |
| --- | --- | --- |---|
...
|  | Teams die ingebouwde hoge beschikbaarheid, herstel na noodgevallen en upgrademogelijkheden van de database nodig hebben. | Hetzelfde als de databases op Azure SQL Database-server. | Teams die kunnen configureren, fijn afstemmen, aanpassen en beheren van hoge beschikbaarheid, herstel na noodgevallen en patchen voor SQL Server. Sommige geleverde geautomatiseerde functies vereenvoudigen dit aanzienlijk. | |
|  | Teams die het onderliggende besturingssysteem en configuratie-instellingen niet willen beheren. | Hetzelfde als de databases op Azure SQL Database-server. | U hebt een aangepaste omgeving met volledige beheerdersrechten nodig. | |
|  | Databases van maximaal 100 TB. | Hetzelfde als de databases op Azure SQL Database-server. | SQL Server-exemplaren met maximaal 64 TB aan opslag. Het exemplaar kan zoveel databases ondersteunen als nodig is. |
...
Met SQL Database, kunt u een servicelaag die past bij uw behoeften uit een breed scala aan prijzen die vanaf 5$ / maand voor Basic-laag.
