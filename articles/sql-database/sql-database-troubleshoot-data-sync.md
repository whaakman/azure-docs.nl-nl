...
#### <a name="the-client-agent-and-sync-group-are-in-different-data-centers"></a>De agent en sync clientgroep zich in verschillende data centers
...
-   Maak een nieuwe agent in hetzelfde Datacenter als de groep voor synchronisatie. Registreer de database met die agent. Zie [How To: een Clientagent synchroniseren van de SQL-gegevens (Preview) installeren](#install-a-sql-data-sync-client-agent) voor meer informatie.

-   Verwijder de huidige groep voor synchronisatie. Maak deze opnieuw in hetzelfde Datacenter als de agent.

#### <a name="the-client-agents-list-of-databases-is-not-current"></a>Lijst met databases van de clientagent is niet actueel
...
3. Voeg het gebruikersaccount in de **Meld u aan als een service-eigenschappen** dialoogvenster.
