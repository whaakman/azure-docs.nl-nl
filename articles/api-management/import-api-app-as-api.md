---
title: Importeren van een API-App als een API met de Azure portal | Microsoft Docs
description: Deze zelfstudie ziet u het gebruik van API Management (APIM) API-App als een API te importeren.
...
---
# <a name="import-an-api-app-as-an-api"></a>Een API-App als een API importeren

In dit artikel laat zien hoe een API-App als een API te importeren. Ook wordt uitgelegd hoe u kunt de API APIM testen.

In dit artikel leert u hoe:

> [!div class="checklist"]
> * Een API-App als een API importeren
...
+ Zorg ervoor dat er een API-App in uw abonnement. Zie voor meer informatie [-App Service-documentatie] [https://docs.microsoft.com/azure/app-service/]
...
2. Selecteer **API-App** van de **een nieuwe API toevoegen** lijst.

    ! (API-app) [. / media/import-api-app-as-api/api-app.png]
3. Druk op **Bladeren** voor een overzicht van API Apps in uw abonnement.
4. Selecteer de app. APIM zoekt de swagger die zijn gekoppeld aan de geselecteerde app deze kan worden opgehaald en worden geïmporteerd. 
    ...
    Als APIM swagger niet wordt gevonden, wordt deze de API als een 'pass-through'-API. 
