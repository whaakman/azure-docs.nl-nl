---
title: Problemen met synchronisatie van wachtwoordhash met Azure AD Connect-synchronisatie | Microsoft Docs
description: In dit artikel bevat informatie over het oplossen van problemen met de wachtwoord-hash synchronisatie.
...
Voor de implementatie van Azure Active Directory (Azure AD) verbinding maken met versie 1.1.614.0 of na, gebruikt u de taak voor het oplossen van problemen in de wizard wachtwoord-hash synchronisatieproblemen kunt oplossen:
...
Voor implementatie met versie 1.1.524.0 of hoger, er is een diagnostische cmdlet die u gebruiken kunt om op te lossen synchronisatieproblemen wachtwoord-hash:
...
6. Selecteer in het hoofdmenu **problemen met synchronisatie van wachtwoordhash**.

7. Selecteer in het submenu **synchronisatie van wachtwoordhash werkt niet op alle**.
...
* Hiermee valideert u of de wachtwoordfunctie-hash-synchronisatie is ingeschakeld voor uw Azure AD-tenant.
...
* Voor elk bestaand on-premises Active Directory-connector (die overeenkomt met een bestaand Active Directory-forest):
   ...
   * Hiermee valideert u of de wachtwoordfunctie-hash-synchronisatie is ingeschakeld.
   
   * Hiermee zoekt u wachtwoord-hash-synchronisatie heartbeat gebeurtenissen in de gebeurtenislogboeken van Windows-toepassing.
   ...
   * Voor elk Active Directory-domein onder de lokale Active Directory-connector:
      ...
      * Hiermee valideert u of de Active Directory Domain Services (AD DS)-accounts die worden gebruikt door de lokale Active Directory-connector heeft de juiste gebruikersnaam, wachtwoord en machtigingen die vereist zijn voor de synchronisatie van wachtwoordhash.

Het volgende diagram illustreert de resultaten van de cmdlet voor een lokale Active Directory-topologie van één domein:

![Diagnostische uitvoer voor synchronisatie van wachtwoordhash](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phsglobalgeneral.png)
...
#### <a name="password-hash-synchronization-feature-isnt-enabled"></a>wachtwoord-hash-synchronisatiefunctie is niet ingeschakeld
Als u synchronisatie van wachtwoordhash nog niet hebt ingeschakeld met behulp van de Azure AD Connect-wizard, wordt de volgende fout geretourneerd:

![synchronisatie van wachtwoordhash is niet ingeschakeld](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phsglobaldisabled.png)

#### <a name="azure-ad-connect-server-is-in-staging-mode"></a>Azure AD Connect-server is in de faseringsmodus
Als de Azure AD Connect-server zich in de faseringsmodus, synchronisatie van wachtwoordhash is tijdelijk uitgeschakeld en de volgende fout is geretourneerd:

![Azure AD Connect-server is in de faseringsmodus](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phsglobalstaging.png)

#### <a name="no-password-hash-synchronization-heartbeat-events"></a>Er zijn geen wachtwoord-hash synchronisatie heartbeat-gebeurtenissen
Elke lokale Active Directory-connector heeft een eigen synchronisatiekanaal wachtwoord-hash. Wanneer de wachtwoord-hash-synchronisatiekanaal tot stand is gebracht en er zijn geen wachtwoordwijzigingen kunnen worden gesynchroniseerd, wordt elke 30 minuten onder het gebeurtenislogboek van Windows-toepassing in een heartbeat-gebeurtenis (EventId 654) gegenereerd. Voor elke lokale Active Directory-connector zoekt u de cmdlet heartbeat er overeenkomende gebeurtenissen in de afgelopen drie uur. Als er geen heartbeat-gebeurtenis wordt gevonden, wordt de volgende fout geretourneerd:

![Er is geen wachtwoord-hash synchronisatie hartslag gebeurtenis](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phsglobalnoheartbeat.png)
...
![Onjuiste referenties](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phsglobalaccountincorrectpermission.png)
...
![Onjuiste referenties](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phsglobalaccountincorrectcredential.png)
...
* Controleert of er synchronisatieregels met synchronisatie van wachtwoordhash ingeschakeld en toegepast op de Active Directory-object.
...
Het volgende diagram illustreert de resultaten van de cmdlet wanneer u problemen met synchronisatie van wachtwoordhash voor een enkel object:

![Diagnostische uitvoer voor synchronisatie van wachtwoordhash - enkel object](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phssingleobjectgeneral.png)
...
synchronisatie van wachtwoordhash voor deze lokale Active Directory-account is mislukt omdat er geen bijbehorende object in de Azure AD-tenant. De volgende fout is geretourneerd:

![Azure AD-object ontbreekt.](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phssingleobjectnotexported.png)
...
![Tijdelijke wachtwoord wordt niet geëxporteerd.](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phssingleobjecttemporarypassword.png)
...
Azure AD Connect, slaat de resultaten van synchronisatiepogingen van wachtwoord-hash voor de zeven dagen. Als er geen resultaten beschikbaar voor het geselecteerde object in de Active Directory zijn, wordt de volgende waarschuwing geretourneerd:

![Diagnostische uitvoer voor één object - geen wachtwoordgeschiedenis voor synchronisatie](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phssingleobjectnohistory.png)
...
2. Het script uitvoeren in de [ophalen van de status van synchronisatie-instellingen voor wachtwoord](#get-the-status-of-password-sync-settings) sectie. Dit biedt u een overzicht van de configuratie van de wachtwoord-synchronisatie.  
    ...
    ![PowerShell-scriptuitvoer van de synchronisatie-instellingen voor wachtwoord](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/psverifyconfig.png)  
...
4. Ga naar **verbinding maken met Active Directory-Forest**.  
    ...
    ![Account dat wordt gebruikt door Active Directory-connector](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/connectoraccount.png)  
...
6. De domeincontrollers bereikbaar zijn voor Azure AD Connect? Als de Connect-server kan geen verbinding met alle domeincontrollers, configureert u **voorkeur domeincontroller alleen gebruiken**.  
    ...
    ![-Domeincontroller die wordt gebruikt door Active Directory-connector](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/preferreddc.png)  
...
U kunt eenvoudig wachtwoord-hash-synchronisatieproblemen oplossen aan de hand van de status van een object.
...
1. In **Active Directory: gebruikers en Computers**, zoeken naar de gebruiker en controleer vervolgens of de **gebruiker moet wachtwoord bij volgende aanmelding wijzigen** selectievakje is uitgeschakeld.  
    ...
    ![Active Directory-productief wachtwoorden](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/adprodpassword.png)  
...
2. Als het wachtwoord in Active Directory juist zoekt, volgt u de gebruiker in de synchronisatie-engine. Door de gebruiker van de lokale Active Directory naar Azure AD te volgen, kunt u zien of er een beschrijvend foutbericht op het object is.
    ...
    ![Zoeken naar de gebruiker in de connectorruimte met de DN-naam](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/searchcs.png)  
    ...
    ![Object-logboekgegevens](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/csobjectlog.png)  
    ...
    ![Informatie over een gebruiker Lineage](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/cspasswordsync.png)  

    i. Klik op **eigenschappen Metaverse-Object** om een lijst met gebruikerskenmerken weer te geven.  

    ![Metaverse-informatie](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/mvpasswordsync.png)  
    ...
    ![Metaverse-informatie](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/mvconnectors.png)  
    ...
    ![Dialoogvenster voor eigenschappen van het Object ruimte connector](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/cspasswordsync2.png)  
...
* [Implementatie van synchronisatie van wachtwoordhash met Azure AD Connect-synchronisatie](active-directory-aadconnectsync-implement-password-hash-synchronization.md)
