...
1. Typ in het zoekvak **Salesforce Sandbox**, selecteer **Salesforce Sandbox** van resultaat deelvenster klik vervolgens op **toevoegen** om toe te voegen van de toepassing.
...
1. **[Maak een testgebruiker Salesforce Sandbox](#create-a-salesforce-sandbox-test-user)**  : als u wilt een equivalent van Britta Simon in Salesforce-Sandbox die is gekoppeld aan de Azure AD-weergave van de gebruiker hebben.
...
1. Op de **Salesforce sandbox-domein en URL's** sectie, voert u de volgende stappen uit:
    ...
    a. In de **aanmeldings-URL** tekstvak typt u de waarde met behulp van het volgende patroon: `https://<instancename>--Sandbox.<entityid>.my.salesforce.com`

    b. In de **id** tekstvak typt u de waarde met behulp van het volgende patroon: `https://<instancename>--Sandbox.<entityid>.my.salesforce.com`
...
1. Op de **Salesforce Sandbox configuratie** sectie, klikt u op **configureren Salesforce Sandbox** openen **aanmelding configureren** venster. Kopiëren de **SAML entiteit-ID en Single Sign-On Service URL voor SAML-** uit de **Naslaggids sectie.**

    ![Eenmalige aanmelding configureren](./media/salesforce-sandbox-tutorial/tutorial_salesforcesandbox_configure.png) 

1. Open een nieuw tabblad in uw browser en meld u aan bij uw Salesforce-Sandbox administrator-account.
...
1. In het gedeelte SAML Single Sign-On-instellingen op de volgende stappen uitvoeren:
    ...
    c. In de **entiteit-Id** tekstvak, type `https://<instancename>--Sandbox.<entityid>.my.salesforce.com` als dit het eerste exemplaar van Salesforce Sandbox die u aan uw directory toevoegen wilt. Als u al hebt toegevoegd een exemplaar van Salesforce Sandbox, klikt u vervolgens voor de **entiteit-ID** typt u in de **aanmelding URL**, die moet worden in deze indeling: `https://<instancename>--Sandbox.<entityid>.my.salesforce.com`
    ...
    i. SFDC biedt geen ondersteuning voor SAML-afmelding.  Als tijdelijke oplossing, plak `https://login.microsoftonline.com/common/wsfederation?wa=wsignout1.0` deze in de **afmeldings-URL van id-Provider** tekstvak.

    j. Klik op **Opslaan**.

### <a name="enable-your-domain"></a>Uw domein inschakelen

In deze sectie wordt ervan uitgegaan dat u al hebt gemaakt een domein.  Zie voor meer informatie, [definiëren van uw domeinnaam](https://help.salesforce.com/HTViewHelpDoc?id=domain_name_define.htm&language=en_US).

**Als u wilt inschakelen in uw domein, moet u de volgende stappen uitvoeren:**
...
1. Klik op het navigatiedeelvenster links in Salesforce, **Bedrijfsinstellingen** de gerelateerde sectie uitvouwen en klik vervolgens op **mijn domein**.
   ...
   >[!NOTE]
   >Zorg ervoor dat uw domein correct is geconfigureerd.

1. In de **verificatieconfiguratie** sectie, klikt u op **bewerken**, naarmate **verificatieservice**, selecteer de naam van de SAML Single Sign-On-instelling in de vorige uit en klik tot slot **opslaan**.

   ![Eenmalige aanmelding configureren](./media/salesforce-sandbox-tutorial/sf-edit-auth-config.png)

Als u een domein geconfigureerd hebt, moeten uw gebruikers de domein-URL, meld u aan bij de Salesforce-sandbox gebruiken.
...
1. Selecteer in de lijst met toepassingen, **Salesforce Sandbox**.
