...
1. Open de PowerShell ISE als beheerder.
...
De Active Directory-omgeving is klaar. We gaan nu een testaccount maken.  Dit account wordt gemaakt in onze on-premises AD-omgeving en vervolgens gesynchroniseerd met Azure AD.  Ga als volgt te werk:
...
Nu gaan we een Azure AD-tenant maken, zodat we onze gebruikers kunnen synchroniseren met de cloud.  Ga als volgt te werk om een nieuwe Azure AD-tenant te maken:
...
5. Geef een **naam op voor de organisatie**, evenals een **oorspronkelijke domeinnaam**. Selecteer vervolgens **Maken**. Er wordt nu een adreslijst gemaakt.
...
De Azure AD-tenant is klaar en we gaan nu een globale beheerdersaccount maken.  Dit account wordt gebruikt voor het maken van het Azure AD-Connector-account tijdens de installatie van Azure AD Connect.  Het Azure AD-Connector-account wordt gebruikt voor het wegschrijven van gegevens naar Azure AD.   Ga als volgt te werk om het globale beheerdersaccount te maken:
...
3.  Geef een naam en gebruikersnaam op voor deze gebruiker. Dit is de globale beheerder voor de tenant. Wijzig de waarde voor **Maprol** in **Globale beheerder.** U kunt ook het tijdelijke wachtwoord weergeven. Als u klaar bent, selecteert u **Maken**.</br>
...
## <a name="verify-users-are-created-and-synchronization-is-occurring"></a>Controleren of gebruikers zijn gemaakt en synchronisatie plaatsvindt
We gaan nu controleren of de gebruikers die aanwezig waren in onze on-premises adreslijst, zijn gesynchroniseerd en nu aanwezig zijn in onze Azure AD-tenant.  Dit synchronisatieproces kan enkele uren duren.  Ga als volgt te werk om te controleren of gebruikers zijn gesynchroniseerd:
...
![Controleren](media/tutorial-password-hash-sync/verify1.png)</br>
