---
title: Toegang krijgen tot Azure SQL met een Windows-VM
description: Een zelfstudie die u helpt bij het doorlopen van het proces voor het krijgen van toegang tot Azure SQL met een Managed Service Identity voor Windows-VM.
...
1.  Een groep in Azure AD maken en de Managed Service Identity van de VM lid van de groep maken.
...
Voeg vervolgens de Managed Service Identity van de VM aan de groep toe.  Hiervoor hebt u de **object-id** van de Managed Service Identity nodig. U kunt deze ophalen met behulp van Azure PowerShell.  Download eerst [Azure PowerShell](https://docs.microsoft.com/powershell/azure/install-azurerm-ps). Meld u vervolgens aan met behulp van `Connect-AzureRmAccount`, en voer de volgende opdrachten uit om het volgende te doen:
...
- De eigenschappen van de Managed Service Identity van de VM ophalen met behulp van de juiste waarden voor `<RESOURCE-GROUP>` en `<VM-NAME>`.
...
De uitvoer ziet er ongeveer als volgt uit, waarbij ook de service-principal-object-id van de Managed Service Identity van de virtuele machine wordt onderzocht:
```powershell
...
Voeg nu de Managed Service Identity van de VM aan de groep toe.  U kunt alleen een service-principal aan een groep toevoegen met behulp van Azure AD PowerShell.  Voer deze opdracht uit:
```powershell
...
Met behulp van code die wordt uitgevoerd op de virtuele machine kunt nu een token verkrijgen van Managed Service Identity en het token gebruiken voor verificatie bij de SQL-server.
...
Azure SQL biedt systeemeigen ondersteuning voor Azure AD-verificatie, zodat toegangstokens die zijn verkregen met behulp van een Managed Service Identity direct kunnen worden geaccepteerd.  U gebruikt de toegangsmethode met het **toegangstoken** voor het maken van een verbinding met SQL.  Dit maakt deel uit van de integratie van Azure SQL met Azure AD en wijkt af van het opgeven van referenties in de verbindingsreeks.

Hier volgt een voorbeeld van .NET-code voor het openen van een verbinding met SQL met behulp van een toegangstoken.  Deze code moet worden uitgevoerd op de virtuele machine om toegang te krijgen tot het eindpunt van de Managed Service Identity van de VM.  **.NET framework 4.6** of hoger is vereist voor het gebruik van de toegangsmethode met een toegangstoken.  Vervang AZURE-SQL-SERVERNAME en DATABASE door de benodigde waarden.  De resource-id voor Azure SQL is https://database.windows.net/.
...
4.  Verzend met `Invoke-WebRequest` vanuit Powershell een aanvraag naar op het lokale Managed Service Identity-eindpunt om een toegangstoken voor Azure SQL op te halen.
...
Bekijk de waarde van `$DataSet.Tables[0]` om de resultaten van de query weer te geven.  Gefeliciteerd. U hebt een query uitgevoerd op de database met behulp van Managed Service Identity van de VM zonder referenties te hoeven opgeven.
