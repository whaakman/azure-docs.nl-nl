...
In deze zelfstudie leert u hoe u hybride Azure AD join voor apparaten dat is gefedereerd met behulp van AD FS configureren.

> [!div class="checklist"]
> * Hybride Azure AD join configureren
> * Windows downlevel-apparaten inschakelen
> * Controleer of de registratie
> * Problemen oplossen
...
Vanaf versie 1.1.819.0, biedt Azure AD Connect u de wizard een hybride Azure AD join configureren. De wizard kunt u het configuratieproces aanzienlijk te vereenvoudigen. De gerelateerde wizard:

- Hiermee configureert u de serviceverbindingspunten (SCP) voor device Registration service

- Back-up van uw bestaande Azure AD-vertrouwensrelatie van relying party

- Updates van de claimregels in uw Azure AD-vertrouwensrelatie

De configuratiestappen in dit artikel zijn gebaseerd op deze wizard. Als u een oudere versie van Azure AD Connect geïnstalleerd hebt, moet u deze bijwerken naar 1.1.819 of hoger. Als het installeren van de meest recente versie van Azure AD Connect kan niet worden gebruikt voor u, raadpleegt u [het handmatig configureren van device Registration service](../device-management-hybrid-azuread-joined-devices-setup.md).
...
4. Op de **verbinding maken met Azure AD** pagina, typ de referenties van een globale beheerder voor uw Azure AD-tenant en klik vervolgens op **volgende**.   
...
6. Op de **SCP** pagina, het uitvoeren van de volgende stappen uit en klik vervolgens op **volgende**: 
...
7. Op de **besturingssystemen van apparaten** pagina, selecteer de besturingssystemen die worden gebruikt door apparaten in uw Active Directory-omgeving en klik vervolgens op **volgende**. 
    ...
    ![Besturingssysteem apparaat](./media/hybrid-azuread-join-federated-domains/17.png)

8. Op de **federatieconfiguratie** pagina, voert u de referenties van uw AD FS-beheerder en klik vervolgens op **volgende**. 

    ![Federatieconfiguratie](./media/hybrid-azuread-join-federated-domains/18.png)

9. Op de **klaar om te configureren** pagina, klikt u op **configureren**. 

    ![Klaar om te configureren](./media/hybrid-azuread-join-federated-domains/19.png)

10. Op de **configuratie voltooid** pagina, klikt u op **afsluiten**. 
...
- Uw organisatie beveiligingstokenservice (STS - federatieve domeinen)
...
- [Oplossen van problemen met Hybrid Azure AD join voor de huidige Windows-apparaten](../device-management-troubleshoot-hybrid-join-windows-current.md)
- [Oplossen van problemen met Hybrid Azure AD join voor Windows downlevel-apparaten](../device-management-troubleshoot-hybrid-join-windows-legacy.md)
...
> [Configure hybride Azure Active Directory join voor beheerde domeinen](hybrid-azuread-join-managed-domains.md)
> [hybride Azure Active Directory join handmatig configureren](../device-management-hybrid-azuread-joined-devices-setup.md)
