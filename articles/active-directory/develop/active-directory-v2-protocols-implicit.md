...
In eerste instantie het tekenen van de gebruiker in uw app, kunt u sturen een [OpenID Connect](active-directory-v2-protocols-oidc.md) autorisatie-aanvraag aan en ontvang een `id_token` van het v2.0-eindpunt:

> [!IMPORTANT]
> Vragen om met succes is een ID-token, dat de app-registratie in de [registratieportal](https://apps.dev.microsoft.com) moet hebben de **[impliciete](active-directory-v2-protocols-implicit.md)** ingeschakeld voor de webclient. Als deze niet is ingeschakeld, een `unsupported_response` fout geretourneerd: "de opgegeven waarde voor de invoerparameter 'response_type' is niet toegestaan voor deze client. Verwachte waarde is "code" "
...
U kunt kiezen om te valideren de `id_token` in client-code, maar een gebruikelijk is voor het verzenden van de `id_token` naar een back-endserver en er de validatie uit te voeren. Nadat u de handtekening van het id_token hebt gevalideerd, zijn er enkele claims u moet om te controleren. Zie de [v2.0 tokenverwijzing](active-directory-v2-tokens.md) voor meer informatie, met inbegrip van [valideren van Tokens](active-directory-v2-tokens.md#validating-tokens) en [belangrijke informatie over de ondertekening van sleutelrollover](active-directory-v2-tokens.md#validating-tokens). Het is raadzaam om gebruik van een bibliotheek voor het parseren en valideren van tokens: Er is ten minste één beschikbaar voor de meeste talen en platforms.
...
| redirect_uri |aanbevolen |De redirect_uri van uw app, waarbij verificatiereacties kunnen worden verzonden en ontvangen door uw app. Het moet een van de redirect_uris die u in de portal hebt geregistreerd, behalve het url-codering moet exact overeenkomen. |
| scope |vereist |Een door spaties gescheiden lijst met bereiken. Voor het ophalen van tokens, omvat alle [scopes](active-directory-v2-scopes.md) u nodig hebt voor de resource van belang zijn. |
...
Zie voor meer informatie over de claims aanwezig zijn in het toegangstoken, de [tokenverwijzing v2.0-eindpunt](active-directory-v2-tokens.md)
