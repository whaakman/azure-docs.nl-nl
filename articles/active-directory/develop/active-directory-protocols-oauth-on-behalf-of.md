...
> [!IMPORTANT]
> Openbare clients die gebruikmaken van de [impliciete OAuth 2.0-grant](active-directory-dev-understanding-oauth2-implicit-grant.md) de OBO-stroom kan niet worden gebruikt. Deze clients hun toegangstoken moeten worden doorgegeven aan een vertrouwelijk client middelste laag om uit te voeren OBO stromen. Zie voor meer informatie over welke clients OBO aanroepen kunnen uitvoeren, [Clientbeperkingen](#client-limitations).
...
Openbare clients met jokertekens antwoord-URL's niet gebruiken een `id_token` voor OBO stromen. Een vertrouwelijk client kan echter nog steeds gebruikmaken van toegangstokens die zijn verkregen via de impliciete grant-stroom, zelfs als de openbare client heeft een jokerteken redirect die URI geregistreerd.
