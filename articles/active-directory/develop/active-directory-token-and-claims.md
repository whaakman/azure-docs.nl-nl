...
Als uw app een API die is beveiligd met Azure AD dat toegangstokens in HTTP-aanvragen is verwacht, moet u uitvoeren validatie en de controle van de tokens die u ontvangt. Validatie van het toegangstoken voordat u deze gebruikt voor toegang tot resources moet worden uitgevoerd in uw app. Zie voor meer informatie over validatie [valideren van Tokens](#validating-tokens). Zie voor meer informatie over hoe u kunt dit doen met .NET [beveiligen van een Web-API met behulp van Azure AD-Bearer-tokens](active-directory-devquickstarts-webapi-dotnet.md).
...
Vernieuwen van tokens zijn volledig ondoorzichtig aan uw app. Ze zijn lange levensduur hebben, maar niet uw app kunt verwachten dat een vernieuwingstoken voor een periode duurt worden geschreven. Vernieuwen van tokens kunnen ongeldig zijn op elk moment in de tijd om een groot aantal redenen - Zie [Token intrekken](#token-revocation) om deze redenen. De enige manier om uw app weten of een vernieuwingstoken geldig is, is als u wilt gebruikmaken van het door het maken van een tokenaanvraag naar Azure AD-token-eindpunt.
