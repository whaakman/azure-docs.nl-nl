---
title: Apparaatsimulatie met IoT-oplossingsversnellers - Azure | Microsoft Docs
description: In deze gebruiksaanwijzing ziet u hoe de apparaatsimulator gebruiken met de oplossingsversneller voor simulatie van apparaat.
author: dominicbetts
manager: timlt
ms.author: dobett
ms.service: iot-accelerators
services: iot-accelerators
ms.date: 09/28/2018
ms.topic: conceptual
ms.openlocfilehash: a2bd69a8097b05fc8a5b09c9009d0e1c1669069c
ms.sourcegitcommit: 7c4fd6fe267f79e760dc9aa8b432caa03d34615d
ms.translationtype: MT
ms.contentlocale: nl-NL
ms.lasthandoff: 09/28/2018
ms.locfileid: "47434718"
---
# <a name="create-and-test-a-new-simulated-device"></a>Maken en testen van een nieuw gesimuleerd apparaat

De oplossingsversnellers Apparaatsimulatie kunt u uw eigen gesimuleerde apparaten definiëren. In dit artikel wordt beschreven hoe u een nieuw gesimuleerd gloeilamp apparaat definiëren en vervolgens lokaal testen. De oplossingsversnellers bevat gesimuleerde apparaten zoals chillers en trucks. U kunt echter uw eigen gesimuleerde apparaten als u wilt testen van uw IoT-oplossingen voordat u implementeert op echte apparaten definiëren.

In deze gebruiksaanwijzing laat zien hoe u het apparaat simulatie microservice aanpassen. Deze microservice is onderdeel van de oplossingsversneller voor Apparaatsimulatie. Als u wilt laten zien hoe het apparaat simuleren-mogelijkheden, wordt in deze gebruiksaanwijzing twee scenario's in de Contoso-IoT-toepassing gebruikt:

[!INCLUDE [iot-solution-accelerators-create-device](../../includes/iot-solution-accelerators-create-device.md)]

## <a name="next-steps"></a>Volgende stappen

Deze handleiding hebt u geleerd u het typen van een aangepaste gesimuleerd apparaat maken en deze testen door het apparaat simulatie microservice lokaal worden uitgevoerd.

De voorgestelde volgende stap is voor informatie over het implementeren van uw aangepaste gesimuleerd apparaat zo in dat de [oplossingsverbetering voor Apparaatsimulatie](iot-accelerators-device-simulation-deploy-simulated-device.md).
