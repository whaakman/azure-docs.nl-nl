...
1. Selecteer de **virtuele Machines** tabblad van de Portal voor publiceren. Voer in het veld, de aanbiedingsnaam van uw. De aanbiedingsnaam van de is doorgaans de naam van het product of service die u van plan bent om te mogen verkopen in de Azure Marketplace.
1. Selecteer **Maken**.
...
1. **Voeg een SKU toe.** De SKU moet een id, die wordt gebruikt in de URL. De id moet uniek zijn binnen uw publicatieprofiel, maar er is geen risico id conflicteert met andere uitgevers.
   ...
   >
   >
1. **Voeg een samenvattende beschrijving toe voor uw SKU.** Samenvatting beschrijvingen zijn zichtbaar voor klanten, zodat u moet deze gemakkelijk leesbaar maken. Deze informatie hoeft niet te worden vergrendeld totdat de fase 'Naar fasering'. Tot dat moment bent u vrij om de informatie te bewerken.
1. Als u op Windows gebaseerde SKU's gebruikt, volgt u de voorgestelde koppelingen voor het verkrijgen van de goedgekeurde versies van Windows Server.
...
1. Selecteer in de pagina basisinstallatiekopie **virtuele Machine maken** om te worden omgeleid naar de nieuwe [Microsoft Azure portal][link-azure-portal].
    ...
    ![tekenen][img-acom-1]
1. Meld u aan bij de portal met de Microsoft-account en het wachtwoord voor de Azure-abonnement die u wilt gebruiken.
1. Volg de aanwijzingen voor het maken van een virtuele machine met behulp van de basisinstallatiekopie die u hebt geselecteerd. U moet een host (naam van de computer), de naam van de gebruiker (geregistreerd als een beheerder) en wachtwoord voor de virtuele machine opgeven.

    ![tekenen][img-portal-vm-create]
1. Selecteer de grootte van de virtuele machine te implementeren:
...
1. Selecteer de **locatie** voor implementatie:
...
1. Klik op **Create**. De virtuele machine wordt gestart om te implementeren. Binnen enkele minuten beschikt u over een geslaagde implementatie en kunt u beginnen met het maken van de installatiekopie voor uw SKU.
...
1. De virtuele machines-blade wordt geopend. Zorg ervoor dat de virtuele machine die u wilt verbinden met wordt uitgevoerd, en selecteert u deze in de lijst met geïmplementeerde VM's.
1. Er wordt een blade geopend waarin wordt beschreven van de geselecteerde VM. Klik aan de bovenkant **Connect**.
1. U wordt gevraagd om in te voeren de gebruikersnaam en wachtwoord die u hebt opgegeven tijdens het inrichten.
...
1. De virtuele machines-blade wordt geopend. Selecteer de virtuele machine die u hebt geïmplementeerd.
1. Er wordt een blade geopend waarin wordt beschreven van de geselecteerde VM.
1. Klik op **Eigenschappen**.
1. Het eerste deel van de domeinnaam is de servicenaam. De hostnaam is de naam van de VM.

    ![tekenen][img-portal-vm-rdp]
1. De cmdlet voor het downloaden van het RDP-bestand voor de gemaakte VM naar het lokale bureaublad van de beheerder is als volgt.
...
1. Ga naar **mijn installatiekopieën**, en selecteer vervolgens de VM-installatiekopie van waaruit u een VM wilt implementeren:
...
1. Open het certificeringshulpprogramma en klik vervolgens op de **nieuwe Test Start** knop.
1. Uit de **testen informatie** scherm, voer een naam voor de test.
1. Bepaal of uw VM op Linux of op Windows draait. Afhankelijk van uw keuze selecteert u de vervolgopties.
...
1. Als u gebruikmaakt van verificatie op basis van wachtwoorden, voert u de Domain Name System (DNS)-naam, gebruikersnaam en wachtwoord.
1. Als u sleutelbestand verificatie gebruikt, voert u de DNS-naam, gebruikersnaam en locatie persoonlijke sleutel.
...
1. Voer de gebruikersnaam en wachtwoord.
...
1. Download [AzureStorageExplorer6Preview3.zip](https://azurestorageexplorer.codeplex.com/downloads/get/891668) en te installeren nadat het.

    ![tekenen](media/marketplace-publishing-vm-image-creation/img5.2_02.png)

1. Nadat deze is geïnstalleerd, opent u de toepassing.
1. Klik op **Account toevoegen**.

    ![tekenen](media/marketplace-publishing-vm-image-creation/img5.2_03.png)

1. Geef de storage-accountnaam, de opslagaccountsleutel en het domein van opslageindpunten. Dit is het opslagaccount in uw Azure-abonnement waar u uw VHD hebt opgeslagen in Azure portal.

    ![tekenen](media/marketplace-publishing-vm-image-creation/img5.2_04.png)

1. Wanneer Azure Storage Explorer is verbonden met uw specifieke storage-account, wordt deze gestart waarin al de binnen het opslagaccount bevat. Selecteer de container waarnaar u het besturingssysteem schijf VHD-bestand (ook gegevensschijven als ze van toepassing zijn voor uw scenario) gekopieerd.

    ![tekenen](media/marketplace-publishing-vm-image-creation/img5.2_05.png)

1. Na het selecteren van de blob-container, Azure Storage Explorer begint met de bestanden in de container. Selecteer het installatiekopiebestand (VHD) die moet worden verzonden.

    ![tekenen](media/marketplace-publishing-vm-image-creation/img5.2_06.png)

1.  Selecteer het VHD-bestand in de container en klik op de **Security** tabblad.

    ![tekenen](media/marketplace-publishing-vm-image-creation/img5.2_07.png)

1.  In de **beveiliging van de Blob-Container** dialoogvenster vak, laat u de standaardinstellingen op de **toegangsniveau** tabblad en klik vervolgens op **Shared Access Signatures** tabblad.

    ![tekenen](media/marketplace-publishing-vm-image-creation/img5.2_08.png)

1.  De volgende stappen voor het genereren van een shared access signature URI voor de VHD-installatiekopie:
...
1.  Om ervoor te zorgen dat de gegenereerde shared access signature-URI werkt, klikt u op **Test in Browser**. Deze moet het downloadproces beginnen.

1.  Kopieer de shared access signature URI. Dit is de URI die u in de Portal voor Publiceren moet plakken.

1.  Herhaal stap 6-10 voor elke VHD in de SKU.
...
1.  Microsoft Azure Storage Explorer aan uw abonnement configureren met aanmelden bij uw account

    ![tekenen](media/marketplace-publishing-vm-image-creation/img5.2_11.png)

1.  Ga naar storage-account en selecteer de Container

1.  Selecteer **'Handtekening voor gedeelde toegang ophalen'..** met behulp van de rechtermuisknop op van de **container**

    ![tekenen](media/marketplace-publishing-vm-image-creation/img5.2_12.png)

1.  Begintijd, verlooptijd en machtigingen, overeenkomstig de volgende bijwerken
...
1.  Shared access signature voor containers URI kopiëren
...
1.  Om ervoor te zorgen dat de gegenereerde shared access signature-URI werkt, moet u deze in de browser testen. Het downloadproces moet worden gestart

1.  Kopieer de shared access signature URI. Dit is de URI die u in de Portal voor Publiceren moet plakken.

1.  Herhaal deze stappen voor elke VHD in de SKU.
...
1.  Zodra de App is gedownload, geïnstalleerd.

1.  Maak een PowerShell-(of andere uitvoerbare bestand van het script) bestand met de volgende code en lokaal opslaan
    ...
    c. **`<Permission Start Date>`**: Als u wilt beveiligen voor UTC-tijd, de dag vóór de huidige datum te selecteren. Bijvoorbeeld, als de huidige datum 25 oktober 2016 is waarde dan 25-10-2016. Als u Azure CLI 2.0 (az-opdracht), geeft u zowel de datum en tijd in de begin- en einddatums, bijvoorbeeld: 10-25-2016T00:00:00Z.
...
1.  Powershell-editor openen met 'Als Administrator uitvoeren'-modus en bestand openen in stap #3. U kunt een scripteditor die beschikbaar is op uw besturingssysteem.

1.  Voer het script uit en biedt u de SAS-URL voor het toegangsniveau van de container
...
1.  Nu krijgt u het niveau van container SAS-URL en moet u de naam van de VHD toevoegen in het.

    Niveau SAS-URL voor container #

    `https://st20151.blob.core.windows.net/vhds?st=2016-10-25T07%3A00%3A00Z&se=2016-11-02T07%3A00%3A00Z&sp=rl&sv=2015-12-11&sr=c&sig=wnEw9RfVKeSmVgqDfsDvC9IHhis4x0fc9Hu%2FW4yvBxk%3D`

1.  De naam van de VHD invoegen na de containernaam van de in het SAS-URL, zoals hieronder wordt weergegeven `https://st20151.blob.core.windows.net/vhds/<VHDName>?st=2016-10-25T07%3A00%3A00Z&se=2016-11-02T07%3A00%3A00Z&sp=rl&sv=2015-12-11&sr=c&sig=wnEw9RfVKeSmVgqDfsDvC9IHhis4x0fc9Hu%2FW4yvBxk%3D`
...
1. Selecteer de **VM-installatiekopieën** tabblad.
1. De id die wordt vermeld op de bovenkant van de pagina is feitelijk de aanbiedings-id en niet de SKU-id.
1. Vul de eigenschappen onder de **SKU's** sectie.
1. Onder **besturingssysteemfamilie**, klikt u op het type van het besturingssysteem die is gekoppeld aan het besturingssysteem-VHD.
1. In de **besturingssysteem** vak, beschrijven van het besturingssysteem. Denk na over een indeling, zoals besturingssysteemfamilie, type, versie en updates. Een voorbeeld is 'Windows Server Datacenter 2014 R2'.
1. Selecteer maximaal zes aanbevolen VM-groottes. Dit zijn aanbevelingen die aan de klant in de blade prijscategorie in de Azure Portal worden weergegeven wanneer deze besluit uw installatiekopie te kopen en implementeren. **Dit zijn alleen aanbevelingen. De klant kan elke VM-grootte die geschikt is voor de vermelde schijven in uw installatiekopie te selecteren.**
1. Voer de versie in. Het versieveld bevat een semantische versie om het product en de updates te identificeren:
...
1. In de **OS VHD URL** voert u de shared access signature URI voor het besturingssysteem VHD gemaakt.
1. Als er gegevensschijven die zijn gekoppeld aan deze SKU, selecteert u de logische eenheid number (LUN) op die u wilt deze gegevensschijf bij de implementatie worden gekoppeld.
1. In de **LUN X VHD URL** voert u de shared access signature URI voor de eerste VHD met gegevens gemaakt.
