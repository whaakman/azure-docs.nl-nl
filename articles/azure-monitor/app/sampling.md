...
De standaard steekproeven-functie kan worden uitgeschakeld tijdens het toevoegen van Application Insights-service wordt in de methode ```ConfigureServices```, met ```ApplicationInsightsServiceOptions``` binnen de `Startup.cs` bestand:
...
* Lees het artikel Developer Network [optimaliseren telemetrie met Application Insights](https://msdn.microsoft.com/magazine/mt808502.aspx).
