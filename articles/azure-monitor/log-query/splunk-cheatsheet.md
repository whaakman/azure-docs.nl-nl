---
title: Splunk met Azure Log Analytics | Microsoft Docs
description: Hulp voor gebruikers die bekend met Splunk zijn leren van de querytaal van Log Analytics.
...
ms.locfileid: "52882738"
---
# <a name="splunk-to-log-analytics"></a>Splunk naar Log Analytics

In dit artikel is bedoeld ter ondersteuning van gebruikers die bekend met Splunk zijn leren van de querytaal van Log Analytics. Directe vergelijking worden gemaakt tussen de twee om te begrijpen van de belangrijkste verschillen en overeenkomsten waar u kunt gebruikmaken van uw bestaande kennis.

## <a name="structure-and-concepts"></a>Structuur en concepten

De volgende tabel vergelijkt de concepten en gegevensstructuren tussen Splunk en Log Analytics.

 | Concept  | Splunk | Log Analytics |  Opmerking
 | --- | --- | --- | ---
 | Implementatie-eenheid  | cluster |  cluster |  Log Analytics kan willekeurige cross-cluster query's. Splunk bestaat niet. |
 ...
 | De metagegevens van gestructureerde gebeurtenis | N/A | tabel |  Splunk beschikt niet over het concept blootgesteld aan de zoektaal van metagegevens van de gebeurtenis. Log Analytics heeft het concept van een tabel, die kolommen bevat. Elk exemplaar van de gebeurtenis is toegewezen aan een rij. |
...
De volgende tabel worden de functies in Log Analytics die gelijk aan Splunk functies zijn.

|Splunk | Log Analytics |Opmerking
|---|---|---
|strcat | strcat()| (1) |
|split  | split() | (1) |
|if     | IFF()   | (1) |
...
De volgende secties bevatten voorbeelden van het gebruik van verschillende operators tussen Splunk en Log Analytics.

> [!NOTE]
> Ten behoeve van het volgende voorbeeld wordt het veld Splunk _regel_ kaarten naar een tabel in Azure Log Analytics en Splunk van standaard tijdstempel is toegewezen aan de Analytics-logboeken _ingestion_time()_ kolom.

### <a name="search"></a>Search
In Splunk, kunt u weglaten de `search` sleutelwoord en geef een tekenreeks op zonder aanhalingstekens. In Azure Log Analytics begint u elke zoekopdracht met `find`, een tekenreeks zonder aanhalingstekens is de kolomnaam van een en de lookup-waarde moet een tekenreeks tussen aanhalingstekens. 
...
Azure Log Analytics-query's starten van een tabellair resultatenset waar het filter. In Splunk is filteren de standaardbewerking op de huidige index. U kunt ook `where` operator in Splunk, maar wordt niet aanbevolen.

| |  | |
|:---|:---|:---|
| Splunk | **Zoeken** | <code>Event.Rule="330009.2" Session.Id="c8894ffd-e684-43c9-9125-42adc25cd3fc" _indextime>-24h</code> |
| Log Analytics | **waar** | <code>Office_Hub_OHubBGTaskError<br>&#124; where Session_Id == "c8894ffd-e684-43c9-9125-42adc25cd3fc" and ingestion_time() > ago(24h)</code> |
| | |


### <a name="getting-n-eventsrows-for-inspection"></a>N gebeurtenissen/rijen ophalen voor controle 
Azure Log Analytics biedt ook ondersteuning voor `take` als een alias voor `limit`. In Splunk, als de resultaten worden gesorteerd, `head` worden de eerste n resultaten geretourneerd. In Azure Log Analytics, limiet niet is besteld, maar retourneert de eerste n rijen die zijn gevonden.

| |  | |
|:---|:---|:---|
| Splunk | **HEAD** | <code>Event.Rule=330009.2<br>&#124; head 100</code> |
| Log Analytics | **Limiet** | <code>Office_Hub_OHubBGTaskError<br>&#124; limit 100</code> |
| | |
...
Voor de onder resultaten in Splunk u `tail`. In Azure Log Analytics kunt u de richting van schrijfvolgorde wordt toegepast met `asc`.

| |  | |
|:---|:---|:---|
| Splunk | **HEAD** |  <code>Event.Rule="330009.2"<br>&#124; sort Event.Sequence<br>&#124; head 20</code> |
| Log Analytics | **Boven** | <code>Office_Hub_OHubBGTaskError<br>&#124; top 20 by Event_Sequence</code> |
| | |
...
Splunk heeft ook een `eval` functie, die niet moet worden vergeleken met de `eval` operator. Zowel de `eval` operator in Splunk en de `extend` operator in Azure Log Analytics alleen ondersteuning voor scalaire functies en rekenkundige operatoren.

| |  | |
|:---|:---|:---|
| Splunk | **Evaluatie** |  <code>Event.Rule=330009.2<br>&#124; eval state= if(Data.Exception = "0", "success", "error")</code> |
| Log Analytics | **Uitbreiden** | <code>Office_Hub_OHubBGTaskError<br>&#124; extend state = iif(Data_Exception == 0,"success" ,"error")</code> |
| | |


### <a name="rename"></a>Naam wijzigen 
Azure Log Analytics maakt gebruik van de operator voor dezelfde naam en een nieuw veld te maken. Splunk bevat twee afzonderlijke operators, `eval` en `rename`.

| |  | |
|:---|:---|:---|
| Splunk | **Wijzig de naam** |  <code>Event.Rule=330009.2<br>&#124; rename Date.Exception as execption</code> |
| Log Analytics | **Uitbreiden** | <code>Office_Hub_OHubBGTaskError<br>&#124; extend exception = Date_Exception</code> |
| | |
...
| Log Analytics | **Project**<br>**Project-opslag** | <code>Office_Hub_OHubBGTaskError<br>&#124; project exception, state</code> |
| | |



### <a name="aggregation"></a>Aggregatie
Zie de [aggregaties in Log Analytics-query's](aggregations.md) voor de verschillende aggregatiefuncties.

| |  | |
|:---|:---|:---|
| Splunk | **Statistieken** |  <code>search (Rule=120502.*)<br>&#124; stats count by OSEnv, Audience</code> |
| Log Analytics | **samenvatten** | <code>Office_Hub_OHubBGTaskError<br>&#124; summarize count() by App_Platform, Release_Audience</code> |
| | |
...
In Splunk, om te sorteren in oplopende volgorde moet u de `reverse` operator. Azure Log Analytics biedt ook ondersteuning voor de opslag van null-waarden, aan het begin of aan het einde te definiëren.
...
Dit is een vergelijkbare operator in Splunk en de Log Analytics.
...
In de Log Analytics-portal, wordt alleen de eerste kolom weergegeven. Alle kolommen zijn beschikbaar via de API.

| |  | |
|:---|:---|:---|
| Splunk | **Velden** |  <code>Event.Rule=330009.2<br>&#124; fields App.Version, App.Platform</code> |
| Log Analytics | **facetten** | <code>Office_Excel_BI_PivotTableCreate<br>&#124; facet by App_Branch, App_Version</code> |
| | |
...
| Log Analytics | **arg_max() samenvatten** | <code>Office_Excel_BI_PivotTableCreate<br>&#124; summarize arg_max(batterylife, *) by device_id</code> |
| | |
