...
Wanneer u een voorwaarde op een koppeling opgeeft, wordt de doelactiviteit alleen uitgevoerd als de voorwaarde wordt omgezet in waar. Doorgaans gebruikt u een variabele $ActivityOutput in een voorwaarde voor het ophalen van de uitvoer van de bronactiviteit.
...
De volgende activiteit, Connect-AzureRmAccount, voegt de geverifieerde Run As-account voor gebruik in het runbook.<br>
![Parameterset verbinding-AzureRmAccount](media/automation-graphical-authoring-intro/authenticate-conn-to-azure-parameter-set.png)<br>
...
Gegevens die zijn gemaakt door elke activiteit die geen een uitgaande koppeling is de [uitvoer van het runbook](http://msdn.microsoft.com/library/azure/dn879148.aspx). De uitvoer met de runbooktaak is opgeslagen en is beschikbaar voor een bovenliggend runbook wanneer het runbook wordt gebruikt als een onderliggend element.
