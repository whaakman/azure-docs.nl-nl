...
Wanneer het ontwerpen van runbooks die betrekking hebben op meerdere abonnementen uw runbook gebruiken moet de [Disable-AzureRmContextAutosave](/powershell/module/azurerm.profile/disable-azurermcontextautosave) cmdlet om ervoor te zorgen dat de verificatiecontext niet is opgehaald vanuit een ander runbook die worden uitgevoerd in de sandbox. U moet vervolgens gebruiken de `-AzureRmContext` parameter op uw `AzureRM` cmdlets en geeft die door de juiste context.
