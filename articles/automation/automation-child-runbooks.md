...
De uitvoer van de onderliggende runbooks worden niet teruggestuurd naar het bovenliggende runbook betrouwbaar vanwege timing. Ook bepaalde variabelen, zoals $VerbosePreference, $WarningPreference, en anderen kunnen niet worden doorgegeven aan de onderliggende runbooks. Om te voorkomen dat deze problemen, kunt u de onderliggende runbooks aanroepen als afzonderlijke Automation taken met behulp van de `Start-AzureRmAutomationRunbook` cmdlet uit met de `-Wait` overschakelen. Het bovenliggende runbook geblokkeerd totdat het onderliggende runbook voltooid is.

Als u niet dat het bovenliggende runbook wilt geblokkeerd op wachten, kunt u het onderliggende runbook met behulp van aanroepen `Start-AzureRmAutomationRunbook` cmdlet zonder de `-Wait` overschakelen. U moet vervolgens gebruiken `Get-AzureRmAutomationJob` moet worden gewacht op Taakvoltooiing van de en `Get-AzureRmAutomationJobOutput` en `Get-AzureRmAutomationJobOutputRecord` voor het ophalen van de resultaten.
