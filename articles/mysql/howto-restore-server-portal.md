...
2. Selecteer uw Azure Database voor MySQL-server. Deze actie opent u de **overzicht** pagina.
...
Azure Database voor MySQL kunt u de server te herstellen naar een punt-in-time en in een nieuwe kopie van de server. U kunt deze nieuwe server gebruiken om uw gegevens te herstellen of hebben uw clienttoepassingen die verwijzen naar deze nieuwe server.
...
2. Op de werkbalk van de server **overzicht** weergeeft, schakelt **herstellen**.
   ...
   ![Azure Database voor MySQL - overzicht - Restore-knop](./media/howto-restore-server-portal/2-server.png)
