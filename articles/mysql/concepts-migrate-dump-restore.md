...
- [Azure Database for MySQL-server - Azure portal maken](quickstart-create-mysql-server-database-using-azure-portal.md)
- [mysqldump](https://dev.mysql.com/doc/refman/5.7/en/mysqldump.html) opdrachtregelprogramma moet zijn geïnstalleerd op een virtuele machine.
...
Gebruik algemene hulpprogramma's en hulpprogramma's, zoals MySQL Workbench, mysqldump, Toad of Navicat op afstand verbinding maken en herstellen van gegevens in Azure Database for MySQL. Gebruik deze hulpprogramma's op de clientcomputer met een internetverbinding verbinding maakt met de Azure Database voor MySQL. Een versleuteld met SSL-verbinding gebruiken voor aanbevolen procedures voor beveiliging, Zie ook [configureren van SSL-verbindingen in Azure Database for MySQL](concepts-ssl-connection-security.md). U hoeft niet de om dumpbestanden te verplaatsen naar een locatie speciale cloud bij het migreren naar Azure Database voor MySQL. 

## <a name="common-uses-for-dump-and-restore"></a>Veelvoorkomende toepassingen voor dumpen en terugzetten
U kunt MySQL-hulpprogramma's zoals mysqldump en mysqlpump met dump en load-databases in een Azure MySQL-Database gebruiken in verschillende algemene scenario's. In andere scenario's, gebruikt u mogelijk de [importeren en exporteren](concepts-migrate-import-export.md) benadering in plaats daarvan.

- Gebruik database dumpen wanneer u de volledige database migreert. Deze aanbeveling heeft bij het verplaatsen van een grote hoeveelheid gegevens voor MySQL, of als u wilt beperken van de serviceonderbreking voor live sites of toepassingen. 
-  Zorg ervoor dat alle tabellen in de database de InnoDB-opslag-engine gebruiken bij het laden van gegevens in Azure Database voor MySQL. Azure Database for MySQL ondersteunt alleen InnoDB-opslag-engine en daarom biedt geen ondersteuning voor alternatieve opslag-engines. Als uw tabellen zijn geconfigureerd met andere opslag-engines, kunt u deze naar de InnoDB-engine-indeling vóór de migratie naar Azure Database for MySQL converteren.
   Bijvoorbeeld, hebt u een WordPress- of Web-App met behulp van de MyISAM-tabellen, converteert u eerst deze tabellen door te migreren naar InnoDB-indeling voor het herstellen met Azure Database voor MySQL. Gebruik de component `ENGINE=InnoDB` om in te stellen de engine gebruikt bij het maken van een nieuwe tabel, klikt u vervolgens de gegevens overdragen naar de compatibele tabel vóór het herstellen. 

   ```sql
   INSERT INTO innodb_table SELECT * FROM myisam_table ORDER BY primary_key_columns
   ```
- Om te voorkomen dat eventuele compatibiliteitsproblemen, zorg ervoor dat dezelfde versie van MySQL op de bron- en -systemen wordt gebruikt tijdens het dumpen van databases. Bijvoorbeeld, als uw bestaande MySQL-server versie 5.7 is, moet klikt u vervolgens u migreren naar Azure Database voor MySQL is geconfigureerd voor het uitvoeren van versie 5.7. De `mysql_upgrade` opdracht werkt niet in een Azure Database for MySQL-server en wordt niet ondersteund. Als u upgraden naar versies van MySQL wilt, eerst dump of de lagere versie-database exporteren naar een hogere versie van MySQL in uw eigen omgeving. Voer `mysql_upgrade`, voordat u probeert te migreren naar een Azure-Database voor MySQL.
...
## <a name="create-a-backup-file-from-the-command-line-using-mysqldump"></a>Maak een back-upbestand vanaf de opdrachtregel met behulp van mysqldump
Als u wilt back-up van een bestaande MySQL-database op de lokale on-premises server of in een virtuele machine, moet u de volgende opdracht uitvoeren: 
```bash
...
## <a name="create-a-database-on-the-target-azure-database-for-mysql-server"></a>Maak een database in de doel-Azure Database voor MySQL-server
Een lege database maken op de doel-Azure Database voor MySQL-server waar u om de gegevens te migreren. Gebruik een hulpprogramma zoals MySQL Workbench, Toad of Navicat om de database te maken. De database kan dezelfde naam hebben als de database die is opgenomen in de gedumpte gegevens of u een database met een andere naam maken kunt.

Als u wilt verbinding maken, zoekt u de verbindingsgegevens in de **overzicht** van uw Azure Database for MySQL.

![Informatie over de verbinding in Azure portal](./media/concepts-migrate-dump-restore/1_server-overview-name-login.png)
...
## <a name="restore-your-mysql-database-using-command-line-or-mysql-workbench"></a>Uw terugzetten met behulp van opdrachtregel MySQL-database of MySQL Workbench
...
```
In dit voorbeeld de gegevens in de zojuist gemaakte database op de doel-Azure Database voor MySQL-server te herstellen.
```bash
$ mysql -h mydemoserver.mysql.database.azure.com -u myadmin@mydemoserver -p testdb < testdb_backup.sql
```

## <a name="export-using-phpmyadmin"></a>Exporteren met behulp van PHPMyAdmin
Als u wilt exporteren, kunt u de algemene hulpprogramma phpMyAdmin, die u mogelijk al hebt geïnstalleerd lokaal in uw omgeving. Voor het exporteren van uw MySQL-database met behulp van PHPMyAdmin:
1. Open phpMyAdmin.
...
1. Open phpMyAdmin. 
2. Klik in de installatiepagina phpMyAdmin **toevoegen** om toe te voegen van uw Azure Database voor MySQL-server. Geef de verbindingsgegevens en aanmeldingsgegevens.
