---
title: Bestanden uploaden naar een Media Services-account met behulp van REST | Microsoft Docs
...
* Een bestand te uploaden naar blob storage

AMS kunt u activa in bulk uploaden. Zie [deze](media-services-rest-upload-files.md#upload_in_bulk) sectie voor meer informatie.
...
Als geslaagd, het volgende is geretourneerd: HTTP/1.1 204 geen inhoud
...
De IngestManifest is een container voor een set van assets, assetbestanden en statistische gegevens die kan worden gebruikt om te bepalen van de voortgang van grote hoeveelheden opnemen voor de set.
...
Voordat u de IngestManifestAsset maakt, moet u de Asset die wordt voltooid met het opnemen van grote hoeveelheden maken. Een actief is een container voor meerdere typen of sets van objecten in Media Services, waaronder video, audio, afbeeldingen, verzamelingen miniaturen, tekst-nummers en ondertitelingsbestanden bestanden. In de REST-API moet maken van een Asset een HTTP POST-aanvraag verzenden naar Microsoft Azure Media Services en eigenschapsinformatie over uw asset te plaatsen in de aanvraagtekst. In dit voorbeeld wordt de Asset gemaakt met de optie StorageEncrption(1) is opgenomen in de aanvraagtekst.
...
IngestManifestAssets vertegenwoordigen activa in een IngestManifest die worden gebruikt met het opnemen van grote hoeveelheden. De in feite de activum koppelen aan het manifest. Azure Media Services bestandswijzigingsmeldingscontrole intern op basis van IngestManifestFiles verzameling die is gekoppeld aan de IngestManifestAsset het bestand te uploaden. Wanneer deze bestanden zijn geüpload, wordt de asset is voltooid. U kunt een nieuwe IngestManifestAsset maken met een HTTP POST-aanvraag. In de aanvraagtekst bevatten de IngestManifest-Id en de Id van de activa die de IngestManifestAsset samen moet worden gekoppeld voor het opnemen van grote hoeveelheden.
...
### <a name="link-the-contentkey-to-the-asset"></a>De ContentKey koppelen aan de Asset
De ContentKey is gekoppeld aan een of meer activa door een HTTP POST-aanvraag te verzenden. De volgende aanvraag is een voorbeeld van het voorbeeld ContentKey koppelen aan de asset voorbeeld op-id.
