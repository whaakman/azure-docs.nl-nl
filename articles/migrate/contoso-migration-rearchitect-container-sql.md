...
In dit artikel ziet u hoe Contoso migreert en hun Apps SmartHotel in Azure opnieuw modelleren. Zij migreren de app front-end virtuele machine naar een Windows Azure-container en de app-database naar een Azure SQL-database.
...
[Artikel 9: Een app met een Azure-Web-App en Azure SQL-database herstructureren](contoso-migration-refactor-web-app-sql.md) | Laat zien hoe Contoso de app SmartHotel migreert naar een Azure-Web-App en de app-database migreert naar Azure SQL Server-exemplaar | Beschikbaar
...
[Artikel 11: Herstructureren TFS op VSTS](contoso-migration-tfs-vsts.md) | Ziet u hoe Contoso hun Team Foundation Server (TFS) on-premises implementatie door te migreren migreert het naar Visual Studio Team Services (VSTS) in Azure.
Artikel 12: Opnieuw ontwerpen van een app op Azure-containers en Azure SQL Database | Ziet u hoe Contoso migreert en rearchitects hun SmartHotel app naar Azure. Ze opnieuw ontwerpen voor de laag van de web-app als een Windows-container en de app-database in een Azure SQL Database. | In dit artikel
...
--- | --- 
**App-vereisten** | De app in Azure blijft als kritiek omdat het is vandaag.<br/><br/> Deze moet dezelfde prestatiemogelijkheden hebben als op dit moment in VMWare.<br/><br/> Ze willen stoppen met het ondersteunen van Windows Server 2008 R2, waarop de app op dit moment wordt uitgevoerd, en bereid bent te investeren in de app.<br/><br/> Ze willen afstappen van SQL Server 2008 R2 naar een moderne PaaS-Database-platform, de noodzaak voor het beheer te minimaliseren.<br/><br/> Contoso wilt gebruikmaken van hun investering in SQL Server-licenties en Software Assurance waar mogelijk.<br/><br/> Ze willen kunnen de weblaag app kan worden uitgebreid.
...
**Azure-vereisten** | Ze willen verplaatsen van de app naar Azure en de App uitvoeren in een container voor het uitbreiden van de levensduur van de app. Ze wil begin volledig opnieuw voor het implementeren van de app in Azure. 
...
- Voor de laag van de web-app ze hebben besloten converteren naar de Windows-Container met behulp van Visual Studio.
    - Ze de app met behulp van Azure Service Fabric implementeren, en de installatiekopie van de Windows-container ophalen uit de Azure Container Registry (ACR).
...
**Overweging** | **Details**
--- | ---
**Professionals** | De SmartHotel app-code moet worden gewijzigd voor migratie naar Azure Service Fabric. De inspanningen is echter slechts weinig, met behulp van de Service Fabric-SDK-hulpprogramma's voor de wijzigingen.<br/><br/> Ze kunnen starten met de overgang naar Service Fabric, voor het ontwikkelen van microservices om toe te voegen aan de toepassing snel gedurende een periode, zonder risico voor de oorspronkelijke codebasis.<br/><br/> Windows-Containers bieden dezelfde voordelen als containers in het algemeen. Ze verbeteren de flexibiliteit, draagbaarheid en controle.<br/><br/> Ze kunnen gebruikmaken van hun investering in Software Assurance met behulp van Azure Hybrid Benefit voor SQL Server en Windows Server.<br/><br/> Na de migratie moet ze niet langer ondersteuning voor Windows Server 2008 R2. [Meer informatie](https://support.microsoft.com/lifecycle).<br/><br/> Ze kunnen de weblaag van de app configureren met meerdere instanties, zodat deze niet langer een single point of failure.<br/><br/> Deze wordt niet langer afhankelijk van de ouderdom van SQL Server 2008 R2.<br/><br/> SQL Database biedt ondersteuning voor de technische vereisten van Contoso. Ze de on-premises-database met behulp van de Database Migration Assistant bekeken en wordt gevonden of deze compatibel is.<br/><br/> SQL-Database heeft ingebouwde fouttolerantie die Contoso niet nodig hebt om in te stellen. Dit zorgt ervoor dat de gegevenslaag niet langer een single point of failover is.
**Nadelen** | Containers zijn complexer dan andere opties voor de migratie. Het leerproces voor containers is mogelijk een probleem voor Contoso.  Ze introduceren een nieuw niveau van complexiteit waarmee veel van de waarde er de curve.<br/><br/> Het operationele team bij Contoso moet aan de slag om te begrijpen en ondersteuning voor Azure, containers en microservices voor de app.<br/><br/> Als ze de Data Migration Assistant in plaats van de Data Migration Service gebruiken voor het migreren van de database, Contoso geen de infrastructuur die gereed zijn voor het migreren van databases op schaal.



### <a name="migration-process"></a>Migratieproces

1. Contoso inrichten van de Azure service fabric-cluster voor Windows.
2. Deze inrichten van een Azure SQL-exemplaar en migreren van de database SmartHotel toe.
3. Ze converteren de weblaag VM naar een Docker-container met behulp van de Service Fabric-SDK-hulpprogramma's.
4. Ze verbinding maken met de service fabric-cluster en de ACR en implementeer de app met behulp van Azure service fabric.
...
[Azure Container Registry](https://azure.microsoft.com/services/container-registry/) | Store-installatiekopieën voor alle typen containerimplementaties. | De kosten op basis van functies, opslag en duur van gebruik. [Meer informatie](https://azure.microsoft.com/pricing/details/container-registry/).
[Azure Service Fabric](https://azure.microsoft.com/services/service-fabric/) | Ontwikkelen en uitvoeren, altijd ingeschakelde, schaalbare en gedistribueerde apps | De kosten op basis van grootte, de locatie en de duur van de rekenknooppunten. [Meer informatie](https://azure.microsoft.com/pricing/details/service-fabric/).
...
> * **Stap 2: Inrichten Azure Service Fabric**: het inrichten van een Service Fabric-Cluster.
> * **Stap 4: Migreren van de database met DMA**: zij de app-database met de Database Migration Assistant migreren.
> * **Stap 5: De app converteren naar een container**: ze de app converteren naar een container met behulp van Visual Studio en SDK-hulpprogramma's.
> * **Stap 6: Publiceer de app**: ze publiceren naar de app naar de ACR en Service Fabric-Cluster.
> * **Stap 7: De app uit te breiden**: nadat de app openbaar is, ze om te profiteren van de mogelijkheden van Azure, en deze opnieuw te publiceren naar Azure uitbreiden.
...
1. Contoso maakt u een Containerregister in Azure portal.
...
De container SmartHotel wordt uitgevoerd in de Azure Service Fabric-Sluster. Contoso maakt als volgt u de Service Fabric-Cluster:
...
5. In **toegangsbeleid** ze inschakelen toegang tot virtuele machines te implementeren van de key vault.
...
## <a name="step-3-migrate-the-database-with-dma"></a>Stap 3: De database met DMA migreren
...
Voor verbinding met de Azure SQL Database, is een firewallregel vereist.
...
## <a name="step-4-convert-the-app-to-a-container"></a>Stap 4: De app converteren naar een container
...
Contoso om de app converteren naar een container met behulp van isual Studio en de SDK-hulpprogramma's als volgt:

1. Ze kloon de opslagplaats lokaal naar een developer-machine:

    **GIT-kloon https://github.com/Microsoft/SmartHotel360-internal-booking-apps.git**

    ![Container](./media/contoso-migration-rearchitect-container-sql/container1.png)

2. Met behulp van Visual Studio, zij het oplossingsbestand (SmartHotel.Registration.sln) openen de **SmartHotel360-interne-na-apps\src\Registration** map van de lokale opslagplaats.  Twee apps worden weergegeven. De webfrontend SmartHotel.Registration.Web nad de WCF-service-app SmartHotel.Registration.WCF.

    ![Container](./media/contoso-migration-rearchitect-container-sql/container2.png)


3. Ze met de rechtermuisknop op de web-app > **toevoegen** > **Orchestrator Containerondersteuning**.
4. In **Containerondersteuning orkest toevoegen**, selecteren ze **Service Fabric**.

    ![Container](./media/contoso-migration-rearchitect-container-sql/container3.png)

5. Het proces wordt herhaald voor SmartHotel.Registration.WCF app van Contoso.
6. Contoso controleert nu hoe de oplossing is gewijzigd.
...
7. Visual Studio het Docker-bestand gemaakt en opgevraagd de vereiste afbeeldingen lokaal op de machine voor ontwikkelaars.

    ![Container](./media/contoso-migration-rearchitect-container-sql/container5.png)

8. Een manifestbestand (**ServiceManifest.xml**) wordt gemaakt en geopend in Visual Studio. Dit bestand legt uit hoe het configureren van de container als deze geïmplementeerd in Azure Service Fabric.

    ![Container](./media/contoso-migration-rearchitect-container-sql/container6.png)

9. Een andere manifestbestand (** ApplicationManifest.xml) bevat de configuratie van toepassingen voor de containers.

    ![Container](./media/contoso-migration-rearchitect-container-sql/container7.png)

## <a name="step-5-publish-the-app"></a>Stap 5: De app publiceren


Contoso kan de app ten slotte publiceren naar de ACR en Service Fabric-Cluster.

> [!NOTE]
> Sommige wijzigingen met betrekking tot de Service Fabric-cluster zijn aangebracht in de app SmartHotel. U kunt zowel de oorspronkelijke en gemoderniseerd app-code uit downloaden [GitHub](https://github.com/Microsoft/SmartHotel360-internal-booking-apps). Het gewijzigde bestand **AppliationModern/ApplicationParameters/Cloud.xml**.


1. In Visual Studio, de update voor de verbindingsreeks voor de app verbinden met de Azure SQL Database. De verbindingsreeks kan worden gevonden in de database in Azure portal.

    ![Publiceren](./media/contoso-migration-rearchitect-container-sql/publish1.png)

2. Contoso publiceert de app naar Service Fabric met Visual Studio. Ze met de rechtermuisknop op de Service Fabric-toepassing > **publiceren**.

    ![Publiceren](./media/contoso-migration-rearchitect-container-sql/publish2.png)

3. Ze selecteert u het abonnement, het eindpunt voor de verbinding en de ACR. Klik vervolgens op **publiceren**.

    ![Publiceren](./media/contoso-migration-rearchitect-container-sql/publish3.png)

4. Nadat de implementatie is voltooid, wordt de Service Fabric van SmartHotel wordt nu uitgevoerd.

    ![Publiceren](./media/contoso-migration-rearchitect-container-sql/publish4.png)

5. Voor verbinding met de app, stuurt Contoso verkeer naar het openbare IP-adres van de Azure load balancer in de voorgrond van hun Service Fabric-knooppunten.

    ![Publiceren](./media/contoso-migration-rearchitect-container-sql/publish5.png)

## <a name="step-6-extend-the-app-and-republish"></a>Stap 6: De app uit te breiden en opnieuw publiceren

Wanneer de SmartHotel app en database worden uitgevoerd in Azure, wil Contoso de app uit te breiden.
...
Als een eerste stap inrichten Contoso een Azure Cosmos-database.
...
Met de Cosmos DB is ingericht, kunt Contoso verbinding maken met de app configureren.
...
Nadat de app is uitgebreid, Contoso rapport opnieuw publiceert deze naar Azure.

1. In de portal, ze met de rechtermuisknop op de Service Fabric-app > **publiceren**.

    ![Opnieuw publiceren](./media/contoso-migration-rearchitect-container-sql/republish1.png)

2. Ze selecteert u het abonnement, het eindpunt voor de verbinding en de ACR. Klik vervolgens op **publiceren**.

    ![Opnieuw publiceren](./media/contoso-migration-rearchitect-container-sql/republish2.png)

4. Nadat de implementatie is voltooid, wordt de Service Fabric van SmartHotel wordt nu uitgevoerd. Drie services worden nu weergegeven in de console Beheer van uw Fabric gevolgd.

    ![Opnieuw publiceren](./media/contoso-migration-rearchitect-container-sql/republish3.png)

5. Contoso kunt klikken op via de services te gaan of de app SentimentIntegration actief en werkend is
...
- Interne documentatie om weer te geven van de nieuwe locaties voor de app SmartHotel bijwerken. De database die wordt uitgevoerd in Azure SQL-database en de front-end dat deze wordt uitgevoerd in Service Fabric weergeven.
...
- Contoso moet rekening houden met de Web-App in de belangrijkste VS-Oost 2 en de regio VS-midden implementeren als u Web App for Containers beschikbaar. Ze kunnen Traffic Manager om te controleren of failover in het geval van regionale storingen configureren.
...
In dit artikel is geherstructureerd Contoso de SmartHotel-app in Azure door het app-front-end virtuele machine migreren naar Service Fabric. Ze de app-database hebt gemigreerd naar een Azure SQL database.
