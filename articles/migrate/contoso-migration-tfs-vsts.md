...
In dit artikel leest u hoe Contoso hun Team Foundation Server (TFS) on-premises implementatie door te migreren zijn herstructurering deze naar Visual Studio Team Services (VSTS) in Azure. Het Contoso-ontwikkelteam TFS hebt gebruikt voor teamsamenwerking en bronbeheer voor de afgelopen vijf jaar. Ze willen nu verplaatsen naar een cloud-gebaseerde oplossing voor ontwikkelen en testen, en voor broncodebeheer. VSTS wordt een rol spelen als ze naar een DevOps-model verplaatst en nieuwe cloud-eigen Apps.

Dit document is het elfde in een reeks artikelen die laten zien hoe het fictieve bedrijf Contoso de on-premises bronnen naar de Microsoft Azure-cloud migreert. De serie bevat achtergrondinformatie en scenario's die laten zien hoe u een migratie-infrastructuur instellen en uitvoeren van verschillende typen migraties. Scenario's toeneemt in complexiteit en aanvullende artikelen na verloop van tijd toegevoegd.
...
Het team voor IT-leidinggevenden heeft nauw samengewerkt met hun zakelijke doelstellingen te identificeren. Zakelijke partners niet overmatig betrokken zijn bij de ontwikkelprogramma's en technologieën, maar ze deze punten zijn opgenomen:
...
- **Efficiëntie**: Contoso moet te stroomlijnen en te verwijderen van onnodige procedures voor ontwikkelaars en gebruikers. Hierdoor kunnen ze voor de levering op klantvereisten efficiënter. De bedrijfsbehoeften IT snel, zonder verspild tijd of geld.
...
- Ze moeten een hulpprogramma voor het migreren van de gegevens naar de cloud. Moeten worden enkele handmatige processen die nodig zijn.
...
- Ze zich zorgen maakt over de wijziging en wilt testen voordat u een volledige verplaatsen. Ze willen toegang tot TFS behouden, zelfs na de overstap naar VSTS.
...
- Ze begrijpen dat TFS-verzamelingen een-op-een-relatie met VSTS-accounts kunt zijn, zodat ze meerdere URL's hebben. Maar dit komt overeen met het huidige model van scheiding van voor code-databases en projecten.
...
1. Er is een groot aantal voorbereiding die betrokken zijn. Upgraden van de TFS-implementatie naar een ondersteunde moet als eerste stap, Contoso. Ze worden momenteel uitgevoerd met TFS 2017 Update 3, maar voor het gebruik van de migratie van de database die ze nodig hebt om uit te voeren van een ondersteunde versie van 2018 met de nieuwste updates.
2. Na de upgrade moeten ze het TFS-hulpprogramma voor migratie worden uitgevoerd, en valideren van de verzameling.
3. Ze een set voorbereiding bestand bouwen en uitvoeren van een migratie droge uitvoeren voor het testen.
4. Ze kunnen vervolgens een andere migratie worden uitgevoerd, deze tijd een volledige migratie met werkitems, fouten, sprints en code.
5. Na de migratie, wordt ze hun code uit TFVC verplaatsen naar Git.
...
> * **Stap 2: Upgrade TFS**: ze hun implementatie gaat upgraden naar TFS 2018 Upgrade 2. 
> * **Stap 3: Valideren verzameling**: ze hun TFS-verzameling in voorbereiding voor de migratie kunnen valideren.
> * **Stap 4: Voorbereiding buildbestand**: maakt de migratiebestanden met behulp van het TFS-hulpprogramma voor migratie. 


## <a name="step-1-create-a-storage-account"></a>Stap 1: Een storage-account maken

1. In de Azure-portal, Contoso Hiermee maakt u een opslagaccount (**contosodevmigration**).
...
Contoso upgrade van de TFS-server naar TFS 2018 Update 2. Voordat ze starten:
...
5. Contoso controleert of de TFS-installatie aan de hand van de projecten, werkitems en code.
...
Contoso voert het TFS-hulpprogramma voor migratie op de database van de verzameling ContosoDev te valideren voordat de migratie.

1. Contoso gedownload en wordt uitgepakt de [hulpprogramma voor migratie van TFS](https://www.microsoft.com/download/details.aspx?id=54274). Het is belangrijk om te downloaden van de versie van de TFS-update die wordt uitgevoerd. De versie kan worden weergegeven in de beheerconsole.
...
4. Contoso vindt deze vermelding, met betrekking tot identiteit.

    ![TFS](./media/contoso-migration-tfs-vsts/collection4.png)

5. Contoso wordt uitgevoerd **TfsMigration valideren/Help** op de opdrachtregel en ziet dat de opdracht **/tenantDomainName** lijkt te zijn vereist om te controleren van identiteiten.
...
Met de validatie is voltooid, Contoso het TFS-hulpprogramma voor migratie gebruiken om te maken van bestanden voor de migratie.
...
2. Een Azure AD-aanmelding In het scherm wordt weergegeven en Contoso voert de referenties van een globale beheerder.
...
4. Contoso kunt nu zien dat zowel de IdentityMapLog.csv als het bestand import.json zijn gemaakt in een nieuwe map.

    ![Voorbereiden](./media/contoso-migration-tfs-vsts/prep4.png)

5. Het bestand import.json bevat instellingen voor importeren. Het bevat informatie zoals de naam van het gewenste en gegevens over het opslagaccount. De meeste van de velden worden automatisch ingevuld. Sommige velden vereist invoer van de gebruiker. Contoso-bestand wordt geopend en wordt de naam van de VSTS-account moet worden gemaakt: **contosodevmigration**. Met deze naam, de URL van hun VSTS worden **contosodevmigration.visualstudio.com**.
...
Met de voorbereiding op locatie kunt Contoso nu zich richten op de migratie. Nadat de migratie is uitgevoerd, schakel ze TFVC bent met Git voor versiebeheer te gebruiken.

Voordat ze starten, plant u Contoso downtime met het ontwikkelingsteam, de verzameling offline voor migratie uitvoeren. Dit zijn de stappen voor het migratieproces:
...
Voordat u begint gebruikt Contoso een lokale SQL Server-back-up en VMware-momentopname van de TFS-server, voordat u loskoppelen.
...
Contoso maakt een back-up (DACPAC) voor het importeren in VSTS.
...
Contoso genereert de DACPAC als volgt:
...
Eerder, Contoso gedeeltelijk ingevuld het importbestand-specificatie (import.json). Nu moeten de resterende instellingen toe te voegen.
...
Contoso beginnen met een migratie testmodus, om te controleren of alles werkt zoals verwacht.
...
11. Na 15 minuten, Contoso bladert naar de URL en ziet de volgende informatie:

     ![Testmodus](./media/contoso-migration-tfs-vsts/test7.png)

12. Nadat de migratie is voltooid een zich Contoso Dev Leads in VSTS om te controleren of opgeleverd het droge uitvoeren. Na verificatie moet VSTS enkele details voor het bevestigen van het account.
...
14. De Dev Lead openen op een van de projecten en opent u **werkitems** > **aan mij toegewezen**. Hiermee wordt aangegeven dat item werkgegevens is gemigreerd, samen met zijn identiteit.

    ![Testmodus](./media/contoso-migration-tfs-vsts/test10.png)

15. Hij controleert ook of andere projecten en code, om te bevestigen dat de broncode en de geschiedenis is gemigreerd.
...
Met de volledige uitvoering droge Contoso verder met de productie-migratie. Ze het droge uitvoeren verwijderen, bijwerken van de instellingen voor importeren en importeren opnieuw uitvoeren.
...
5. In Azure AD-aanmelding, Contoso Hiermee geeft u een aanmelding Contoso Admin.
...
7. Na ongeveer 15 minuten, Contoso bladert naar de URL en ziet de volgende informatie:

    ![Productie](./media/contoso-migration-tfs-vsts/full5.png)

8. Nadat de migratie is voltooid voor een logboeken Contoso Dev leidt naar VSTS om te controleren of opgeleverd de migratie. Na aanmelding ziet hij kan projecten zijn gemigreerd.

    ![Productie](./media/contoso-migration-tfs-vsts/full6.png)

8. De Dev Lead openen op een van de projecten en opent u **werkitems** > **aan mij toegewezen**. Hiermee wordt aangegeven dat item werkgegevens is gemigreerd, samen met zijn identiteit.

    ![Productie](./media/contoso-migration-tfs-vsts/full7.png)

9. Hij controleert of andere werkgegevens item om te bevestigen.
...
Met de migratie is voltooid wil Contoso verplaatsen van TFVC naar Git voor broncodebeheer. Ze moeten de broncode op dit moment in hun VSTS-account importeren als Git-opslagplaatsen in hetzelfde account.
...
3. In **gegevensbrontype** selecteren ze **TFVC**, en geef het pad naar de opslagplaats. Ze hebben besloten om niet te migreren van de geschiedenis.
    ...
    > [!NOTE]
    > Vanwege de verschillen in hoe informatie over het beheer van versie TFVC en Git opgeslagen, wordt aangeraden om niet te migreren van de geschiedenis. Dit is de benadering die Microsoft heeft genomen wanneer deze Windows en andere producten gemigreerd vanuit gecentraliseerd versiebeheer naar Git.

4. Contoso beoordelingen na het importeren van de code.
