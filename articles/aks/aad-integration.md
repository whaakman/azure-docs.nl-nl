...
Gebruik vervolgens de volgende manifest te maken van een ClusterRoleBinding voor een Azure AD-account. In dit voorbeeld geeft de account volledige toegang tot alle naamruimten van het cluster. 

Krijgen de *objectId* account met behulp van de gebruiker de [az ad-gebruiker weergeven] [ az-ad-user-show] opdracht. Geef de user principal name (UPN) van het vereiste account:

```azurecli-interactive
az ad user show --upn-or-object-id user@contoso.com --query objectId -o tsv
```

Maak een bestand, zoals *rbac-aad-user.yaml*, en plak de volgende inhoud. Werk de gebruikersnaam met de object-ID van uw gebruikersaccount van Azure AD in de vorige stap verkregen:
