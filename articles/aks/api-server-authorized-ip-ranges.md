...
Als u wilt configureren API server gemachtigd IP-bereiken, moet u de *aks-preview* CLI versie van de extensie 0.4.1 of hoger. Installeer de *aks-preview* Azure CLI-extensie met de [az-extensie toevoegen][az-extension-add] command, then check for any available updates using the [az extension update][az-extension-update] opdracht:
...
Voor het maken van een Azure-Firewall, installeert de *azure-firewall* CLI-extensie met de [az-extensie toevoegen][az-extension-add] command. Then, create a firewall using the [az network firewall create][az-network-firewall-create] opdracht. Het volgende voorbeeld wordt een firewall van Azure met de naam *myAzureFirewall*:
...
Een firewall van Azure is een openbaar IP-adres die uitgaand verkeer via toegewezen. Maak een openbaar adres met de [az network public-ip maken][az-network-public-ip-create] command, then create an IP configuration on the firewall using the [az network firewall ip-config create][az-network-firewall-ip-config-create] die van toepassing is het openbare IP-adres:
...
Maak nu de Azure-netwerk firewallregel *toestaan* alle *TCP* verkeer met behulp van de [az netwerk netwerk-firewallregel maken][az-network-firewall-network-rule-create] opdracht. Het volgende voorbeeld wordt een regel met de naam *AllowTCPOutbound* voor verkeer met een bron- of -adres:
...
Maak ten slotte een route in de bestaande AKS netwerk route tabel met de [az network route-table route maken][az-network-route-table-route-create] opdracht waarmee verkeer op het apparaat van de firewall van Azure gebruiken voor communicatie tussen de API-server.
...
Zie voor meer informatie, [veiligheidsconcepten voor toepassingen en -clusters in AKS][concepts-security] and [Best practices for cluster security and upgrades in AKS][operator-best-practices-cluster-security].
