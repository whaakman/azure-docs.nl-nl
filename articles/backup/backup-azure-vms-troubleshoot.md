...
| De bewerking voor recovery services-extensie is mislukt: <br>Zorg ervoor dat de nieuwste VM-Agent aanwezig is op de virtuele machine en de VM-Agent-service wordt uitgevoerd. Probeer de back-upbewerking. Als de back-upbewerking is mislukt, moet u contact op met Microsoft Support. |Deze fout treedt op wanneer de VM-Agent verouderd is. Raadpleeg [los problemen met Azure virtuele machine back-up](#updating-the-VM-agent) om bij te werken van de VM-Agent. |
...
| Back-up is mislukt voor het installeren van de Azure Recovery Services-extensie op het geselecteerde item: <br>De VM-Agent is een vereiste voor de Azure Recovery Services-extensie. De Azure VM-Agent installeren en opnieuw starten van de bewerking voor de registratie. |<ol> <li>Controleer of de VM-Agent correct is geïnstalleerd. <li>Zorg ervoor dat de vlag aan de configuratie van de virtuele machine correct is ingesteld.</ol> Meer informatie over [installeren van de VM-Agent](#validating-vm-agent-installation) en hoe u de installatie van de VM-Agent te valideren. |
...
Als uw back-up meer dan 12 uur wordt of terugzetten meer dan 6 uur heeft:
* Inzicht in [factoren die aan de back-uptijd bijdragen](backup-azure-vms-introduction.md#time-considerations) en [factoren die bijdragen herstellen van tijd](backup-azure-vms-introduction.md#restore-considerations).
