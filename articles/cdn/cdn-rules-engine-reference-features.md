...
Het derde deel van een regel is de functie. Een functie bepaalt het type actie die wordt toegepast op het type aangeduid met een set voorwaarden overeen met aanvraag.
...
Token Auth | Hiermee wordt bepaald of verificatie op basis van tokens worden toegepast op een aanvraag.
Token Auth DOS-Code | Bepaalt het type van de reactie die wordt geretourneerd voor een gebruiker wanneer een aanvraag wordt geweigerd vanwege verificatie op basis van tokens.
...
Token Auth-Parameter | Hiermee wordt bepaald of de verificatie op basis van tokens querytekenreeksparameter moet worden gewijzigd.
...
Naam | Doel
-----|--------
Bandbreedte-Parameters | Hiermee wordt bepaald of parameters (bijvoorbeeld ec_rate en ec_prebuf) voor bandbreedteregeling actief zijn.
Bandbreedtebeperking | Beperkt de bandbreedte voor de respons van onze randservers.
De Bypass-Cache | Hiermee wordt bepaald of de aanvraag van onze caching-technologie gebruikmaken kunt.
Cache-Control Header behandeling | Hiermee bepaalt u het genereren van Cache-Control headers door de edge-server als onderdeel van de externe maximumleeftijd actief is.
Cache-sleutel queryreeks | Hiermee wordt bepaald of de cache-sleutel wordt opnemen of uitsluiten van queryreeksparameters die zijn gekoppeld aan een aanvraag.
Cache-sleutel opnieuw schrijven | Herschrijft de cache-sleutel die is gekoppeld aan een aanvraag.
...
Bestandstypen comprimeren | Hiermee definieert u de bestandsindelingen die zal worden gecomprimeerd op de server.
Standaard interne-maximumleeftijd | Het standaardinterval voor de maximale leeftijd voor edge-server naar de oorsprong server cache hervalidatie bepaalt.
Koptekst behandeling verloopt | Hiermee bepaalt het genereren van Expires-koppen door een edge-server wanneer de functie externe maximumleeftijd actief is.
Externe maximumleeftijd | Bepaalt de maximumleeftijd interval voor de browser edge-server opnieuw valideren.
Interne maximumleeftijd forceren | De maximale leeftijd interval voor edge-server naar de oorsprong server cache hervalidatie bepaald.
H.264-ondersteuning (http-progressief downloaden) | Bepaalt de soorten H.264 bestandsindelingen die kunnen worden gebruikt om inhoud streamen.
EEr No-Cache-aanvraag | Hiermee wordt bepaald of Nee-cache-aanvragen van een HTTP-client zal worden doorgestuurd naar de oorspronkelijke server.
...
Ongeldig bereiken negeren | Bepaalt de reactie die naar clients wordt geretourneerd wanneer een aanvraag statuscode 416 aangevraagd bereik niet geldig genereert.
Interne Max-verouderd | Bepaalt hoe lang voorbij de verlooptijd van de normale een activum in de cache kan worden geleverd vanuit een edge-server wanneer de edge-server niet kan valideren van de cache asset met de bronserver.
...
Statuscodes voor caching geschikte instellen | Definieert de set van statuscodes die tot de inhoud in cache leiden kunnen.
Verouderde Contentlevering bij fout | Bepaalt of verlopen in de cache inhoud wordt geleverd als een fout optreedt tijdens de hervalidatie van de cache of bij het ophalen van de aangevraagde inhoud op de bronserver van de klant.
Verouderde tijdens Revalidate | Verbetert de prestaties doordat onze edge-servers om de verouderde client leveren aan de aanvrager terwijl hervalidatie plaatsvindt.
Opmerking | De functie commentaar kunt een opmerking in een regel worden toegevoegd.
...
Waarde|Resultaat
--|--
Ingeschakeld|Kunnen onze randservers bandbreedte, snelheidsbeperking aanvragen.
Uitgeschakeld|Zorgt ervoor dat op onze randservers voor het negeren van bandbreedte, snelheidsbeperking parameters. De gevraagde inhoud normaal kan worden geleverd (dat wil zeggen, zonder bandbreedtebeperking).

**Standaardgedrag:** ingeschakeld.

###<a name="bandwidth-throttling"></a>Bandbreedtebeperking
**Doel:** beperkt de bandbreedte voor de respons van onze randservers.
...
**Doel:** Hiermee bepaalt u het genereren van Cache-Control headers door de edge-server wanneer externe maximumleeftijd functie actief is.
...
Een cache-sleutel is het relatieve pad dat een asset voor de doeleinden van in het cachegeheugen identificeert. Met andere woorden, onze servers controleren op de versie van een cache van een activum volgens het bijbehorende pad zoals gedefinieerd door de cachesleutel.
...
Een bestandsindeling kan worden opgegeven met de Internet-mediatype (bijvoorbeeld Content-Type). Mediatype Internet is platformonafhankelijk metagegevens waarmee onze servers voor het identificeren van de bestandsindeling van een bepaald actief. Hieronder vindt u een lijst met algemene typen voor Internet-media.
...
Inschakelen van deze functie genereert Cache-Control: max-na verloop van tijd en headers op onze servers rand verloopt en ze verzenden naar de HTTP-client. Standaard wordt deze headers die zijn gemaakt met de oorspronkelijke server overschreven. Maar de behandeling van Cache-Control-Header en de onderdelen van de Header behandeling verloopt gebruikt voor het wijzigen van dit probleem.
...
- Als u deze functie op een negatieve waarde zorgt ervoor dat onze randservers om het verzenden van een Cache-Control: no-cache en een Expires-tijd die is ingesteld in het verleden met elk antwoord naar de browser. Hoewel een HTTP-client wordt de reactie niet in cache, wordt deze instelling geen invloed op de mogelijkheid onze randservers in de cache van het antwoord op de bronserver.
...
- Deze functie wordt overschreven door het maximumleeftijd interval dat is gedefinieerd in de Cache-Control of Expires-koppen gegenereerd op basis van een bronserver.
- Deze functie heeft geen invloed op de browser edge server cache revalidations. Deze typen revalidations worden bepaald door de Cache-Control of Expires-koppen verzonden naar de browser.
- Deze functie heeft geen waarneembare gevolgen heeft voor het antwoord geleverd door een edge-server aan de aanvrager. Het kan wel gevolgen heeft voor de hoeveelheid verkeer van hervalidatie van onze randservers naar de oorspronkelijke server verzonden.
- Deze functie door configureren:
...
**Doel:** bepaalt of onze CDN wordt genegeerd door de volgende richtlijnen geleverd van een bronserver:
...
Waarde|Resultaat
-|-
Ingeschakeld|Hiermee voorkomt dat onze edge-servers reageren op een aanvraag ongeldig bytebereik met statuscode 416 aangevraagd bereik niet geldig. In plaats daarvan onze servers leveren de aangevraagde asset en een 200 OK naar de client geretourneerd.
...
Een waarde die is geconfigureerd voor deze functie wordt vervangen door Cache-Control: moet-valideren of de Cache-Control: proxy-headers heeft ontvangen van de bronserver de validatie opnieuw. Als een van deze koppen wordt ontvangen op de bronserver wanneer u een actief is in eerste instantie in de cache opgeslagen, klikt u vervolgens in de edge-server niet een verlopen in de cache asset fungeren. In dat geval als de edge-server kan niet valideren met de oorsprong wanneer de activa maximumleeftijd interval is verstreken, wordt retourneert de edge-server een 504 Gateway timeout gegenereerd.
...
Bepaalt of verlopen in de cache inhoud wordt geleverd als een fout optreedt tijdens de hervalidatie van de cache of bij het ophalen van de aangevraagde inhoud op de bronserver van de klant.

Waarde|Resultaat
-|-
Ingeschakeld|Verouderde inhoud kan worden geleverd aan de aanvrager wanneer er een fout optreedt tijdens een verbinding met een bronserver.
Uitgeschakeld|Fout bij de oorspronkelijke server doorgestuurd naar de aanvrager.

**Standaardgedrag:** uitgeschakeld

###<a name="stale-while-revalidate"></a>Verouderde tijdens Revalidate
**Doel:** verbetert de prestaties doordat onze randservers om verouderde informatie leveren aan de aanvrager terwijl hervalidatie plaatsvindt.
...
Naam | Doel
-----|--------
Leeftijd antwoordheader | Hiermee wordt bepaald of een antwoordheader leeftijd wordt opgenomen in het antwoord verzonden naar de aanvrager.
Fouten opsporen in Cache antwoordheaders | Hiermee wordt bepaald of een antwoord antwoordheader van de X-EC-Debug die informatie over het cachebeleid voor de aangevraagde asset bevat kan bevatten.
...
Stel aangepaste Header voor Client-IP | Kan het IP-adres van de aanvragende client moet worden toegevoegd aan de aanvraag als een aangepaste aanvraagheader.
...
Waarde|Resultaat
--|--
Ingeschakeld | De antwoordheader van de leeftijd worden opgenomen in het antwoord verzonden naar de aanvrager.
Uitgeschakeld | De antwoordheader van de leeftijd worden van het antwoord verzonden naar de aanvrager uitgesloten.
...
- Verwijderen van een koptekst, kunnen er vanaf een bronserver met onze randservers die ernaar worden doorgestuurd.
...
- Aanvraag is niet in aanmerking voor de identificatie van een koptekst genomen. Elk van de volgende variaties van de naam van de Cache-Control-header kan bijvoorbeeld worden gebruikt bij het identificeren ervan:
    ...
    - CACHE-CONTROL
    - cachE-Control
- Verwijderen van een koptekst, kunnen er worden doorgestuurd naar de aanvrager.
- De volgende headers zijn gereserveerd en kunnen niet worden gewijzigd door deze functie:
    - Accepteer codering
    - leeftijd
    - verbinding
...
De optie Header-naam definieert de naam van de aangepaste aanvraagheaders waarin de client-IP-adres wordt opgeslagen.
...
Naam | Doel
-----|--------
Aangepast logboekveld 1 | Bepaalt de indeling en de inhoud die wordt toegewezen aan het aangepaste logboekveld in een onbewerkte logboekbestand.
Queryreeks logboek | Hiermee wordt bepaald of een queryreeks samen met de URL in Logboeken worden opgeslagen.
...
Naam | Doel
-----|--------
Maximum aantal keepalive-aanvragen | Definieert het maximum aantal aanvragen voor een Keep-Alive verbinding wordt gesloten.
Speciale proxy-kopteksten | Definieert de set van CDN-specifieke aanvraagheaders die uit een edge-server, worden doorgestuurd naar een bronserver.
...
**Doel:** definieert de set met [aanvraagheaders CDN-specifieke]() die zal worden doorgestuurd vanaf een edge-server naar een bronserver.
...
Naam | Doel
-----|--------
Caching geschikte HTTP-methoden | Bepaalt de set met extra HTTP-methoden die in de cache op ons netwerk kunnen worden opgeslagen.
Grootte van standaardberichthoofdtekst voor caching geschikte aanvraag | Hiermee definieert u de drempelwaarde voor het bepalen of een POST-antwoord in cache.

###<a name="cacheable-http-methods"></a>Caching geschikte HTTP-methoden
**Doel:** bepaalt de set met extra HTTP-methoden die in de cache op ons netwerk kunnen worden opgeslagen.
...
Naam | Doel
-----|--------
Omleidingen volgen | Hiermee bepaalt u of aanvragen kunnen worden omgeleid naar de hostnaam die is gedefinieerd in de locatieheader geretourneerd door een klant-bronserver.
URL-omleiding | U wordt doorgestuurd aanvragen via de locatie-header.
Herschrijven van URL  | Herschrijft de aanvraag-URL.
...
In dit voorbeeld wordt het omleiden van een CNAME-URL die wordt omgezet naar dit basis-URL in CDN-rand wordt gedemonstreerd: http://marketing.azureedge.net/brochures
...
-|-
 ...
 Deze functie kunt onze randservers om het herschrijven van de URL zonder dat u een traditionele omleiding uitvoert. Dit betekent dat de aanvrager de dezelfde antwoordcode ontvangt alsof de herschreven URL hadden aangevraagd.
...
In dit voorbeeld wordt gedemonstreerd het omleiden van een edge CNAME URL uit hoofdletters in kleine letters reguliere expressies gebruiken.
