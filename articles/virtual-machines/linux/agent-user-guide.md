...
De Microsoft Azure Linux Agent (waagent) beheert Linux en FreeBSD inrichting en VM interactie met de Azure-Infrastructuurcontroller. Dit biedt de volgende functionaliteit voor Linux en FreeBSD IaaS implementaties:
...
* identiteitsgegevens: poging tot het opschonen van het systeem, waardoor het geschikt is om opnieuw in te richten. Deze bewerking verwijderd van het volgende:
...
Een configuratiebestand (/ etc/waagent.conf) bepaalt de acties van waagent. Een voorbeeldconfiguratiebestand wordt hieronder weergegeven:
...
Het versleutelingstype voor het nieuwe sleutelpaar kan worden geconfigureerd door de vermelding Provisioning.SshHostKeyPairType. Houd er rekening mee dat sommige distributies opnieuw SSH-sleutelparen voor eventuele ontbrekende versleutelingstypen gemaakt worden wanneer de SSH-daemon opnieuw wordt gestart (bijvoorbeeld, na het opnieuw opstarten).

**Provisioning.SshHostKeyPairType:**  
Type: String  
Standaard: rsa

Dit kan worden ingesteld op een versleutelingstype van de algoritme die wordt ondersteund door de SSH-daemon op de virtuele machine. De doorgaans ondersteunde waarden zijn 'rsa', 'dsa' en 'ecdsa'. Houd er rekening mee 'putty.exe' in Windows biedt geen ondersteuning voor 'ecdsa'. Dus als u van plan bent putty.exe gebruiken op Windows verbinding maken met een Linux-implementatie, gebruik 'rsa' of 'dsa'.

**Provisioning.MonitorHostName:**  
Type: Booleaanse  
Standaard: y

Als is ingesteld, waagent worden bewaken van de virtuele Linux-machine voor wijzigingen van de hostnaam (zoals geretourneerd door de opdracht 'hostname') en automatisch bijwerken van de netwerkconfiguratie in de afbeelding in overeenstemming met de wijziging. Om de naamswijziging push naar de DNS-servers, wordt netwerk opgestart in de virtuele machine. Dit resulteert in het kort verlies van verbinding met Internet.
...
Als is ingesteld, de resource-schijf die door het platform wordt geformatteerd en door waagent gekoppeld als het type bestandssysteem is aangevraagd door de gebruiker in 'ResourceDisk.Filesystem' iets anders dan 'ntfs'. Een enkele partitie van het type Linux (83) wordt beschikbaar gemaakt op de schijf. Houd er rekening mee dat deze partitie niet geformatteerd als met succes worden gekoppeld.
...
Hiermee geeft u het pad waar de resource-schijf is gekoppeld. Denk eraan dat de resource-schijf is een *tijdelijke* schijfruimte en kan worden leeggemaakt wanneer de virtuele machine is gemaakt.
...
Dit kan worden gebruikt om op te geven van een alternatief pad voor de openssl binaire moet worden gebruikt voor cryptografische bewerkingen.
...
* De volgende configuratieparameters hebben geen effect op Ubuntu Cloud installatiekopieën die cloud init gebruiken voor het beheren van de resource-schijf en wisselruimte:
  ...
  * **ResourceDisk.EnableSwap**
  * **ResourceDisk.SwapSizeMB**
* Zie de volgende bronnen voor het configureren van het koppelpunt van de resource-schijf en wisselruimte op Ubuntu Cloud installatiekopieën tijdens het inrichten:
