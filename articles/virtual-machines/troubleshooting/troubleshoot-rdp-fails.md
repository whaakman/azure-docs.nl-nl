---
title: RDP mislukt als er geen extern bureaublad-licentieserver beschikbaar is voor het bieden van een licentie in Azure | Microsoft Docs
description: Informatie over het oplossen van problemen met RDP mislukt, omdat er geen extern bureaublad-licentieserver beschikbaar | Microsoft Docs
...
ms.locfileid: "49989190"
---
# <a name="rdp-fails-if-no-remote-desktop-license-server-is-available-in-azure"></a>RDP mislukt als er geen extern bureaublad-licentieserver beschikbaar in Azure is

Dit artikel helpt los het probleem waarbij u geen verbinding naar een Azure-virtuele Machine (VM) maken omdat er geen extern bureaublad-licentieserver beschikbaar is voor het bieden van een licentie.

## <a name="symptoms"></a>Symptomen

Wanneer u probeert verbinding maken met een virtuele machine, treden de volgende scenario's:

- De virtuele Machine (VM)-schermafbeelding ziet u dat het besturingssysteem volledig geladen en in afwachting van referenties is.
- U ontvangt de volgende foutberichten weergegeven wanneer u probeert een Microsoft Remote Desktop Protocol (RDP)-verbinding te maken:
  ...
  - **Er is geen extern bureaublad-licentieserver is beschikbaar. Extern bureaublad-Services meer werken omdat deze computer voorbij de respijtperiode is en heeft geen verbinding gemaakt met ten minste een geldige Windows Server 2008-licentieserver. Klik op dit bericht extern bureaublad-sessiehost-Server configureren voor het gebruik van Licentiecontrole openen.**
...
Dit probleem treedt op als een extern bureaublad-licentieserver niet beschikbaar is voor een licentie voor het starten van een externe sessie. Dit probleem kan worden veroorzaakt door verschillende scenario's, zelfs als een Remote Desktop Session Host-rol is ingesteld op de virtuele machine:

- Er is nooit een extern bureaublad-licentieverlening rol in de omgeving en de respijtperiode (180 dagen) ligt boven de.
- Een licentie voor extern bureaublad in de omgeving is geïnstalleerd, maar deze nooit is geactiveerd.
...
1. Verbinding maken met de virtuele machine met een beheersessie:
  ...
  Als u geen verbinding met de virtuele machine maken met behulp van een beheersessie, kunt u de [seriële Console](serial-console-windows.md) toegang krijgen tot de virtuele machine als volgt:
...
2. Controleer of de virtuele machine een Remote Desktop Session Host-rol is ingeschakeld heeft. Als de functie is ingeschakeld, controleert u of deze correct werkt. Open een verhoogde CMD-exemplaar en volg deze stappen:
  ...
  2. Gebruik de volgende opdracht om te controleren van het beleid en zo nodig opnieuw te configureren:
    ...
    Als de **SpecifiedLicenseServers** waarde bestaat niet of heeft een onjuiste licentiegegevens voor de server, als volgt wijzigen:
  ...
  4. Als u geen CAL's hebt, verwijdert u de Remote Desktop Session Host-rol. Vervolgens wordt het RDP-protocol wordt opnieuw worden ingesteld op Normaal en hierdoor kan alleen twee gelijktijdige RDP-verbindingen met de virtuele machine.

    ```
    dism /ONLINE /Disable-feature /FeatureName:Remote-Desktop-Services
    ```

    Als de virtuele machine de rol van extern bureaublad-licentieverlening heeft en wordt niet gebruikt, kunt u ook die rol verwijderen.
