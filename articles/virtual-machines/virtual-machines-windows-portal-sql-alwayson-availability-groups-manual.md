...
> [!NOTE]
> Currently the Azure Portal only allows you to use a specific frontend port once on a load balancer. To use the same port for all listeners, use PowerShell to attach the listener IP addresses to the load balancer. For instructions, see [Create availability group listener and load balancer | Azure](virtual-machines-windows-portal-sql-ps-alwayson-int-listener.md). 
> 
> 
