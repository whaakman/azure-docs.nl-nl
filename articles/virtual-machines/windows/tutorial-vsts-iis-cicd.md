---
title: 'Zelfstudie: een CI/CD-pijplijn te maken in Azure met Team Services | Microsoft Docs'
description: In deze zelfstudie leert u over het maken van een Visual Studio Team Services-pijplijn voor continue integratie en levering waarmee een web-app in IIS op een Windows-VM in Azure wordt geïmplementeerd.
...
ms.locfileid: "37932089"
---
# <a name="tutorial-create-a-continuous-integration-pipeline-with-visual-studio-team-services-and-iis"></a>Zelfstudie: Een pijplijn voor continue integratie met Visual Studio Team Services en IIS maken
Voor het automatiseren van de build-, test- en implementatiefasen van toepassingsontwikkeling, kunt u een continue integratie en implementatie (CI/CD)-pijplijn. In deze zelfstudie maakt u een CI/CD-pijplijn met behulp van Visual Studio Team Services en een Windows virtuele machine (VM) in Azure waarop IIS wordt uitgevoerd. In deze zelfstudie leert u procedures om het volgende te doen:

> [!div class="checklist"]
> * Een ASP.NET-webtoepassing met een Team Services-project publiceren
...
> * De IIS-exemplaar toevoegen aan een implementatiegroep in Team Services
> * Maken van een release definitie voor het publiceren van nieuwe web-pakketten implementeren in IIS
...
## <a name="create-project-in-team-services"></a>Project maken in Team Services
Visual Studio Team Services kunnen eenvoudig samenwerking en ontwikkeling zonder het onderhouden van een on-premises-oplossing voor het beheer van code. Teamservices biedt cloud code testen, build en application insights. U kunt een versie besturingselement opslagplaats en IDE die het beste past bij de ontwikkeling van uw code. Voor deze zelfstudie maakt kunt u een gratis account gebruiken om een standaard ASP.NET-web-app en CI/CD-pijplijn te maken. Als u nog geen een Team Services-account, [maakt u er een](http://go.microsoft.com/fwlink/?LinkId=307137).

Voor het beheren van de code van het doorvoerproces, definities bouwen en release-definities, een project maken in Team Services als volgt:

1. Uw Team Services-dashboard in een webbrowser openen en kies **nieuw project**.
...
In de vorige stap hebt u een project in Team Services gemaakt. De laatste stap wordt het nieuwe project geopend in Visual Studio. Beheren van uw code doorvoeringen in de **Team Explorer** venster. Maken van een lokale kopie van het nieuwe project en maak vervolgens een ASP.NET-webtoepassing met een sjabloon als volgt:

1. Selecteer **kloon** te maken van een lokale git-opslagplaats van uw Team Services-project.
    
    ![Kloon de opslagplaats van Team Services-project](media/tutorial-vsts-iis-cicd/clone_repo.png)
...
5. In de **Team Explorer** venster, kiest u **wijzigingen**.
    ...
    ![Voer lokale wijzigingen door in Team Services git-opslagplaats](media/tutorial-vsts-iis-cicd/commit_changes.png)
...
In Team Services, kunt u een build-definitie gebruiken om te beschrijven hoe uw toepassing moet worden gemaakt. In deze zelfstudie maken we een eenvoudige definitie die wordt onze broncode, bouwt de oplossing en maakt vervolgens web pakket die we gebruiken kunnen voor het uitvoeren van de web-app op een IIS-server implementeren.

1. In uw Team Services-project, kies **Build & Release** aan de bovenzijde, selecteert u vervolgens **bouwt**.
...
5. Laat alle standaard waarden van de taak. Onder **bronnen ophalen**, zorg ervoor dat de *myWebApp* opslagplaats en *master* vertakking worden geselecteerd.
    ...
    ![Build-definitie in Team Services-project maken](media/tutorial-vsts-iis-cicd/create_build_definition.png)

6. Op de **Triggers** tabblad, verplaats de schuifregelaar voor **inschakelen van deze trigger** naar *ingeschakeld*.
7. De build-definitie en de wachtrij een nieuwe build opslaan door **opslaan en in de wachtrij** , klikt u vervolgens **opslaan en in de wachtrij** opnieuw. Laat de standaardwaarden en selecteer **wachtrij**.
...
![Geslaagde build van Team Services-project](media/tutorial-vsts-iis-cicd/successful_build.png)


## <a name="create-virtual-machine"></a>Virtuele machine maken
Voor een platform voor het uitvoeren van uw ASP.NET-web-app, moet u een virtuele machine van Windows waarop IIS wordt uitgevoerd. Teamservices maakt gebruik van een agent om te communiceren met de IIS-exemplaar als u code doorvoert en builds worden geactiveerd.

Maak een Windows Server 2016-VM met [New-AzureRmVM](/powershell/module/azurerm.compute/new-azurermvm). Het volgende voorbeeld wordt een virtuele machine met de naam *myVM* in de *VS-Oost* locatie. De resourcegroep *myResourceGroupVSTS* en ondersteunende netwerkbronnen ook worden gemaakt. Om toe te staan webverkeer te genereren, TCP-poort *80* wordt geopend met de virtuele machine. Geef desgevraagd een gebruikersnaam en wachtwoord moet worden gebruikt als de aanmeldingsreferenties voor de virtuele machine:
...
Distribueren Internet pakket implementeren op de IIS-server, het definiëren van een implementatiegroep in Team Services. Deze groep kunt u opgeven welke servers het doel van de nieuwe builds worden als u code in Team Services doorvoeren en builds worden voltooid.

1. Kies in Team Services, **Build & Release** en selecteer vervolgens **implementatiegroepen**.
...
Toevoegen aan de implementatiegroep is gemaakt, elk IIS-exemplaar aan de groep. Teamservices genereert een script dat wordt gedownload en configureert u een agent op de virtuele machine die ontvangt van de nieuwe web-pakketten implementeren en vervolgens toegepast op IIS.

1. Klik in de **beheerder PowerShell** sessie op de virtuele machine, plakken en voer het script hebt gekopieerd vanuit Team Services.
...
5. In de **implementatiegroepen** pagina van de **Build & Release** menu, open de *myIIS* implementatiegroep. Op de **Machines** tabblad, zorg ervoor dat uw virtuele machine wordt weergegeven.
    ...
    ![Virtuele machine toegevoegd aan de groep van Team Services-implementatie](media/tutorial-vsts-iis-cicd/deployment_group.png)


## <a name="create-release-definition"></a>Release-definitie maken
Voor het publiceren van uw builds, maakt u een release-definitie in Team Services. Deze definitie wordt automatisch geactiveerd door een build van uw toepassing. U hebt gekozen de implementatiegroep om uw web-pakket dat u wilt implementeren, en definieert u de juiste IIS-instellingen.

1. Kies **Build & Release**en selecteer vervolgens **bouwt**. Kies de build-definitie in een vorige stap hebt gemaakt.
2. Onder **onlangs voltooid**, kies de meest recente build, en selecteer vervolgens **Release**.
3. Kies **Ja** te maken van een release-definitie.
4. Kies de **leeg** sjabloon, selecteert u vervolgens **volgende**.
5. Controleer of de build-definitie van het project en de bron worden ingevuld met uw project.
...
7. Selecteer de vervolgkeuzelijst naast **+ toevoegen taken** en kies *toevoegen van een groep implementatiefase*.
    ...
    ![Als u wilt vrijgeven definitie in Team Services-taak toevoegen](media/tutorial-vsts-iis-cicd/add_release_task.png)
...
9. Selecteer de **uitvoeren op implementatiegroep** bovenliggende taak.
    ...
    ![Release-definitie implementatie groepstaak voor IIS](media/tutorial-vsts-iis-cicd/release_definition_iis.png)
...
1. Selecteer in de release-definitie **+ Release**, en kies vervolgens **Create Release**.
...
3. Kleine wordt een banner weergegeven aan de bovenkant van uw release-definitie, zoals *Release 'Release-1' is gemaakt*. Selecteer de koppeling release.
4. Open de **logboeken** tab om de voortgang van de release bekijken.
    
    ![Geslaagde Team Services-release en web implementeren pakket push](media/tutorial-vsts-iis-cicd/successful_release.png)
...
7. In Team Services-werkruimte, wordt een nieuwe build van het doorvoeren van de code geactiveerd. 
    - Kies **Build & Release**en selecteer vervolgens **bouwt**. 
    - Kies uw build-definitie, en selecteer vervolgens de **in de wachtrij geplaatst & uitgevoerd** build te kijken de voortgang van de build.
...
In deze zelfstudie maakt u een ASP.NET-webtoepassing in Team Services gemaakt en geconfigureerd build en release-definities voor het implementeren van nieuwe web-pakketten implementeren in IIS op elke doorvoercode. U hebt geleerd hoe u:
