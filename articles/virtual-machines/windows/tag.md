...
Als u toevoegen van tags via PowerShell wilt, kunt u de `Set-AzResource` opdracht. Houd er rekening mee bij het bijwerken van tags via PowerShell, tags worden bijgewerkt als geheel. Dus als u een tag wilt aan een resource die al tags toevoegen heeft, moet u om op te nemen van de labels die u wilt worden geplaatst op de resource. Hieronder volgt een voorbeeld van het toevoegen van extra labels aan een resource via PowerShell-Cmdlets.

Deze eerste cmdlet worden alle van de labels op geplaatst *MyTestVM* naar de *$tags* variabele, met behulp van de `Get-AzResource` en `Tags` eigenschap.
