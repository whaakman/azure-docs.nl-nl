...
Als u de [klassieke portal](https://manage.windowsazure.com/) van Azure gebruikt, zijn X.509-certificaten in PEM-indeling vereist voor de SSH-sleutels.  Er zijn geen andere typen openbare SSH-sleutels toegestaan. Het *moeten* X.509-certificaten zijn.
...
Als u gebruikmaakt van het klassieke implementatiemodel (`asm`-modus in de CLI), kunt u een openbare SSH-RSA-sleutel of een RFC4716-sleutel gebruiken in een PEM-container.  De openbare SSH-RSA-sleutel is de sleutel die eerder in dit artikel is gemaakt met behulp van `ssh-keygen`.
