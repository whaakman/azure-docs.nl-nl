...
* [SUSE SAP HA gidsen met aanbevolen procedures] [ suse-ha-guide] de handleidingen bevat alle vereiste gegevens voor het instellen van Netweaver HA en SAP HANA-Systeemreplicatie on-premises. Gebruik die dit als een algemene basislijn leidt. Ze bieden nog veel meer gedetailleerde informatie.
* [Maximaal beschikbare NFS-opslag met DRBD en Pacemaker] [ suse-drbd-guide] de handleiding bevat alle vereiste gegevens voor het instellen van een maximaal beschikbare NFS-server. Deze handleiding gebruiken als een basislijn.
...
![Overzicht van SAP NetWeaver hoge beschikbaarheid](./media/high-availability-guide-suse/img_001.png)
...
SAP NetWeaver moet een gedeelde opslag voor de map transport en profiel. Lezen [hoge beschikbaarheid voor NFS op Azure VM's in SUSE Linux Enterprise Server] [ nfs-ha] over het instellen van een NFS-server voor SAP NetWeaver.
...
1. SAP HANA beveiligd archief bijwerken
   ...
   De uitvoer ziet u dat het IP-adres van de standaardvermelding die naar de virtuele machine en niet naar het IP-adres van de load balancer verwijst. Dit item moet worden gewijzigd om te verwijzen naar de virtuele hostnaam van de load balancer. Zorg ervoor dat u dezelfde poort (**30313** in de bovenstaande uitvoer).
