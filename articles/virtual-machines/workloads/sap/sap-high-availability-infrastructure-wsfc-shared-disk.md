...
* [Cloudtoepassingsarchitectuur-handleiding: een SAP ASCS/SCS-exemplaar op een Windows-failovercluster met behulp van een gedeelde clusterschijf van Cluster][sap-high-availability-guide-wsfc-shared-disk]
...
- In de Azure-portal in de **Parameters** deelvenster in de **SYSTEMAVAILABILITY** Schakel **HA**.
  ...
  ![Afbeelding 1: Stel SAP hoge beschikbaarheid Azure Resource Manager-parameters][sap-ha-guide-figure-3000]

_**Afbeelding 1:** ingesteld SAP hoge beschikbaarheid Azure Resource Manager-parameters_
...
In de ASCS/SCS multi-SID-sjabloon instellen de [ASCS/SCS multi-SID sjabloon] [ sap-templates-3-tier-multisid-xscs-marketplace-image] of [ASCS/SCS multi-SID-sjabloon met behulp van Managed Disks] [ sap-templates-3-tier-multisid-xscs-marketplace-image-md], voer waarden in voor de volgende parameters:
  ...
  - **Resource-voorvoegsel**: de resource-voorvoegsel dat wordt gebruikt als voorvoegsel voor alle resources die zijn gemaakt tijdens de implementatie instellen. Omdat de resources niet tot slechts één SAP-systeem behoren, wordt het voorvoegsel van de resource is niet de SID van een SAP-systeem.  Het voorvoegsel moet tussen de drie tot zes tekens lang zijn.
  - **Stack-Type**: Selecteer het type van de stack van de SAP-systeem. Azure Load Balancer zijn afhankelijk van het type stack, een (ABAP of alleen Java) of twee (ABAP + Java) privé IP-adressen per SAP-systeem.
  ...
  -  **Admin Username en het wachtwoord van beheerder**: Maak een nieuwe gebruiker die zich aanmeldt bij de computer kan worden gebruikt.
  -  **Nieuwe of bestaande Subnet**: stellen of wilt maken van een nieuw virtueel netwerk en een subnet of een bestaand subnet. Als u al een virtueel netwerk dat is verbonden met uw on-premises netwerk hebt, selecteert u **bestaande**.
...
Voor het instellen van de sjabloon van de database-multi-SID, in de [database multi-SID sjabloon] [ sap-templates-3-tier-multisid-db-marketplace-image] of [multi-SID-sjabloon voor database met behulp van Managed Disks] [ sap-templates-3-tier-multisid-db-marketplace-image-md], voer waarden in voor de volgende parameters:
  ...
  -  **SAP-systeem grootte**: het aantal SAP's vindt u het nieuwe systeem instellen. Als u niet zeker weet hoeveel SAP's het systeem is vereist, vraagt u uw SAP-technologie-Partner of systeemintegrator.
  -  **Beschikbaarheid van het systeem**: Selecteer **HA**.
  -  **Admin Username en het wachtwoord van beheerder**: Maak een nieuwe gebruiker die zich aanmeldt bij de computer kan worden gebruikt.
...
2.  Selecteer uw instellingen op basis van het type netwerk die u hebben. Zie de volgende bronnen voor meer informatie:
    ...
    * [Alleen in de cloud implementatie][planning-guide-2.1]: een extra virtuele machine in hetzelfde Virtueelnetwerk-exemplaar dat als een DNS-server fungeert implementeren. Toevoegen het IP-adressen van de Azure virtuele machines die u hebt ingesteld tot de DNS-service wordt uitgevoerd.
...
2.  In de **IP-adressen** deelvenster onder **toewijzing**, selecteer **statische**. In de **IP-adres** voert u de IP-adres dat u wilt gebruiken.
  ...
  ![Afbeelding 3: Statisch IP-adressen voor de netwerkkaart van elke virtuele machine instellen][sap-ha-guide-figure-3002]
...
| ... |... |... |... |
| Laatste SAP application server-exemplaar |di-5-PR1 |PR1-nic-di-5 |10.0.0.55 |
...
3.  Stel het IP-adres van de interne load balancer **pr1-lb-dbms** naar het IP-adres van de virtuele host-naam van de DBMS-exemplaar.
  ...
  ![Afbeelding 4: Statisch IP-adressen instellen voor de interne load balancer voor de SAP ASCS/SCS-exemplaar][sap-ha-guide-figure-3003]
...
| HTTP-berichtenserver / *Lbrule8100* |81\<InstanceNumber\> |8100 |
...
| HTTP-berichtenserver / *Lbrule8101* |81\<InstanceNumber\> |8101 |
...
2.  Voor alle load balancer-regels die deel uitmaken van het SAP ASCS of SCS-exemplaar, kunt u deze waarden wijzigen:
  ...
  * Back-end-poort
  ...
  ![Afbeelding 6: Wijzigen in de ASCS/SCS standaard load balancer-regels voor de interne Azure load balancer][sap-ha-guide-figure-3005]
...
**Tabel 3:** wijzigt u de eerste parameter van de TCP/IP
...
2.  Het failover-cluster instellen met behulp van Failoverclusterbeheer. Selecteer in Failoverclusterbeheer **Cluster maken**, en voegt u alleen de naam van het eerste cluster (een knooppunt). Het tweede knooppunt niet nog; toevoegen u kunt het tweede knooppunt toevoegen in een latere stap.
  ...
  ![Afbeelding 8: De server of virtuele machine de naam van het eerste clusterknooppunt toevoegen][sap-ha-guide-figure-3007]

  _**Afbeelding 8:** de server of virtuele machine de naam van het eerste clusterknooppunt toevoegen_
...
3.  Voer de netwerknaam (virtuele host-naam) van het cluster.
  ...
  _**Afbeelding 9:** de clusternaam invoeren_
...
4.  Nadat u het cluster hebt gemaakt, voert u een clustervalidatietest.
  ...
  _**Afbeelding 10:** de validatiecontrole voor cluster uitvoeren_
...
5.  Wijzig het IP-adres van de core cluster-service. Het cluster kan totdat u het IP-adres van de core cluster-service niet starten omdat het IP-adres van de server naar een van de virtuele machine-knooppunten verwijst. Dit doen op de **eigenschappen** pagina van de core-clusterservice IP-resource.
  ...
  ![Afbeelding 13: Wijzigen In het dialoogvenster Eigenschappen van het IP-adres][sap-ha-guide-figure-3012]

  _**Afbeelding 13:** In de **eigenschappen** dialoogvenster vak, wijzigt u het IP-adres_

  ![Afbeelding 14: De IP-adres dat is gereserveerd voor het cluster toewijzen][sap-ha-guide-figure-3013]

  _**Afbeelding 14:** toewijzen van de IP-adres dat is gereserveerd voor het cluster_

6.  Virtuele hostnaam van het cluster online brengen.

  ![Afbeelding 15: De cluster-core-service actief is, met de juiste IP-adres][sap-ha-guide-figure-3014]

  _**Afbeelding 15:** de cluster-core-service actief is, met de juiste IP-adres_
...
3.  Selecteer om de clusternaamobject toe aan de lijst met **toevoegen**. Wijzig het filter om te controleren op computerobjecten, naast die wordt weergegeven in afbeelding 22.
  ...
  ![Afbeelding 21: Wijziging objecttypen om op te nemen van computers][sap-ha-guide-figure-3020]
...
5.  Selecteer de **Security** tabblad van de share en stel vervolgens meer gedetailleerde machtigingen voor het clusternaamobject.
  ...
  ![Afbeelding 23: Beveiligingskenmerken van voor de clusternaamobject instellen op het bestandsshare quorum][sap-ha-guide-figure-3022]
...
1.  Open de instelling-Wizard clusterquorum configureren.
  ...
  _**Afbeelding 24:** Start de Wizard configureren clusterquorum-instelling_

2.  Op de **optie voor quorumconfiguratie** pagina, selecteert u **selecteert u de quorumwitness**.

  ![Afbeelding 25: Quorumconfiguraties die kunt u kiezen uit][sap-ha-guide-figure-3024]
...
4.  Geef het UNC-pad naar de bestandsshare (in ons voorbeeld \\domcontr 0\FSW). Voor een overzicht van de wijzigingen die u kunt maken, selecteert u **volgende**.
  ...
  ![Afbeelding 27: Definieer de bestandssharelocatie voor de witness-share][sap-ha-guide-figure-3026]

  _**Afbeelding 27:** de bestandssharelocatie voor de share witness definiëren_
...
5.  Selecteer de gewenste wijzigingen aan, en selecteer vervolgens **volgende**. U moet de configuratie van het cluster is opnieuw te configureren zoals wordt weergegeven in afbeelding 28:  
  ...
  _**Afbeelding 28:** bevestiging dat u het cluster hebt geconfigureerd_
...
- Gebruik de toevoegen Wizard functies en onderdelen in Windows, zoals wordt weergegeven in afbeelding 29:
  ...
  ![Afbeelding 29: Installeer .NET Framework 3.5 met behulp van de toevoegen Wizard functies en onderdelen][sap-ha-guide-figure-3028]

  _**Afbeelding 29:** Installeer .NET Framework 3.5 met behulp van de toevoegen Wizard functies en onderdelen_

  ![Afbeelding 30: Installatie van de voortgangsbalk wanneer u .NET Framework 3.5 installeren met behulp van de toevoegen Wizard functies en onderdelen][sap-ha-guide-figure-3029]
...
2.  Selecteer in het dialoogvenster **Ja**.
  ...
  ![Afbeelding 32: DataKeeper u wordt geïnformeerd dat een service wordt uitgeschakeld][sap-ha-guide-figure-3032]
...
3.  In het dialoogvenster wordt aangeraden dat u selecteert **domein-of Server**.
  ...
  ![Afbeelding 33: De selectie van de gebruiker voor SIOS DataKeeper][sap-ha-guide-figure-3033]
...
5.  Installeer de licentiesleutel voor uw SIOS DataKeeper-exemplaar, zoals wordt weergegeven in afbeelding 35.
  ...
  ![Afbeelding 35: Voer de licentiesleutel SIOS DataKeeper][sap-ha-guide-figure-3035]
...
1.  Start het configuratiehulpprogramma DataKeeper-beheer en de configuratie en selecteer vervolgens **verbinding maken met Server**.
  ...
  ![Afbeelding 36: SIOS DataKeeper beheer en hulpprogramma voor serverconfiguratie][sap-ha-guide-figure-3036]
...
2.  Voer de naam of de TCP/IP-adres van het eerste knooppunt dat het hulpprogramma voor beheer en de configuratie verbinding met, en, in een tweede stap van het tweede knooppunt maken moet.
  ...
  ![Afbeelding 37: Voegt u de naam of TCP/IP-adres van het eerste knooppunt van het beheer en hulpprogramma voor serverconfiguratie verbinding moeten maken, en met een tweede stap van het tweede knooppunt][sap-ha-guide-figure-3037]

  _**Afbeelding 37:** plaatst u de naam of de TCP/IP-adres van het eerste knooppunt het hulpprogramma voor beheer en de configuratie verbinding, en met een tweede stap van het tweede knooppunt maken moet_
...
4.  De naam van de replicatietaak definiëren.
  ...
  _**Afbeelding 39:** definiëren van de naam van de replicatietaak_
...
5.  Definieer de naam, de TCP/IP-adres en het schijfvolume van het doelknooppunt.
  ...
  ![Afbeelding 41: Definieer de naam, de TCP/IP-adres en het schijfvolume van het huidige doelknooppunt][sap-ha-guide-figure-3041]
...
8.  Definiëren of het volume dat door de replicatietaak voor wordt gerepliceerd naar een Windows Server failover cluster-configuratie als een gedeelde schijf moet worden weergegeven. Selecteer voor de configuratie SAP ASCS/SCS **Ja** zodat het Windows-cluster ziet de gerepliceerd volume als een gedeelde schijf die kan worden gebruikt als een clustervolume.
  ...
  _**Afbeelding 44:** DataKeeper synchroon spiegelen voor de SAP ASCS/SCS schijf delen is actief_
  ...
  _**Afbeelding 45:** Failoverclusterbeheer ziet u de schijf die DataKeeper gerepliceerd_
