...
Er is een afhankelijkheid tussen de Azure interne load balancer (Azure Load Balancer-service) en de groep cloud-service in het klassieke implementatiemodel Azure. Elke interne load balancer moet één cloud service-groep.

In Azure Resource Manager, hoeft u niet een Azure-resourcegroep te gebruiken Load Balancer van Azure. De omgeving is eenvoudiger en flexibeler.
...
3. SIOS DataKeeper Cluster Edition configureren zodat deze overeenkomt met de inhoud van de aanvullende VHD gekoppeld volume van de virtuele bronmachine het extra VHD gekoppeld volume van de virtuele doelmachine. SIOS DataKeeper isoleert de bron- en lokale volumes en vervolgens gepresenteerd aan het Windows Server Failover Clustering als één gedeelde schijf.
...
Omdat de Azure storage-account een potentieel storingspunt is, is het belangrijk dat u hebt ten minste twee Azure storage-accounts, waarbij ten minste twee virtuele machines worden gedistribueerd. In een ideaal instelling, zou de schijven van elke virtuele machine die wordt uitgevoerd een SAP-dialoogvenster-exemplaar worden geïmplementeerd in een ander opslagaccount.
...
Voor het instellen van de sjabloon ASC's / SCS-multi-SID in de [ASC's / SCS multi-SID sjabloon][sap-templates-3-tier-multisid-xscs-marketplace-image], voer waarden in voor de volgende parameters:
...
Voor het instellen van het databasesjabloon multi-SID in de [multi-SID databasesjabloon][sap-templates-3-tier-multisid-db-marketplace-image], voer waarden in voor de volgende parameters:
...
Voor het instellen van de sjabloon toepassingen servers multi-SID in de [toepassingssjabloon servers multi-SID][sap-templates-3-tier-multisid-apps-marketplace-image], voer waarden in voor de volgende parameters:
...
Nadat u SIOS DataKeeper op beide knooppunten hebt geïnstalleerd, moet u de configuratie te starten. Het doel van de configuratie is dat synchrone replicatie tussen de extra virtuele harde schijven gekoppeld aan elk van de virtuele machines.
