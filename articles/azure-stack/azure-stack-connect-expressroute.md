...
Het volgende diagram toont hoe meerdere tenants verbinding maken vanaf de Azure Stack-infrastructuur via de router voor ExpressRoute met Azure in de aan de rand van Microsoft:
...
Het voorbeeld in dit artikel wordt de dezelfde architectuur met meerdere tenants wordt weergegeven in afbeelding 2 verbinding maken met Azure Stack Azure met behulp van ExpressRoute-privépeering. De verbinding wordt gedaan met behulp van een site-naar-site VPN-verbinding van de virtuele netwerkgateway in Azure Stack aan een ExpressRoute-router.

De stappen in dit artikel ziet u het maken van een end-to-end-verbinding tussen twee VNets uit twee verschillende tenants in Azure Stack met bijbehorende vnet's in Azure. Instellen van twee tenants is optioneel. u kunt deze stappen ook gebruiken voor één tenant.

## <a name="configure-azure-stack"></a>Configureren van Azure Stack

Als u de Azure Stack-omgeving instelt voor de eerste tenant, door de stappen in het volgende diagram als richtlijn te gebruiken. Als u meer dan één tenant instelt bent, Herhaal deze stappen:
...
3. Onder **Azure Marketplace**, selecteer **netwerken**.
...
6. Ziet u het abonnement dat u eerder hebt ingevuld de **abonnement** veld. Voor de resterende velden:

    * Onder **resourcegroep**, selecteer **nieuw** te maken van een nieuwe resourcegroep of als u nog niet hebt, selecteert u **gebruik bestaande**.
...
1. Onder **virtueel netwerk**, selecteer **Tenant1VNet1**.
...
1. Klik op **OK** om het gatewaysubnet te maken.

#### <a name="create-the-virtual-network-gateway"></a>De gateway van het virtuele netwerk maken

1. Klik in de gebruikersportal van Azure Stack op **+ een resource maken**.
...
1. Controleer of **Abonnement** en **Locatie** juist zijn. Klik op **Create**.
...
1. Selecteer de **abonnement**, **resourcegroep**, en **locatie**. Klik op **OK**.
...
1. In **maken van virtuele machine**, selecteer **basisbeginselen**, typt u **VM01** als de **naam**.
1. Voer een geldige gebruikersnaam en wachtwoord. U gebruikt dit account aanmelden bij de virtuele machine nadat deze is gemaakt.
...
1. Onder **samenvatting**, Controleer de configuratie van de virtuele machine en klik vervolgens op **OK**.

Als u wilt toevoegen van meer tenants, Herhaal de stappen die u in deze secties:

* [Het virtuele netwerk en VM-subnet maken](#create-the-virtual-network-and-vm-subnet)
* [Het gatewaysubnet maken](#create-the-gateway-subnet)
...
Standaard staat Windows Server 2016 niet toe dat binnenkomende ICMP-pakketten via de firewall. Voor elke virtuele machine die u voor ping-tests gebruikt, moet u binnenkomende ICMP-pakketten toestaan. Voor het maken van een firewallregel voor ICMP, moet u de volgende cmdlet uitvoeren in een PowerShell-venster met verhoogde bevoegdheid:
...
1. Ping de IPv4-adres van de virtuele machine in het Azure-VNet.
   ...
   In de voorbeeldomgeving is het IPv4-adres van het subnet 10.1.1.x/24. In uw omgeving, het adres kan afwijken, maar deze moet het subnet dat u hebt gemaakt voor de tenant-VNet-subnet.
