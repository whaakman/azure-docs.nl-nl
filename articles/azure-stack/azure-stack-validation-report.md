...
ms.locfileid: "58800981"
---
# <a name="azure-stack-validation-report"></a>Azure Stack-validatierapport

Gebruik de *Azure Stack-gereedheid Checker* hulpprogramma voor het uitvoeren van validaties die ondersteuning bieden voor implementatie en het onderhoud van een Azure Stack-omgeving. Het hulpprogramma schrijft de resultaten naar een .json-rapportbestand. Het rapport bevat gedetailleerde en samengevatte gegevens over de status van de vereisten voor de implementatie van Azure Stack. Het rapport bevat ook informatie over het draaien van geheimen voor bestaande Azure Stack-implementaties.  

## <a name="where-to-find-the-report"></a>Waar vind ik het rapport

Wanneer het hulpprogramma wordt uitgevoerd, registreert hij de resultaten naar **AzsReadinessCheckerReport.json**. Het hulpprogramma maakt ook een logboek met de naam **AzsReadinessChecker.log**. De locatie van deze bestanden worden weergegeven, samen met de validatieresultaten in PowerShell:
...
- Gebruik de `-OutputPath <path>` parameter aan het einde van de opdrachtregel om op te geven van een andere locatie.
- Gebruik de `-CleanReport` parameter aan het einde van de opdrachtregel om te wissen van informatie over eerdere uitvoeringen van het hulpprogramma uit **AzsReadinessCheckerReport.json**.
...
Bijvoorbeeld, als u wilt weergeven van het rapport vanuit een PowerShell-prompt die is geopend op de locatie van het rapport, voer de volgende opdracht:
...
Als u een overzicht van het rapport, kunt u toevoegen de `-summary` parameter aan het einde van de PowerShell-opdracht. Bijvoorbeeld:

```powershell
Read-AzsReadinessReport -ReportPath .\Contoso-AzsReadinessReport.json -summary
```

De samenvatting validaties waarvoor geen resultaten bevat, en geeft aan doorgeven of mislukken voor validaties die voltooid zijn. De uitvoer lijkt op die in het volgende voorbeeld:
