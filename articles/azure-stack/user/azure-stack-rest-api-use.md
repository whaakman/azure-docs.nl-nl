...
Het proces vereist dat uw client-verificaties voor het eindpunt van Microsoft Azure-aanmelding. Het eindpunt retourneert een token. Geef het token in de koptekst van elke aanvraag verzonden naar de Stack-API van Azure. Oauth2.0 maakt gebruik van Azure.

Dit artikel bevat enkele eenvoudige voorbeelden specifieke naar Azure Stack met het hulpprogramma curl. In dit onderwerp wordt uitgelegd hoe een token voor toegang tot de Azure-Stack-API op te halen. De meeste talen bieden Oauth 2.0-bibliotheken die krachtige token beheer- en verwerking van taken die het vernieuwen van het token.

Kijken naar het hele proces van het gebruik van de Stack REST-API van Azure met een algemene REST-client vindt, zoals curl u informatie over de onderliggende aanvragen en wat u kunt verwachten in de nettolading van het antwoord is ontvangen.

In dit artikel verkennen niet ook alle opties die beschikbaar zijn voor het ophalen van tokens zoals interactieve aanmelding of specifieke App-id's maken. Zie voor meer informatie [Azure REST API-verwijzing](https://docs.microsoft.com/rest/api/).
...
Voor elke waarde:
 ...
 > Als u toegang wilt krijgen tot de tenant-API beheerder bent en vervolgens moet u ervoor zorgen eindpunt te gebruiken tenant, bijvoorbeeld 'https://adminmanagement.{region}.{Azure Stack domein} / metagegevens/eindpunten? api-version = 2015-01-011
  ...
  Bijvoorbeeld, met de Azure-Stack Development Kit:
  ```
...
### <a name="example"></a>Voorbeeld
  ...
  **Gebruikersnaam**
  ...
  De Azure-Stack AAD-account, bijvoorbeeld:
  ```
...
Algemene aanvraag-URI, bestaat uit: {URI-schema} :// {URI-host} / {bronpad}? {query-string}
