...
De IT-Service Management-Connector biedt een bidirectionele integratie tussen een ondersteund IT Service Management (ITSM) productservice en Log Analytics.  U kunt via deze verbinding, incidenten, waarschuwingen of gebeurtenissen maken in ITSM product op basis van Log Analytics waarschuwingen of logboekrecords. De connector importeert ook gegevens, zoals incidenten en wijzigingsaanvragen van ITSM product in de OMS-logboekanalyse.
...
IT-Service Management-Connector tegel zoals u ziet in de galerie met oplossingen:
...
> Standaard wordt met de IT-Service Management-Connector van de verbinding gegevens vernieuwen eenmaal in elke 24 uur. Uw verbinding om gegevens te vernieuwen direct voor alle bewerkingen of de sjabloon-updates die u aanbrengt, klikt u op de knop Vernieuwen weergegeven naast de verbinding.
...
IT-Service Management-Connector-oplossing ondersteunt verbindingen met **System Center Service Manager**, **ServiceNow**, **Provance**, en **Cherwell**. Configureer uw verbinding met
...
IT-Service Management-Connector ondersteunt momenteel integratie met de oplossing Serviceoverzicht.
...
Met ITSM Connector oplossing, kunt u waarschuwingen OMS om te activeren van het maken van werkitems in uw verbonden ITSM hulpprogramma als volgt configureren:
...
U kunt ook maken werkitems in de verbonden ITSM bronnen rechtstreeks vanuit een logboekrecord als volgt:

1. Van **logboek zoeken**, de vereiste gegevens zoeken, selecteert u de details en klik op **maken werkitem**.

    De **maken ITSM werkitem** venster wordt weergegeven:
...
3. Met een bestaande werkitemsjabloon voor een incident, klikt u op **Ja** onder **werkitem genereren op basis van de sjabloon** optie en klik vervolgens op **maken**.
...
3.  Als OMS waarschuwingen geactiveerd, maar werkt items ITSM product niet worden gemaakt of configuratie-items zijn niet gemaakt/gekoppeld aan werkitems of andere algemene informatie, vindt u in de volgende locaties:
 ...
 - Of de fouten/gerelateerde gegevens rechtstreeks in de **logboek zoeken** pagina met behulp van de query *Type = ServiceDeskLog_CL*.

## <a name="troubleshoot-service-manager-web-app-deployment"></a>Problemen met de Web-App Service Manager-implementatie oplossen
1.  Als u ervaart mogelijk problemen met web-app-implementatie, zorg ervoor dat u gemachtigd bent in het abonnement vermeld resources maken/implementeren.
