...
Zie voor meer informatie over het configureren van Azure Monitor voor virtuele machines [Azure Monitor inschakelen voor virtuele machines](monitoring-vminsights-onboard.md).

## <a name="monitoring-configuration-details"></a>Informatie over de configuratie controleren
In deze sectie geeft een overzicht van de standaard status criteria gedefinieerd voor het bewaken van Azure Windows en Linux-machines.
...
![Azure Monitor voor een overzicht van virtuele machines van een geselecteerde Azure-machine](./media/monitoring-vminsights-health/vminsights-directvm-health.png)
...
Selecteren **status diagnostische gegevens weergeven** opent een pagina weergegeven van alle onderdelen van de VM, de bijbehorende status criteria, statuswijzigingen en andere belangrijke problemen aangetroffen door de bewaking van onderdelen die betrekking hebben op de virtuele machine. Raadpleeg de [diagnostische gegevens over gezondheid](#health-diagnostics) voor meer informatie. 
...
![VM-inzichten bewakingsweergave van Azure Monitor](./media/monitoring-vminsights-health/vminsights-aggregate-health.png)

Uit de **abonnement** en **resourcegroep** vervolgkeuzelijst lijsten, selecteer een markt met de doel-VM's toegevoegd om hun status weer te geven. 
...
![VM-inzichten virtuele machine distributie perspectief](./media/monitoring-vminsights-health/vminsights-vmdistribution-by-os.png)
...
![Voorbeeld van totalisering van Red Hat Linux-VM 's](./media/monitoring-vminsights-health/vminsights-rollup-vm-rehl-01.png)
...
![Inzicht van de virtuele machine van een geselecteerde Azure-machine](./media/monitoring-vminsights-health/vminsights-directvm-health.png)
...
U kunt inzoomen verder naar beneden om te zien welke instanties zijn niet in orde door te klikken op een waarde onder de **niet in orde onderdeel** kolom.  Op de pagina bevat een tabel de onderdelen die zich in een kritieke status heeft.    
...
![Voorbeeld van diagnostische gegevens van Health-pagina voor een virtuele machine](./media/monitoring-vminsights-health/health-diagnostics-page-01.png)
...
![Secties van diagnostische gegevens van Health-pagina](./media/monitoring-vminsights-health/health-diagnostics-page-02.png)
...
![Onderdeel voorbeeldmodel weergegeven in de gezondheid van diagnostische gegevens](./media/monitoring-vminsights-health/health-diagnostics-page-component.png)

### <a name="health-criteria"></a>Criteria voor servicestatus
De middelste kolom in de Health-pagina voor diagnostische gegevens is de **Criteria voor beveiligingsstatus** kolom. Het statusmodel gedefinieerd voor de virtuele machine wordt weergegeven in een hiërarchische structuur. Het statusmodel van een virtuele machine bestaan uit eenheid, de afhankelijkheid en de cumulatieve status criteria.  

![Voorbeeld van de status criteria die zijn gepresenteerd in de gezondheid van diagnostische gegevens](./media/monitoring-vminsights-health/health-diagnostics-page-healthcriteria.png)
...
![Voorbeeld van de gezondheid van updatepakket](./media/monitoring-vminsights-health/health-diagnostics-windows-svc-rollup.png)
...
![Een voorbeeld van een health criteria configureren](./media/monitoring-vminsights-health/health-diagnostics-linuxvm-example-03.png)
...
![Configureren van een health-criteria van het voorbeeld van een unit-monitor](./media/monitoring-vminsights-health/health-diagnostics-linuxvm-example-04.png)
...
![Voorbeeld van de wijzigingen die zijn gepresenteerd in de gezondheid van diagnostische gegevens](./media/monitoring-vminsights-health/health-diagnostics-page-statechanges.png)
...
![Voorbeeld van het bewaakte exemplaar en de resultaten selecteren](./media/monitoring-vminsights-health/health-diagnostics-linuxvm-example-02.png)
...
![Voorbeeld van alle ernst niveau 1-waarschuwingen](./media/monitoring-vminsights-health/vminsights-sev1-alerts-01.png)

U kunt deze weergave filteren op waarden selecteren in het vervolgkeuzemenu's aan de bovenkant van de pagina.

|Kolom |Beschrijving | 
|-------|------------| 
|Abonnement |Selecteer een Azure-abonnementen. Alleen waarschuwingen in het geselecteerde abonnement zijn opgenomen in de weergave. | 
...
|Service bewaken |Selecteer een service of selecteer *alle* om op te nemen alle services. Alleen waarschuwingen van de Infrastructure Insights wordt ondersteund voor deze functie. | 
...
De **waarschuwen details** pagina wordt weergegeven wanneer u een waarschuwing geven details van de waarschuwing en zodat u kunt de status te veranderen. Zie voor meer informatie over het werken met regels voor waarschuwingen en het beheren van waarschuwingen, [maken, weergeven en beheren van waarschuwingen via Azure Monitor](../monitoring-and-diagnostics/monitor-alerts-unified-usage.md).

![Deelvenster met Waarschuwingsdetails voor een geselecteerde waarschuwing](./media/monitoring-vminsights-health/alert-details-pane-01.png)
...
Voor het identificeren van knelpunten en het algehele gebruik met de prestaties van uw VM's, Zie [weergave Azure VM-prestaties](monitoring-vminsights-performance.md), of als u afhankelijkheden van gedetecteerde toepassingen, Zie [weergave Azure Monitor voor virtuele machines kaart](monitoring-vminsights-maps.md). 