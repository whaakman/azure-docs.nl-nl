...
U kunt nu gegevens aan uw nieuwe graaf toevoegen met behulp van Data Explorer.

1. Vouw in Data Explorer **sample-database** en **sample-graph** uit, klik op **Graaf** en klik vervolgens op **Nieuwe hoekpunt** en **Nieuwe rand** om items aan uw graaf toe te voegen. Data Explorer is ook de plek waar u uw doorvoer kunt schalen en opgeslagen procedures, door de gebruiker gedefinieerde functies en triggers aan uw container kunt toevoegen.

    ![Hoekpunten en randen toevoegen aan een graaf in Data Explorer](./media/create-graph-dotnet/azure-cosmos-db-graph-sample-data.png)

2. Als u een aantal items hebt toegevoegd, klikt u op de knop **Filter toepassen** of klikt u met de rechtermuisknop op **Graaf** en klikt u op **Nieuwe graafquery** om een visuele weergave van uw gegevens in de vorm van een graaf te zien. U kunt wijzigen hoe gegevens worden gelabeld en opgemaakt door te klikken op de knop **Stijl** en vervolgens uw instellingen te wijzigen. Hier volgt een voorbeeld van een graaf in Data Explorer. De weergeven labels, kleuren en gegevens kunnen allemaal worden gewijzigd.

    ![Visuele graafverkenner in Data Explorer in Azure Portal](./media/create-graph-dotnet/azure-cosmos-db-graph-explorer.png)
...
* De DocumentClient wordt geïnitialiseerd. In het voorbeeld hebben we een API voor het toevoegen van een graafextensie toegevoegd aan de DocumentDB-client. We werken op een zelfstandige graafclient die is losgekoppeld van de DocumentDB-client en DocumentDB-resources.
