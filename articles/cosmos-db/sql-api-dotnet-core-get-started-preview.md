...
1. Klik op het tabblad **NuGet** op **Bladeren** en typ **Microsoft.Azure.Cosmos** in het zoekvak.
...
Een database kan worden gemaakt met de functie [**CreateDatabaseIfNotExistsAsync**](https://aka.ms/CosmosDotnetAPIDocs) of [**CreateDatabaseAsync**](https://aka.ms/CosmosDotnetAPIDocs) van de klasse ``Databases``. Een database is de logische container voor items die zijn gepartitioneerd in containers.
...
Nu gaat u de database verwijderen. Als u de gemaakte database verwijdert, worden de database en alle onderliggende resources (containers, items en eventuele opgeslagen procedures, door de gebruiker gedefinieerde functies en triggers) verwijderd. U gaat ook het **CosmosClient**-exemplaar verwijderen.
