...
# Azure Cosmos DB: een Node.js-toepassing ontwikkelen met de Graph API
...
1. Klik in [Azure Portal](http://portal.azure.com/), in uw Azure Cosmos DB-account, in het navigatiemenu links op **Sleutels** en klik vervolgens op **Sleutels voor lezen/schrijven**. In de volgende stap gebruikt u de kopieerknoppen aan de rechterkant van het scherm om de URI en primaire sleutel in het bestand `app.js` te kopiëren.

    ![De blade Sleutels in Azure Portal](./media/create-graph-nodejs/keys.png)

2. Kopieer uw Gremlin URI-waarde vanaf de portal (met de kopieerknop) en geef deze als waarde aan de eindpuntsleutel `config.endpoint` in config.js. Het Gremlin-eindpunt moet alleen de hostnaam zijn zonder het protocol-/poortnummer, zoals `mygraphdb.graphs.azure.com` (niet `https://mygraphdb.graphs.azure.com` of `mygraphdb.graphs.azure.com:433`).

    `config.endpoint = "GRAPHENDPOINT";`

3. Kopieer de waarde van uw primaire sleutel vanaf de portal en geef deze als config.primaryKey-waarde in config.js in. U hebt uw app nu bijgewerkt met alle informatie die nodig is voor de communicatie met Azure Cosmos DB. 
