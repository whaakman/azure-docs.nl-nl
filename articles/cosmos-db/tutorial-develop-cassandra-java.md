...
Deze zelfstudie laat zien hoe u een Azure Cosmos DB-account maakt met behulp van de Azure-portal en vervolgens een Cassandra-tabel (sql-api-partition-data.md#partition-keys) maakt met behulp van de [Cassandra-API ](cassandra-introduction.md). Door een primaire sleutel te definiëren wanneer u een tabel maakt, kan uw toepassing in de toekomst moeiteloos worden uitgebreid als uw gegevens groeien. 
...
Toegang tot het preview-programma van de Cassandra-API van Azure Cosmos DB. Als u nog geen toegang hebt aangevraagd, [meldt u zich nu aan](https://aka.ms/cosmosdb-cassandra-signup).
...
1. Klik in [Azure Portal](http://portal.azure.com/) op **Verbindingsreeks**. 
    ...
    ![Een gebruikersnaam bekijken en kopiëren via de pagina Verbindingsreeks in Azure Portal](./media/tutorial-develop-cassandra-java/keys.png)

2. Gebruik de ![knop Kopiëren](./media/tutorial-develop-cassandra-java/copy.png) aan de rechterkant van het scherm om de CONTACT POINT-waarde te kopiëren.
...
Nu kunt u aanvullende gegevens in uw Azure Cosmos DB-container importeren. 
