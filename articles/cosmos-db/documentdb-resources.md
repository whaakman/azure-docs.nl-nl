...
> [!NOTE]
> Azure Cosmos DB biedt een zeer efficiënte TCP-protocol, dat ook RESTful in het communicatiemodel, beschikbaar via de [SQL .NET client-API](documentdb-sdk-dotnet.md).
> 
> 
...
Naast het inrichten, configureren en beheren van uw databaseaccount vanuit de Azure-portal u kunt ook via een programma maken en Cosmos DB database accounts beheren met behulp van de [Azure Cosmos DB REST API's](/rest/api/documentdb/) evenals [client-SDK's](documentdb-sdk-dotnet.md).  
...
Een UDF kan worden opgegeven als deel van de SQL-query en wordt gebruikt als een manier om uit te breiden de kern [SQL-querytaal](documentdb-sql-query-reference.md).
