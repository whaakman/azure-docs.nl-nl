...
ms.locfileid: "34807172"
---
# <a name="azure-cosmos-db-c-console-application-tutorial-for-the-sql-api"></a>Azure Cosmos DB: zelfstudie voor de C++-consoletoepassing voor de SQL-API
...
Welkom bij de C++-zelfstudie voor de Azure Cosmos DB SQL-API SDK voor C++. Wanneer u deze zelfstudie hebt voltooid, beschikt u over een consoletoepassing waarmee u Azure Cosmos DB-resources kunt maken en er query's op kunt uitvoeren. Een van deze resources is een C++ database.

In deze snelstart komen de volgende onderwerpen aan bod:
...
* [Visual Studio 2017](https://www.visualstudio.com/downloads/), waarin de C++-taalonderdelen zijn geïnstalleerd. Als u Visual Studio 2017 nog niet hebt geïnstalleerd, kunt u het downloaden en de **gratis** [Community Edition van Visual Studio 2017](https://www.visualstudio.com/downloads/) gebruiken. Zorg ervoor dat u **Azure-ontwikkeling** inschakelt tijdens de installatie van Visual Studio.
...
5. Klik in het tabblad **NuGet: hellodocumentdb** op **Bladeren** en zoek naar *documentdbcpp*. Selecteer DocumentDbCPP in de resultaten, zoals wordt weergegeven in de volgende schermafbeelding:   
   
    ![Schermafbeelding waarin het DocumentDbCpp-pakket is gemarkeerd](media/sql-api-cpp-get-started/cpp.png)
   
    Met dit pakket worden referenties naar C++ REST SDK geïnstalleerd, een afhankelijkheid voor de DocumentDbCPP. Zodra de pakketten aan uw project zijn toegevoegd, kunt u code gaan schrijven.   

## <a id="Config"></a>Stap 3: verbindingsgegevens kopiëren vanuit Azure Portal voor uw Azure Cosmos DB-database
Open [Azure Portal](https://portal.azure.com) en ga naar het Azure Cosmos DB-account dat u hebt gemaakt. U hebt de URI en de primaire sleutel uit Azure Portal nodig in de volgende stap om verbinding te maken vanaf uw C++-codefragment. 
...
2. Voeg daarna de volgende code toe aan de functie Main en vervang de accountconfiguratie en primaire sleutel, zodat ze overeenkomen met uw Azure Cosmos DB-instellingen uit stap 3. 
    ...
    Nu u de code hebt om de client te initialiseren, kunt u zich verder verdiepen in het werken met Azure Cosmos DB-resources.
...
Kort samengevat maakt u met deze code een Azure Cosmos DB-database, -verzameling en -documenten, waarop u query's kunt toepassen in Data Explorer in Azure Portal. 
...
Azure Cosmos DB biedt ondersteuning voor [uitgebreide query's](sql-api-sql-query.md) op de JSON-documenten die zijn opgeslagen in elke verzameling. De volgende voorbeeldcode bevat een query die is gemaakt met de SQL-syntaxis. U kunt deze query uitvoeren voor de documenten die u in de vorige stap hebt gemaakt.
...
U hebt nu code toegevoegd om verschillende Azure Cosmos DB-resources te maken, aan te passen, te verwijderen en er query's op toe te passen.  Verbind alles door aanroepen toe te voegen aan de verschillende functies vanuit de hoofdfunctie in hellodocumentdb.cpp, samen met een aantal diagnostische berichten.
