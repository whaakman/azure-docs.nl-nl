...
| [MessageLockLostException](/dotnet/api/microsoft.azure.servicebus.messagelocklostexception) |LOCK-token dat is gekoppeld aan het bericht is verlopen of het lock-token is niet gevonden. |Het bericht verwijderen. |Nieuwe poging werkt niet. |
| [SessionLockLostException](/dotnet/api/microsoft.azure.servicebus.sessionlocklostexception) |Vergrendeling die is gekoppeld aan deze sessie wordt verbroken. |Afbreken de [MessageSession](/dotnet/api/microsoft.servicebus.messaging.messagesession) object. |Nieuwe poging werkt niet. |
