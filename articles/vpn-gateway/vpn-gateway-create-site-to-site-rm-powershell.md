...
In dit artikel leert u stapsgewijs hoe u een virtueel netwerk en een site-naar-site-VPN-gatewayverbinding met uw on-premises netwerk maakt met behulp van het Azure Resource Manager-implementatiemodel. Site-naar-site-verbindingen kunnen worden gebruikt voor cross-premises en hybride configuraties. U kunt deze configuratie ook uitvoeren met andere implementatieprogramma's of (alleen voor het klassieke implementatiemodel) door in de volgende lijst een andere optie te selecteren:
...
Gebruik het volgende voorbeeld als hulp bij het maken van een virtueel netwerk en een gatewaysubnet:
