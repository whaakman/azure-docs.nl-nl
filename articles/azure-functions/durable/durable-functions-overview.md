...
Duurzame functies biedt ingebouwde API's waarmee de code die u schrijft voor interactie met langlopende functie uitvoeringen. De [quickstart-voorbeeld](durable-functions-create-first-csharp.md) ziet u een eenvoudige REST-opdracht die kan worden gebruikt om nieuwe orchestrator-functie-exemplaren worden gestart. Nadat een exemplaar is gestart, wordt de extensie webhook HTTP APIs deze query de status van de orchestrator-functie. Het volgende voorbeeld ziet de REST-opdrachten om te starten van een orchestrator en de status opvragen. Sommige gegevens zijn voor alle duidelijkheid worden weggelaten uit het voorbeeld.
