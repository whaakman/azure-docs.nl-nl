...
WordPress op een virtuele machine is gekozen omdat het een van de minst dure resources dat kunnen worden geïmplementeerd in een VNET. Houd er rekening mee dat dit scenario kan ook worden gebruikt met een resource in een VNET, zoals REST-API's, App Service-omgevingen en andere Azure-services.
...
4. Configureer de virtuele machine met de volgende informatie in de blade voor het maken:
    ...
    1. Voor de administrator-account, kiest u wachtwoordverificatie en voer een unieke gebruikersnaam en wachtwoord. Voor deze zelfstudie moet u niet aanmelden bij de virtuele machine, tenzij u moet oplossen.
        ...
        ![Grondbeginselen van VM-tabblad maken](./media/functions-create-vnet/create-vm-1.png)
...
## <a name="create-a-premium-plan-function-app"></a>Een functie-App van de Premium-abonnement maken

De volgende stap is het maken van een functie-app in een premium-abonnement. Het premium-abonnement is een nieuwe aanbieding die zorgt voor serverloze schaal met alle van de voordelen van een toegewezen App Service-Plan. Verbruik plan functie-apps bieden geen ondersteuning voor VNet-integratie.

[!INCLUDE [functions-premium-create](../../includes/functions-premium-create.md)]  

## <a name="connect-your-function-app-to-your-vnet"></a>Uw functie-App verbinden met uw VNET

Een WordPress-site die als host optreedt van bestanden in uw VNET, kunt u nu de functie-app verbinden met het VNET.
...
1.  Selecteer **Klik om te configureren** onder VNet-integratie
    ...
    ![Status netwerkfunctie configureren](./media/functions-create-vnet/Networking-1.png)
...
Functies die worden uitgevoerd in een Premium-abonnement delen de dezelfde onderliggende App Service-infrastructuur als Web-Apps op PV2 plannen. Dit betekent dat alle documentatie voor Web-Apps wordt toegepast op de functies van uw Premium-abonnement.

1. [Meer informatie over de netwerkopties van functions hier](./functions-networking-options.md)
1. [Lees de veelgestelde vragen over hier netwerken functies](./functions-networking-faq.md)
1. [Meer informatie over vnet's in Azure](../virtual-network/virtual-networks-overview.md)
1. [Inschakelen van meer netwerken functies en controle met App Service-omgevingen](../app-service/environment/intro.md)
1. [Verbinding maken met afzonderlijke on-premises bronnen zonder wijzigingen van de firewall maken via hybride verbindingen](../app-service/app-service-hybrid-connections.md)
1. [Meer informatie over de functie proxy 's](./functions-proxies.md)
