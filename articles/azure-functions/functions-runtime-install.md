...
1. Hierna kunt de wizard setup wordt gestart de **Azure Functions-Runtime** hulpprogramma voor serverconfiguratie.
...
1. Klik op de **Database** tabblad, de verbindingsdetails voor uw exemplaar van SQL Server en klik op **toepassen**.  Dit is vereist om de Azure Functions-Runtime voor het maken van een database voor de ondersteuning van de Runtime.
    
    ![Azure Functions-Runtime Preview databaseconfiguratie][7]

1. Klik op de **referenties** tabblad.  In dit scherm moet u twee nieuwe referenties voor gebruik met een bestandsshare maken voor het hosten van uw Azure Functions.  Opgeven **gebruikersnaam** en **wachtwoord** combinaties voor de **share bestandseigenaar** en voor de **file share gebruiker**, klikt u vervolgens op **Toepassen**.
...
1. Klik op de **IIS** tabblad.  Op dit tabblad ziet de details van de websites in IIS die het hulpprogramma Azure Functions-Runtime-configuratie wordt gemaakt.  Klik op **toepassen** te voltooien.

    ![Preview van Azure Functions-Runtime IIS][10]

1. Klik op de **Services** tabblad.  Op dit tabblad toont de status van de services in uw Azure Functions-Runtime-hulpprogramma voor serverconfiguratie.  Als na de initiële configuratie de **Activation-Service van Azure Functions Host** wordt niet uitgevoerd Klik **Service starten**.
