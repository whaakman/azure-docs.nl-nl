...
|content|0-elementen, kenmerken 3|
...
|---|---|
|type|anonieme complexType|
|content|0-elementen, kenmerken 2|
...
|---|---|
|type|anonieme complexType|
|content|0-elementen, kenmerken 4|
...
|---|---|
|type|anonieme complexType|
|content|1-elementen, kenmerken 0|
...
|type|[KeyValuePairType](service-fabric-service-model-schema-complex-types.md#keyvaluepairtype-complextype)|
|content|element(en) van 0, 0 of meer kenmerken|
|gedefinieerd|lokaal in capaciteit element|
|naam|Capaciteit|
|minOccurs|0|
|maxOccurs|ongebonden|
...
Hiermee geeft u informatie over een X509 certificaat dat moet worden blootgesteld aan de container-omgeving. Het certificaat moet worden geïnstalleerd in het archief LocalMachine van alle clusterknooppunten.
Wanneer de toepassing wordt gestart, wordt de runtime leest het certificaat en genereert een PFX-bestand en -wachtwoord (op Windows) of een PEM-bestand (op Linux).
De PFX-bestand en het wachtwoord zijn in de container met behulp van de omgevingsvariabelen Certificates_ServicePackageName_CodePackageName_CertName_PFX en Certificates_ServicePackageName_CodePackageName_CertName_Password toegankelijk. Het PEM-bestand is beschikbaar in de container met behulp van de omgevingsvariabelen Certificates_ServicePackageName_CodePackageName_CertName_PEM en Certificates_ServicePackageName_CodePackageName_CertName_PrivateKey.
...
|---|---|
|type|anonieme complexType|
|content|2-elementen, kenmerken 0|
...
|---|---|
|type|anonieme complexType|
|content|2-elementen, kenmerken 1|
...
|---|---|
|type|anonieme complexType|
|content|3-elementen, kenmerken 0|
...
## <a name="diagnostics-element-type-diagnosticstype"></a>Diagnostische gegevens element (type DiagnosticsType) 
...
## <a name="diagnostics-element-type-servicediagnosticstype"></a>Diagnostische gegevens element (type ServiceDiagnosticsType) 
...
|---|---|
|type|anonieme complexType|
|content|5-elementen, kenmerken 2|
...
|---|---|
|type|anonieme complexType|
|content|3-elementen, kenmerken 2|
|gedefinieerd|lokaal in [ServicePackageType complexType](service-fabric-service-model-schema-complex-types.md#servicepackagetype-complextype)|
...
|---|---|
|type|anonieme complexType|
|content|2-elementen, kenmerken 2|
|gedefinieerd|lokaal in [ServicePackageType complexType](service-fabric-service-model-schema-complex-types.md#servicepackagetype-complextype)|
...
|---|---|
|type|anonieme complexType|
|content|4-elementen, kenmerken 0|
...
|type|anonieme complexType|
|content|element(en) van 0, 1 of meer kenmerken|
|gedefinieerd|lokaal in element Membership|
|naam|DomainGroup|
|minOccurs|0|
|maxOccurs|ongebonden|
...
Omgevingsvariabelen doorgeven aan de container- of exe. 
...
|---|---|
|type|anonieme complexType|
|content|1-elementen, kenmerken 2|
|gedefinieerd|Globaal|
|naam|EvictionPolicy|
...
|content|element(en) van 0, 0 of meer kenmerken|
|gedefinieerd|lokaal in de entrypoint-element|
|naam|ExeHost|
...
## <a name="group-element-defined-in-groups"></a>Groepelement (gedefinieerd in groepen) 
...
## <a name="group-element-defined-in-memberof"></a>Groepelement (gedefinieerd in MemberOf) 
...
Beschrijft het beleid voor het evalueren van health-gebeurtenissen die zijn gerapporteerd op verschillende entiteiten toepassingsgerelateerde. Als er geen beleid is opgegeven, wordt een entiteit uitgegaan slecht als het statusrapport een waarschuwing of fout is.
...
Container afbeelding overeenkomt met de OS-buildversienummer moet worden gestart. Als de Os-kenmerk niet is opgegeven, wordt de container-afbeelding wordt ervan uitgegaan dat werkt in alle versies van het besturingssysteem en heeft voorrang op de installatiekopie die is opgegeven in het servicemanifest.
...
De opslagplaats en de installatiekopie op https://hub.docker.com of het register van Azure-Container.
...
Windows Server-containers zijn mogelijk niet in verschillende versies van het besturingssysteem compatibel is.  U kunt meerdere installatiekopieën van het besturingssysteem per container opgeven en deze tag met de build-versies van het besturingssysteem. Haal de build-versie van het besturingssysteem door 'winver' voeren achter de opdrachtprompt van Windows. Als het onderliggende besturingssysteem build versie 16299 (Windows Server versie 1709), Service Fabric neemt over van de installatiekopie van de container voorzien van een besturingssysteem = '16299'. Een installatiekopie van een niet-gecodeerde container wordt ervan uitgegaan dat werkt in alle versies van het besturingssysteem en de installatiekopie die is opgegeven in het servicemanifest onderdrukkingen.
...
|---|---|
|type|anonieme complexType|
|content|6-elementen, kenmerken 0|
|gedefinieerd|lokaal in [ClusterManifestType complexType](service-fabric-service-model-schema-complex-types.md#clustermanifesttype-complextype)|
|naam|Infrastructuur|
...
Gebruikers kunnen worden toegevoegd aan een bestaande lidmaatschapsgroep, zodat deze alle eigenschappen en beveiligingsinstellingen van die lidmaatschapsgroep kan overnemen. De lidmaatschapsgroep kan worden gebruikt voor het beveiligen van externe bronnen die moeten worden geopend door andere services of dezelfde service (op een andere computer).

|Kenmerk|Waarde|
|---|---|
|type|anonieme complexType|
|content|2-elementen, kenmerken 0|
|gedefinieerd|lokaal in element van de gebruiker|
|naam|MemberOf|
|minOccurs|0|
...
|type|anonieme complexType|
|content|3-elementen, kenmerken 0|
|gedefinieerd|lokaal in een groepelement|
|naam|Lidmaatschap|
|minOccurs|0|
...
|---|---|
|type|anonieme complexType|
|content|0-elementen, kenmerken 6|
|gedefinieerd|lokaal in element van de gebruiker|
|naam|NTLMAuthenticationPolicy|
|minOccurs|0|
...
|---|---|
|type|anonieme complexType|
|content|7-elementen, kenmerken 1|
...
## <a name="parameter-element-defined-in-parameters"></a>Parameter-element (gedefinieerd in Parameters) 
...
## <a name="parameter-element-defined-in-section"></a>Parameter-element (gedefinieerd in de sectie) 
...
Indien waar, wordt de waarde van deze parameter wordt versleuteld. De ontwikkelaar van toepassingen is verantwoordelijk voor het maken van een certificaat en de cmdlet Invoke-ServiceFabricEncryptSecret gebruiken om gevoelige informatie te versleutelen. De certificaatgegevens die worden gebruikt voor het versleutelen van de waarde is opgegeven in de sectie certificaten.
|Kenmerk|Waarde|
|---|---|
|naam|isEncrypted|
|type|xs:Boolean|
...
|---|---|
|type|anonieme complexType|
|content|1-elementen, kenmerken 4|
|gedefinieerd|Globaal|
...
Hiermee geeft u op welke resource eindpunt binden aan de container-poort weergegeven.
...
## <a name="property-element-type-keyvaluepairtype"></a>Eigenschapselement (type KeyValuePairType) 
|Kenmerk|Waarde|
|---|---|
|type|[KeyValuePairType](service-fabric-service-model-schema-complex-types.md#keyvaluepairtype-complextype)|
|content|element(en) van 0, 0 of meer kenmerken|
...
## <a name="property-element-defined-in-servicetypeextensionpolicypropertiestype"></a>Eigenschapselement (gedefinieerd in ServiceTypeExtensionPolicyPropertiesType) 
|Kenmerk|Waarde|
|---|---|
|type|anonieme complexType|
|content|0-elementen, kenmerken 2|
...
Hiermee geeft u limieten voor een codepakket.
...
Beschrijving van de resource-onderdrukkingen voor eindpunten in service manifest resources.
...
## <a name="role-element-type-azureroletype"></a>Element role (type AzureRoleType) 
|Kenmerk|Waarde|
|---|---|
|type|[AzureRoleType](service-fabric-service-model-schema-complex-types.md#azureroletype-complextype)|
|content|element(en) van 0, 0 of meer kenmerken|
|gedefinieerd|lokaal in element Roles|
|naam|Rol|
|maxOccurs|ongebonden|
...
## <a name="role-element-type-blackbirdroletype"></a>Element role (type BlackbirdRoleType) 
|Kenmerk|Waarde|
|---|---|
...
## <a name="role-element-type-paasroletype"></a>Element role (type PaaSRoleType) 
|Kenmerk|Waarde|
|---|---|
...
Een vergroten/verkleinen Beleidsbeschrijving die bestaan uit een trigger en een mechanisme voor het schalen. 
...
|---|---|
|type|anonieme complexType|
|content|1-elementen, kenmerken 1|
...
|---|---|
|type|anonieme complexType|
|content|2-elementen, kenmerken 4|
...
|---|---|
|type|anonieme complexType|
|content|5-elementen, kenmerken 0|
|gedefinieerd|lokaal in [ApplicationManifestType complexType](service-fabric-service-model-schema-complex-types.md#applicationmanifesttype-complextype)|
...
Een bevoorrechte toegangspunt die standaard wordt uitgevoerd met dezelfde referenties als Service Fabric (meestal de Netwerkservice-account) voordat andere toegangspunt. De aanwezigheid van een toegangspunt voor de afzonderlijke instellingen zo voorkomt u dat de ServiceHost uitvoeren met hoge bevoegdheden voor langere tijd. U kunt de beveiligingsmachtigingen voor het uitvoeren van het opstartscript onder het lokale systeemaccount of een administrator-account wijzigen in het toepassingsmanifest.
...
Specifieke GUID om te gebruiken als de gedeelde logboek-ID.
...
|---|---|
|type|anonieme complexType|
|content|2-elementen, kenmerken 5|
...
De standaard rol clientcertificaat gebruiker gebruikt om client / server-communicatie te beveiligen.
