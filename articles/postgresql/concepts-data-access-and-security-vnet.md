---
title: Azure-Database voor PostgreSQL Server vnet eindpunt Serviceoverzicht | Microsoft Docs
description: Beschrijft hoe service-eindpunten vnet werken voor uw Azure-Database voor PostgreSQL-server.
services: postgresql
author: mbolz
ms.author: mbolz
manager: jhubbard
...
ms.locfileid: "34655187"
---
# <a name="use-virtual-network-service-endpoints-and-rules-for-azure-database-for-postgresql"></a>Het gebruik van virtueel netwerk service-eindpunten en regels voor Azure-Database voor PostgreSQL

*Virtueel netwerk regels* zijn een functie van de firewall beveiliging die bepaalt of uw Azure-Database voor PostgreSQL server accepteert communicaties die van bepaalde subnetten in virtuele netwerken worden verzonden. Dit artikel wordt uitgelegd waarom de functie van de regel virtueel netwerk is het soms de beste optie voor het toestaan van veilige communicatie met uw Azure-Database voor PostgreSQL-server.

Een regel virtueel netwerk maken eerst moet er een [virtueel netwerk] [ vm-virtual-network-overview] (VNet) en een [virtueel netwerk service-eindpunt] [ vm-virtual-network-service-endpoints-overview-649d] voor de regel om te verwijzen naar. De volgende afbeelding ziet u hoe een service-eindpunt van het virtuele netwerk werkt met Azure Database voor PostgreSQL:

![Voorbeeld van de werking van een VNet-Service-eindpunt](media/concepts-data-access-and-security-vnet/vnet-concept.png)

> [!NOTE]
> Deze functie is beschikbaar in public preview in alle regio's van Azure openbare cloud waarop Azure voor PostgreSQL-Database wordt geïmplementeerd voor Azure-Database voor PostgreSQL.
...
**Virtueel netwerk service-eindpunt:** A [Virtual Network service-eindpunt] [ vm-virtual-network-service-endpoints-overview-649d] een subnet waarvan de eigenschapwaarden een of meer namen van formele Azure-service bevatten. In dit artikel we geïnteresseerd bent in de typenaam van **Microsoft.Sql**, die verwijst naar de Azure-service met de naam SQL-Database. Deze servicetag geldt ook voor de Azure-Database voor PostgreSQL en MySQL-services. Het is belangrijk te weten bij het toepassen van de **Microsoft.Sql** tag service met een VNet-service-eindpunt wordt het netwerkverkeer voor de service-eindpunt configureren voor Azure SQL Database, Azure-Database voor PostgreSQL en Azure-Database voor de MySQL-servers op het subnet. 

**Virtueel netwerk regel:** een regel voor uw Azure-Database voor PostgreSQL-server het virtuele netwerk is een subnet op dat wordt vermeld in de toegangsbeheerlijst (ACL) van uw Azure-Database voor PostgreSQL-server. Als u in de ACL voor uw Azure-Database voor PostgreSQL-server, het subnet moet bevatten de **Microsoft.Sql** typenaam.

Een virtueel netwerk regel geeft aan dat uw Azure-Database voor PostgreSQL-server te accepteren van communicatie van elk knooppunt in het subnet.
...
### <a name="c-cannot-yet-have-azure-database-for-postgresql-on-a-subnet-without-defining-a-service-endpoint"></a>C. Nog geen Azure-Database voor PostgreSQL in een subnet zonder te definiëren van een service-eindpunt

Als uw **Microsoft.Sql** server is een knooppunt in een subnet van het virtuele netwerk, alle knooppunten in het virtuele netwerk kunnen communiceren met uw Azure-Database voor PostgreSQL-server. In dit geval kunnen uw virtuele machines communiceren met Azure-Database voor PostgreSQL zonder een virtueel netwerk regels of IP-regels.

Echter vanaf mei 2018, de Azure-Database voor PostgreSQL-service nog niet tussen de services die kunnen worden toegewezen rechtstreeks aan een subnet.
...
Elke regel van het virtuele netwerk is van toepassing op uw hele Azure-Database voor PostgreSQL-server, niet alleen op een bepaalde database op de server. Met andere woorden, virtueel netwerk regel wordt toegepast op de server-niveau, niet op het database-niveau.
...
- **Database-Admin:** &nbsp; bijwerken van de toegangsbeheerlijst (ACL) bepaald subnet toevoegen aan de Azure-Database voor PostgreSQL-server.
...
> [!NOTE]
> In sommige gevallen zijn de Azure-Database voor PostgreSQL en het VNet-subnet in verschillende abonnementen behoren. In dergelijke gevallen moet u ervoor zorgen dat de volgende configuraties:
...
De functie van de regels voor virtueel netwerk heeft voor de Azure-Database voor PostgreSQL, de volgende beperkingen:

- In de firewall voor uw Azure-Database voor PostgreSQL verwijst elke regel van het virtuele netwerk naar een subnet. Alle deze subnetten waarnaar wordt verwezen, moeten in dezelfde geografische regio die als host fungeert voor de Azure-Database voor PostgreSQL worden gehost.

- Elke Azure-Database voor PostgreSQL-server kan maximaal 128 ACL-vermeldingen voor een bepaald virtueel netwerk hebben.
...
- Inschakelen van virtueel netwerk service-eindpunten met Azure-Database voor het gebruik van PostgreSQL de **Microsoft.Sql** code van de service kan ook de eindpunten voor alle Azure-Database-services: Azure Database voor MySQL, Azure-Database voor PostgreSQL , Azure SQL Database en Azure SQL datawarehouse.
...
- [Maken en beheren van Azure-Database voor PostgreSQL VNet regels met de Azure portal](howto-manage-vnet-using-portal.md)
- [Maken en beheren van Azure-Database voor PostgreSQL VNet regels met Azure CLI](howto-manage-vnet-using-cli.md)
