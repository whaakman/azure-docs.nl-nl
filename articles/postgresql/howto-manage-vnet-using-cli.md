...
az login
```
Installeer de CLI-extensie voor Azure Database voor PostgreSQL-VNet-service-eindpunten met behulp van de `az extension add --name rdbms-vnet` opdracht. 
```azurecli-interactive
...
az extension list
```
De uitvoer van de opdracht geeft een lijst van alle geïnstalleerde extensies. De Azure Database for PostgreSQL-CLI-extensie is:
