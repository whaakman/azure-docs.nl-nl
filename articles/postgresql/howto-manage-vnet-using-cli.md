---
title: Maken en beheren van Azure-Database voor de service-eindpunten PostgreSQL VNet en regels die gebruikmaken van Azure CLI | Microsoft Docs
description: In dit artikel wordt beschreven hoe maken en beheren van Azure-Database voor de service-eindpunten PostgreSQL VNet en regels met behulp van Azure CLI-opdrachtregel.
services: postgresql
author: mbolz
ms.author: mbolz
manager: jhubbard
...
ms.locfileid: "34736623"
---
# <a name="create-and-manage-azure-database-for-postgresql-vnet-service-endpoints-using-azure-cli"></a>Maken en beheren van Azure-Database voor service-eindpunten PostgreSQL VNet met Azure CLI
Virtueel netwerk (VNet) services-eindpunten en regels van de persoonlijke adresruimte van een virtueel netwerk uitbreiden naar uw Azure-Database voor PostgreSQL-server. U handige Azure-opdrachtregelinterface (CLI)-opdrachten gebruikt, kunt u maken, bijwerken, verwijderen, weergeven en service-eindpunten VNet en regels voor het beheren van uw server weergeven. Zie voor een overzicht van Azure-Database voor PostgreSQL VNet service-eindpunten, met inbegrip van beperkingen, [Azure Database voor de service-eindpunten PostgreSQL Server VNet](concepts-data-access-and-security-vnet.md). VNet-service-eindpunten zijn beschikbaar in public preview in alle ondersteunde regio's voor Azure-Database voor PostgreSQL.
...
## <a name="configure-vnet-service-endpoints-for-azure-database-for-postgresql"></a>Vnet-service-eindpunten configureren voor Azure-Database voor PostgreSQL
...
az login
```
Installeren van de CLI-extensie voor Azure-Database voor PostgreSQL VNet service-eindpunten met behulp van de `az extension add --name rdbms-vnet` opdracht. 
```azurecli-interactive
...
az extension list
```
De uitvoer van de opdracht geeft een lijst met alle geïnstalleerde uitbreidingen. De Azure-Database voor PostgreSQL CLI-extensie is:
...
### <a name="sample-script-to-create-an-azure-database-for-postgresql-database-create-a-vnet-vnet-service-endpoint-and-secure-the-server-to-the-subnet-with-a-vnet-rule"></a>Voorbeeldscript te maken van een Azure-Database voor PostgreSQL-database, maakt u een VNet, VNet service-eindpunt en de server met het subnet met een VNet-regel beveiligen
