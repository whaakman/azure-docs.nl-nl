...
ms.locfileid: "48242600"
---
# <a name="troubleshoot-your-azure-machine-learning-service-deployments"></a>Problemen met uw Azure Machine Learning-service-implementaties oplossen
...
Dit is vooral handig als u de `Webservice.deploy` -API of `Webservice.deploy_from_model` API, omdat deze functies de bovengenoemde stappen in één actie groeperen. Deze API's zijn meestal heel gemakkelijk, maar u kunt het opsplitsen van de stappen bij het oplossen van problemen. 

Dit zijn de functies moet worden gebruikt wanneer de implementatie belangrijke omlaag stappen:
...
Als systeem kan niet aan het bouwen van de Docker-installatiekopie, is de `image.wait_for_creation` aanroep is mislukt met bepaalde foutberichten. U vindt ook meer informatie over de fout in het logboek van de build installatiekopie. Het logboek-uri is een SAS-URL die verwijst naar een logboekbestand die zijn opgeslagen in de Azure blob-opslag. Alleen kopiëren en plakken de uri in een browservenster en u kunnen downloaden en weergeven van het logboek.
...
Nadat de installatiekopie wordt is gemaakt, wordt het systeem probeert te starten van een container in ACI of AKS, afhankelijk van de configuratie van uw implementatie. Het verdient aanbeveling altijd eerst moet proberen een ACI-implementatie omdat het een eenvoudige één container-implementatie. Op deze manier u kunt vervolgens sprake is van een AKS-specifieke probleem.
...
Als de installatiekopie is gemaakt, maar er fouten optreden bij het implementeren van de afbeelding als een container, u informatie foutbericht wordt weergegeven in het Docker-logboek vindt.
...
Nu gaat u naar uw opdrachtregelvenster en type volgende opdrachten uit om aan te melden bij de ACR (Azure Container Registry) die zijn gekoppeld aan de werkruimte waar de installatiekopie is opgeslagen.
...
```
Als u geen azure-cli is geïnstalleerd, kunt u ook gebruiken `docker login` opdracht uit om aan te melden bij de ACR. U hoeft alleen de gebruikersnaam en het wachtwoord van de ACR ophalen uit Azure portal.

U kunt nu opgehaald van de Docker-installatiekopie en een container lokaal starten en start vervolgens een bash-sessie voor foutopsporing.
...
Vaak, in de `init()` functie in het scoring-script `Model.get_model_path()` functie aangeroepen om een modelbestand of map van de bestanden niet vinden in de container. Dit is vaak een bron van de fout als de modelbestand of map niet kan worden gevonden. De eenvoudigste manier om op te sporen van deze fout is om uit te voeren de volgende Python-code in de Container-shell:
...
Dit zou het lokale pad dat het scoring-script verwacht de modelbestand of map zoeken afdrukken. Vervolgens kunt u controleren of het bestand of map inderdaad waarop is naar verwachting.


## <a name="function-fails-runinputdata"></a>Functie mislukt: run(input_data)
Als de service is geïmplementeerd, en deze loopt vast bij het plaatsen van gegevens naar het scoring-eindpunt, kunt u fout-instructie in afvangen toevoegen uw `run(input_data)` functie spit van het foutbericht. Bijvoorbeeld:
