...
- Maximaal toegestane grootte van de map project: 25 MB.
    >[!NOTE]
    >Deze limiet niet van toepassing op `.git`, `docs` en `outputs` mappen. Deze mapnamen zijn hoofdlettergevoelig. Als u met grote bestanden werkt, raadpleegt u [wijzigingen permanent worden gemaakt en betrekking hebben op grote bestanden](../desktop-workbench/how-to-read-write-files.md).
...
- Maximale grootte van bijgehouden bestand in `outputs` map na een uitvoering: 512 MB
  - Dit betekent dat als het script wordt een bestand groter is dan 512 MB in de map voor uitvoer geproduceerd, deze is niet verzameld er. Als u met grote bestanden werkt, raadpleegt u [wijzigingen permanent worden gemaakt en betrekking hebben op grote bestanden](../desktop-workbench/how-to-read-write-files.md).
