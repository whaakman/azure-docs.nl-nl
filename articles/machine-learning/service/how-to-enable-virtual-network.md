...
>
> Voor __niet-standaard opslag accounts voor experimenten__of als u een opslag account __gebruikt voor__demijnen, moet u __onbeperkte toegang hebben tot het opslag account__.
> 
...
Als u de standaard Azure Storage account voor de werk ruimte in een virtueel netwerk wilt gebruiken, gebruikt u de volgende stappen:
...
Volg de onderstaande stappen om Azure Machine Learning experimenten te gebruiken met Key Vault achter een virtueel netwerk:
...
Als u Azure Machine Learning Compute wilt gebruiken in een virtueel netwerk, gebruikt u de volgende informatie over netwerk vereisten:
...
- Het subnet dat is opgegeven voor het Machine Learning Compute cluster moet voldoende niet-toegewezen IP-adressen hebben om het aantal Vm's voor het cluster te kunnen ondersteunen. Als het subnet onvoldoende niet-toegewezen IP-adressen heeft, wordt het cluster gedeeltelijk toegewezen.

- Als u van plan bent het virtuele netwerk te beveiligen door verkeer te beperken, moet u sommige poorten voor de Machine Learning Compute-service open laten. Zie [vereiste poorten](#mlcports)voor meer informatie.
...
- Als u meerdere Machine Learning Compute clusters in één virtueel netwerk wilt plaatsen, moet u mogelijk een quotum verhoging aanvragen voor een of meer van uw resources.

    Machine Learning Compute wijst automatisch extra netwerk bronnen toe aan de resource groep die het virtuele netwerk bevat. Voor elk Machine Learning Compute Cluster wijst de Azure Machine Learning-service de volgende resources toe:

    - Eén netwerk beveiligings groep (NSG)
...
Wees voorzichtig als u regels voor binnenkomend/uitgaand verkeer wijzigt of toevoegt in batch-geconfigureerde Nsg's. Als een NSG communicatie met de reken knooppunten blokkeert, wordt de status van de reken knooppunten door de Machine Learning Compute Services ingesteld op onbruikbaar.

U hoeft geen Nsg's op te geven op het niveau van het subnet, omdat batch een eigen Nsg's configureert. Als het opgegeven subnet echter is gekoppeld Nsg's en/of een firewall, configureert u de regels voor binnenkomende en uitgaande verbindingen zoals eerder beschreven. 
...
* Een door de gebruiker gedefinieerde route voor elk IP-adres dat wordt gebruikt door de Azure Batch-service in de regio waar uw resources zich bevinden. Deze Udr's inschakelen de batch-service om te communiceren met reken knooppunten voor het plannen van taken. Neem contact op met de ondersteuning van Azure voor een lijst met de IP-adressen van de batch-service.
...
Voer de volgende stappen uit om een Machine Learning Compute cluster te maken met behulp van de Azure Portal:
...
1. Selecteer in de sectie __toepassing__ __Compute__. Selecteer vervolgens __Compute toevoegen__. 
    ...
    ![Een compute toevoegen in Azure Machine Learning-service](./media/how-to-enable-virtual-network/add-compute.png)
...
Wanneer het maken van het proces is voltooid, kunt u uw model trainen met behulp van het cluster. Zie voor meer informatie [een berekenings doel selecteren en gebruiken voor training](how-to-set-up-training-targets.md).

## <a name="use-a-virtual-machine-or-hdinsight-cluster"></a>Een virtuele machine of een HDInsight-cluster gebruiken

Als u een virtuele machine of een Azure HDInsight-cluster in een virtueel netwerk met uw werk ruimte wilt gebruiken, voert u de volgende stappen uit:
...
1. Als u wilt dat de Azure Machine Learning-service kan communiceren met de SSH-poort op de virtuele machine of het cluster, moet u een bron vermelding voor de NSG configureren. De SSH-poort is doorgaans poort 22. Als u verkeer van deze bron wilt toestaan, gebruikt u de volgende informatie:
...
> [!IMPORTANT]
> Controleer de vereisten en plan IP-adres sering voor uw cluster voordat u verdergaat met de stappen. Zie [geavanceerde netwerken configureren in azure Kubernetes service](https://docs.microsoft.com/azure/aks/configure-advanced-networking)voor meer informatie.
> 
>
> Behoud de standaard regels voor uitgaande verbindingen voor de NSG. Zie de standaard beveiligings regels in [beveiligings groepen](https://docs.microsoft.com/azure/virtual-network/security-overview#default-security-rules)voor meer informatie.
>
...
Als u de Azure Kubernetes-service in een virtueel netwerk wilt toevoegen aan uw werk ruimte, volgt u deze stappen in de Azure Portal:

1. Zorg ervoor dat de NSG-groep die het virtuele netwerk beheert, binnenkomende regel heeft ingeschakeld voor Azure Machine Learning service met behulp van het __service label__ __AzureMachineLearning__
...
1. Zorg ervoor dat de NSG-groep die het virtuele netwerk beheert, binnenkomende regel heeft ingeschakeld voor het Score-eind punt zodat deze kan worden aangeroepen vanuit buiten het virtuele netwerk
