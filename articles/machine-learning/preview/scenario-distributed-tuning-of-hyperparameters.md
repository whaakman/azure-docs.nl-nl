...
Een populaire techniek voor het afstemmen van hyperparameters is een *raster zoeken* gecombineerd met *kruisvalidatie*. Kruisvalidatie is een techniek die evalueert hoe goed een model in een trainingset getraind voorspelt ten opzichte van de testset. Met deze techniek, in eerste instantie we de gegevensset onderverdelen in K vouwen en vervolgens de algoritme K tijdstippen, op een wijze round robin trainen op slechts één van de vouwen, ondergebracht out vouwen aangeroepen. We berekeningscluster de gemiddelde waarde van de metrische gegevens van K modellen via K ondergebracht out vouwen. Deze gemiddelde waarde met de naam *cross-gevalideerde prestaties schatting*, is afhankelijk van de waarden van hyperparameters gebruikt bij het maken van K-modellen. Wanneer hyperparameters afstemmen, zoeken we via de ruimte candidate hyperparameter waarden naar schatting maken van de waarden die de kruisvalidatie prestaties optimaliseren. Raster search is een algemene search-methode, waarbij de ruimte van candidate waarden van meerdere hyperparameters een vectorproduct van sets van candidate waarden van afzonderlijke hyperparameters. 

Raster zoeken op basis van kruisvalidatie kan tijdrovend. Als een algoritme 5 hyperparameters, elk met een waarde van 5 candidate is en we K = 5 vouwen gebruiken, moet om uit te voeren een zoekopdracht raster we voor het trainen van 5<sup>6</sup>= 15625 modellen. Gelukkig raster zoeken met behulp van de kruisvalidatie is een perfect parallelle procedure en kunnen alle deze modellen worden getraind parallel.
...
* Om dit scenario worden uitgevoerd met een Spark-cluster, inrichten van HDInsight-cluster door de [instructies](https://docs.microsoft.com/en-us/azure/hdinsight/hdinsight-hadoop-provision-linux-clusters). Het is raadzaam om een cluster met ten minste zes worker-knooppunten en ten minste 8 kernen en 28 Gb geheugen in de kop- en werkrollen knooppunten hebben. D4 exemplaren van virtuele machines hebt die capaciteit. Voor maximale prestaties van het cluster die wij adviseren om de parameters spark.executor.instances, spark.executor.cores en spark.executor.memory wijzigen door de [instructies](https://docs.microsoft.com/en-us/azure/hdinsight/hdinsight-apache-spark-resource-manager) en de definities in 'aangepaste bewerken sectie Spark standaardwaarden'.
...
* Maak een Azure storage-account dat wordt gebruikt voor het opslaan van de gegevensset. Volg de [instructies](https://docs.microsoft.com/en-us/azure/storage/common/storage-create-storage-account) om een opslagaccount te maken.
...
We verbinding uitvoeringsomgeving bij Azure-account maken in de volgende stappen. Open opdrachtregelvenster (CLI) door te klikken op het menu bestand in de linkerbovenhoek van AML Workbench en kiezen "Opdrachtprompt openen." Voer in de CLI
...
en zoek de abonnement-ID van Azure-abonnement met uw account AML Workbench werkruimte. Ten slotte uitvoeren in de CLI
...
Hierna gegevensset container in de lijst selecteren en klik op de knop uploaden. Azure-portal kunt meerdere bestanden tegelijk uploaden. In de sectie 'Blob uploaden' klikt u op de mapknop, selecteert u alle bestanden uit de gegevensset aan, klik op openen en klik op uploaden. De onderstaande schermafbeelding ziet u deze stappen:
...
In onze code gebruiken we [Azure-opslag-SDK](https://azure-storage.readthedocs.io/en/latest/) voor het downloaden van de gegevensset van blob-opslag naar de huidige uitvoeringsomgeving. De download wordt uitgevoerd in load\_of data() functie uit load_data.py-bestand. Als u wilt de volgende code gebruiken, moet u vervangen < accountnaam > en < ACCOUNT_KEY > met de naam en de primaire sleutel van uw opslagaccount die als host fungeert voor de gegevensset. Accountnaam wordt weergegeven in de linkerbovenhoek van Azure pagina van uw opslagaccount. Ophalen van account key, selecteer toegangstoetsen in Azure storage-pagina (Zie de eerste schermafbeelding in de sectie gegevensopname) en kopieer de lange tekenreeks in de eerste rij van de sleutelkolom:
...
U ziet dat u niet wilt load_data.py bestand handmatig uit te voeren. Dit wordt later op worden aangeroepen vanuit andere bestanden.
...
* Functies van de indicator voor categorieën van apps die zijn gebruikt door waarmee gebeurtenissen gegenereerd (één\_hot_category functie)
...
* colsample\_by_level
* deelsteekproef
* doelstelling een beschrijving van deze hyperparameters vindt [hier](http://xgboost.readthedocs.io/en/latest/python/python_api.html#module-xgboost.sklearn) en [hier](https://github.com/dmlc/xgboost/blob/master/doc/parameter.md). In eerste instantie we met externe DSVM en afstemmen hyperparameters van een klein raster van candidate waarden:

    tuned_parameters = [{'n_estimators': [300,400] 'max_depth': [3,4] 'doelstelling': ['multi:softprob'] 'reg_alpha': [1], 'reg_lambda': [1], 'colsample_bytree': [1], 'learning_rate': [0,1] 'colsample_bylevel': [0,1,], 'deelsteekproef': [0,5]}]  
...
We ook maken sweeping_results.txt-bestand met een negatieve logboek cross-gevalideerde verliezen van alle combinaties van waarden in het raster hyperparameter:
...
In de rechterbovenhoek van het venster Eigenschappen uitvoeren met de lijst van alle bestanden die zijn gemaakt in een sectie uitvoerbestanden is '. \output' map in de omgeving worden uitgevoerd. verstrekkende\_resultaat.txt kunnen worden gedownload vanaf daar door te selecteren en te klikken op de knop downloaden. sweeping_results.txt moeten hebben de volgende uitvoer:
...
Met deze opdracht is voltooid in 6 minuten van 1 uur wanneer Spark-cluster 6 worker-knooppunten met 28 Gb geheugen heeft. De resultaten van het afstemmen van hyperparameters in Spark-cluster, namelijk Logboeken, aanbevolen waarden van hyperparameters en sweeping_results.txt bestand, kunnen worden geopend in Azure Machine Learning-Workbench op dezelfde manier als in uitvoering op afstand DSVM. 

### <a name="architecture-diagram"></a>Architectuurdiagram

Het volgende diagram toont de end-to-end-werkstroom: ![architectuur](media/scenario-distributed-tuning-of-hyperparameters/architecture.png) 

## <a name="conclusion"></a>Conclusie 

In dit scenario we hebt u geleerd hoe u met Azure Machine Learning Workbench uitvoeren afstemming van hyperparameter in externe virtuele machine en in externe Spark-cluster. Azure Machine Learning Workbench biedt hulpprogramma's voor eenvoudige configuratie van uitvoeringsomgevingen en overschakelen tussen deze twee hebt gezien. 
