...
Voor meer informatie vindt u over de officiële [Jupyter documentatie](http://jupyter.readthedocs.io/en/latest/architecture/how_jupyter_ipython_work.html). Fowllowing is een diagram waarin u ziet hoe deze client, de server en de architectuur van de kernel worden toegewezen aan de Azure ML-onderdelen.
...
U hebt toegang tot veel verschillende kernels in Azure ML-Workbench door gewoon configureren uitvoeren configuraties en compute doelen in de `aml_config` map in uw project. Een nieuwe compute-doel toevoegen door uitgifte van `az ml computetarget attach` opdracht is het equivalent van het toevoegen van een nieuwe kernel.
...
>[!IMPORTANT]
>In de `.compute` -bestand van de HDI compute doel, moet u de `yarnDeployMode` veld `client` (de standaardwaarde is `cluster`) om te kunnen gebruiken deze kernel. 
