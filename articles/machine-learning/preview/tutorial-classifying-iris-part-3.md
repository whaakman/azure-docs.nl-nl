...
- Volg eerst de zelfstudie [Gegevens voorbereiden](tutorial-classifying-iris-part-1.md) om Azure Machine Learning-resources te maken en de toepassing Azure Machine Learning Workbench te installeren.
- Voltooi daarna de zelfstudie [Een model bouwen](tutorial-classifying-iris-part-2.md) om een model voor logistieke regressie te maken in Azure Machine Learning.
...
6. Wacht totdat in het deelvenster Jobs aan de rechterkant van het venster Machine Learning Workbench voor de laatste taak **iris_score.py** de status **Completed** wordt vermeld (groen gekleurd). Klik vervolgens op de hyperlink **iris_score.py [1]** voor de laatste taak om de details van de uitvoering van de run **iris_score.py** te zien. 

7. Ga op de pagina Run Properties naar de sectie **Outputs** en selecteer het zojuist gemaakte bestand **service_schema.json**. Sla het bestand op in de hoofdmap van het project.

8. Ga terug naar het tabblad waar u het script **iris_score.py** hebt geopend. 
...
>[!NOTE]
>Als u geen lokale Docker-engine hebt, kunt u een cluster maken in Azure voor de implementatie. Vergeet echter niet om het cluster na de zelfstudie te verwijderen, anders worden er lopende kosten in rekening gebracht.

1. Open de opdrachtregelinterface In de Azure Machine Learning Workbench en klik in het menu File op **Open Command Prompt**.

   De opdrachtregelprompt wordt geopend in de huidige locatie van de projectmap `c:\temp\myIris>`.

2. Zorg ervoor dat de Azure-resourceprovider `Microsoft.ContainerRegistry` is geregistreerd in uw abonnement. U moet deze resourceprovider registreren om in stap 3 een omgeving te kunnen maken. Met de volgende opdracht kunt u controleren of de provider al is geregistreerd:
   ``` 
...
3. Maak de omgeving. Deze stap moet eenmaal per omgeving worden uitgevoerd, bijvoorbeeld eenmaal voor de ontwikkelomgeving en eenmaal voor de productieomgeving. Gebruik de _lokale modus_ voor deze eerste omgeving. (U kunt proberen de schakeloptie `-c` of `--cluster` te gebruiken in de volgende opdracht om later een omgeving in de _clustermodus_ in te stellen.)
   ...
   De naam van het cluster is een manier voor u om de omgeving te identificeren. De locatie moet hetzelfde zijn als de locatie van het account voor Modelbeheer dat u hebt gemaakt in Azure Portal.
...
Als de configuratie is voltooid, stelt u de omgevingsvariabelen in die vereist zijn om het model operationeel te maken. Dit kan met de volgende opdracht. Gebruik voor de naam van de omgeving de naam die u in stap 1 hierboven hebt gebruikt. De naam van de resourcegroep is afkomstig uit hetzelfde proces en kunt u vinden in het opdrachtvenster op het moment dat het configuratieproces is afgerond.
   ```azurecli
   az ml env set -n <deployment environment name> -g <existing resource group name>
   ```

   Gebruik de volgende opdracht om te controleren of de omgeving voor implementatie van de lokale webservice goed is geconfigureerd:
...
## <a name="create-a-real-time-web-service"></a>Een realtime webservice maken
Gebruik de volgende opdracht om een realtime webservice te maken:
   ...
   De volgende schakelopties worden ondersteund voor de opdracht `az ml service create realtime`:
   ...
   * --collect-model-data true: gegevensverzameling inschakelen
   ...
   Wanneer u de opdracht uitvoert, worden het model en het scoring-bestand geüpload naar het opslagaccount dat u eerder hebt gemaakt tijdens het voorbereiden van de omgeving. Tijdens het implementatieproces wordt er een Docker-installatiekopie gemaakt met daarin uw model, schema en scoring-bestand. Deze installatiekopie wordt naar het ACR-register gepusht: `<ACR_name>.azureacr.io/<imagename>:<version>`. Vervolgens wordt die installatiekopie gedownload naar uw computer en wordt er op basis van die installatiekopie een Docker-container gestart. (Als uw omgeving is geconfigureerd in de clustermodus, wordt de Docker-container overigens geïmplementeerd in het ACS-cluster van Kubernetes.)
   ...
   U kunt de actieve Docker-container weergeven via de opdracht `docker ps`:
   ```azurecli
   docker ps
   ```
### <a name="alternative-route"></a>Alternatieve route
Als alternatief voor de opdracht `az ml service create realtime` hierboven, kunt u de stappen voor het registreren van het model, het genereren van het manifest, het maken van een Docker-installatiekopie en het maken van de webservice ook afzonderlijk uitvoeren. Hierdoor kunt u in elke stap flexibeler te werk gaan en kunt u entiteiten die zijn gegenereerd in de vorige stap opnieuw gebruiken. Het voordeel hiervan is dat entiteiten alleen opnieuw worden opgebouwd wanneer dit echt nodig is. Volg de volgende instructies om te zien hoe u dit kunt doen:
...
1. Bij het maken van de webservice zijn voorbeeldgegevens gebruikt. Wanneer u de service in de lokale modus uitvoert, kunt u de opdracht `az ml service show realtime` aanroepen voor het ophalen van een voorbeeldopdracht die u kunt gebruiken om de service te testen. U krijgt hiermee ook de beschikking over de scoring-URL, die u kunt gebruiken om de service in uw eigen aangepaste app te integreren:
...
6. U kunt deze gegevens uit Azure-blobs op verschillende manieren gebruiken met zowel software van Microsoft als open-source tools. 

   Voorbeelden van manieren om de gegevens in blobs te gebruiken:
