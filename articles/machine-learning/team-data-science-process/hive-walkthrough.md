...
1. De 'trip_data' CSV-bestanden bevatten reis details, zoals het aantal passagiers, ophalen en dropoff punten reis duur en duur van reis. Hier volgen enkele voorbeeldrecords:
...
Ophalen van de [NYC Taxi reizen](http://www.andresmh.com/nyctaxitrips/) gegevensset van de openbare locatie, u kunt een van de methoden die worden beschreven [gegevens verplaatsen en naar Azure Blob Storage](move-azure-blob.md) de gegevens kopiëren naar uw computer.

Hier wordt beschreven hoe AzCopy gebruiken om over te dragen van de bestanden met gegevens. Downloaden en installeren AzCopy Volg de instructies voor [aan de slag met het AzCopy-opdrachtregelprogramma](../../storage/common/storage-use-azcopy.md).
...
* ***&#60; path_to_data_folder >*** de map (samen met het pad) op de computer die de uitgepakte bestanden bevatten  
...
## <a name="#download-hql-files"></a>Meld u aan bij het hoofdknooppunt van Hadoop-cluster en en voorbereiden voor experimentele data-analyse
> [!NOTE]
> Dit is doorgaans een **Admin** taak.
> 
> 
...
Deze twee opdrachten zal alle .hql bestanden downloaden die nodig zijn bij dit scenario naar een lokale map ***C:\temp &#92;*** in het hoofdknooppunt.
...
Er zijn nu gereed om Hive-tabellen voor onze NYC taxi gegevensset te maken.
...
* de tabel 'trip' bevat reis details van elke rijpositie (stuurprogrammagegevens, ophalen tijd, reis afstand en tijden)
* de tabel 'tarief' bevat tarief details (tarief, tip bedrag, tolgelden en toeslagen).
...
Voer de volgende opdracht in de Hadoop-opdrachtregel zodat de databases die zijn gemaakt in HDInsight Hadoop-cluster in het venster Hadoop-opdrachtregel:
...
Is het vaak nuttig voor het opslaan van de records in een bestand om handige weer te geven. Een kleine wijziging in de bovenstaande query bewerkstelligt dit:
...
Is van belang hoe het aantal reizen varieert tijdens het kalenderjaar. Groeperen per maand kunt laten zien hoe deze verdeling van reizen eruit.
...
Telling van het totale aantal records in de gegevensset tarief dat kan worden gedaan met behulp van de onderstaande opdracht achter de opdrachtprompt van de directory Hive:
...
Geef de onderstaande opdracht vanaf de opdrachtprompt van de map Hive:
...
Wanneer een dataset verkennen, willen we vaak het nummer van co-exemplaren van groepen van waarden te onderzoeken. Deze sectie vindt u een voorbeeld van hoe u dit doet voor CAB-bestanden en stuurprogramma's.

De *voorbeeld\_hive\_reis\_aantal\_door\_straten\_license.hql* bestand groepen van de gegevensset tarief op 'straten' en 'hack_license' en retourneert de aantallen voor elke combinatie. Hieronder vindt u de inhoud ervan.
...
Een algemene doel van experimentele gegevensanalyse is wieden ongeldige of onjuiste records. In het voorbeeld in deze sectie bepaalt of de lengte of de breedte velden een waarde ver buiten het gebied van de NYC bevatten. Aangezien het waarschijnlijk dat dergelijke records een onjuiste lengtegraad-breedtegraadwaarden hebben, willen we deze alle gegevens die moet worden gebruikt voor het modelleren te elimineren.
...
De *voorbeeld\_hive\_punt\_frequencies.hql* bestand hieronder wordt weergegeven, wordt dit.
...
In de bovenstaande query R is de radius van de aarde in mijl en pi wordt omgezet in radialen. Houd er rekening mee dat de lengtegraad breedtegraad punten zijn 'gefilterd' om de waarden die ver van het gebied van de NYC te verwijderen.

In dit geval schrijven we onze resultaten naar een map met de naam 'queryoutputdir'. De reeks opdrachten hieronder weergegeven maakt eerst deze uitvoermap en vervolgens de Hive-opdracht wordt uitgevoerd.
...
De queryresultaten worden geschreven naar 9 Azure blobs ***queryoutputdir/000000\_0*** naar ***queryoutputdir/000008\_0*** onder de standaardcontainer van het Hadoop-cluster.
...
Na de analysefase experimentele gegevens bent er nu klaar om omlaag voorbeeld de gegevens voor het bouwen van modellen in Azure Machine Learning. In deze sectie laten we zien hoe een Hive-query omlaag voorbeeld gebruiken de gegevens, die vervolgens toegankelijk is vanuit de [importgegevens] [ import-data] -module in Azure Machine Learning.
...
De pijl-omlaag gebruiken voorbeeldgegevens rechtstreeks vanuit de [importgegevens] [ import-data] module in Azure Machine Learning, is het nodig zijn voor het opslaan van de resultaten van de bovenstaande query naar een interne Hive-tabel. In het volgende, we een interne Hive-tabel maken en de inhoud met de gekoppelde en omlaag steekproefgegevens vullen.
...
De query vervolgens voorbeelden omlaag de gegevens zodat de queryresultaten in Azure Machine Learning Studio inpassen kunnen. Alleen ongeveer 1% van de oorspronkelijke gegevensset wordt geïmporteerd in de Studio.

Hieronder vindt u de inhoud van *voorbeeld\_hive\_voorbereiden\_voor\_aml\_full.hql* worden gegevens voorbereid voor model bouwen in Azure Machine Learning-bestand.
...
### <a name="use-the-import-data-module-in-azure-machine-learning-to-access-the-down-sampled-data"></a>Gebruik de module gegevens importeren in Azure Machine Learning voor toegang tot de omlaag steekproefgegevens
Vereisten voor het uitgeven van Hive query's in de [importgegevens] [ import-data] -module van Azure Machine Learning, moeten we toegang tot een Azure Machine Learning-werkruimte en toegang tot de referenties van het cluster en de gekoppelde storage-account.
...
Sinds onze naar beneden steekproefgegevens bevindt zich in de standaardcontainer, en de resulterende Hive-query uit Azure Machine Learning is zeer eenvoudig en is alleen een "Selecteer * uit nyctaxidb.nyctaxi\_verkleind\_gegevens '.
...
De momentopname hieronder ziet u ons experiment te voorspellen of een tip voor een bepaalde reis is betaald.
...
De momentopname hieronder toont de distributie van tip klasse labels voor de binair klassificatieprobleem.

![Distributie van tip klasse labels](./media/hive-walkthrough/9mM4jlD.png)

Als gevolg hiervan krijgen we een AUC van 0.987 zoals weergegeven in de afbeelding hieronder.
...
Nu zien hoe de distributie van onze werkelijke test klasse eruit ziet. Ziet u dat hoewel klasse 0 en 1 van de klasse een bekende, de andere klassen zeldzame zijn.

![Klasse distributie testen](./media/hive-walkthrough/Vy1FUKa.png)

b. We gebruiken een matrix verwarring om te kijken naar onze accuratesse voorspelling voor dit experiment. Dit wordt hieronder weergegeven.
...
De momentopname belows bevat onze experiment te voorspellen, het bedrag van de opgegeven punt.

![Experiment momentopname](./media/hive-walkthrough/11TZWgV.png)

b. Regressie problemen meten we de accuratesse van onze voorspelling door te kijken naar de gekwadrateerde fout in de voorspellingen, de determinatiecoëfficiënt en dergelijke. We weergegeven deze hieronder.
