---
title: Bouw en implementeer een classificeringsmodel voor tekst met behulp van Azure Machine Learning-pakket voor de Text Analytics.
description: Informatie over het bouwen, trainen, testen en implementeren van een tekst-classificatie-model met behulp van de Azure Machine Learning-pakket voor de Text Analytics.
...
In dit artikel leert u hoe u trainen en implementeren van een model van de indeling tekst met **Azure Machine Learning-pakket voor Tekstanalyse** (AMLPTA). Het doel van de classificatie van de tekst is een stuk tekst toewijzen aan een of meer vooraf gedefinieerde klassen of categorieën. Deze tekst kan bijvoorbeeld een document, nieuwsartikel, zoekquery, e-mailbericht, tweet, ondersteuningstickets.

Er zijn grote toepassingen van de classificatie van de tekst, zoals: 
+ Krantenartikelen en nieuws wire inhoud categoriseren in onderwerpen
+ Webpagina's te organiseren in hiërarchische categorieën 
+ Filteren van spam-e-mailadres
+ Sentimentanalyse
+ De bedoeling van gebruikers van de zoekquery's voorspellen
+ Routering ondersteuningstickets
+ Analyse van feedback van klanten 

De tekst classificatie model bouwen- en implementatiewerkstroom voor een model met AMLPTA is als volgt:

1. De gegevens laden
2. Het model trainen
3. De classificatie van toepassing 
4. Modelprestatie evalueren
5. Opslaan van de pijplijn
6. De pijplijn testen
8. Het model als een webservice implementeren

Raadpleeg de [referentiedocumentatie voor het pakket](https://aka.ms/aml-packages/text) voor de uitgebreide referentie voor elke module en de klasse.

De voorbeeldcode in dit artikel maakt gebruik van een scikit-informatie van de pijplijn.
...
1. De volgende accounts en de toepassing moeten worden ingesteld en worden geïnstalleerd:
   - Azure Machine Learning experimenten-account 
...
1. De Azure Machine Learning-pakket voor Text Analytics moet zijn geïnstalleerd. Meer informatie over het [hier in dit pakket installeert](https://aka.ms/aml-packages/text).
...
Probeer het zelf uit. Download de notebook en zelf uitvoert.
...
Het volgende voorbeeld wordt de [20 nieuwsgroepen gegevensset](http://qwone.com/~jason/20Newsgroups/) die is beschikbaar via de scikit-bibliotheek om u te laten zien hoe u een classificatie voor tekst maken met Azure Machine Learning-pakket voor Tekstanalyse overschrijd meer. 

De gegevensset 20 nieuwsgroepen heeft ongeveer 18.000 nieuwsgroepen berichten op 20 verschillende onderwerpen, onderverdeeld in twee subsets: één voor training en de andere oplossing voor evaluatie van de prestaties. De splitsing in train en test is gebaseerd op de datum van elk bericht post of vóór of na een bepaalde datum.
...
Stel de locatie waar u de gegevens in de parameter data dir hebt gedownload. U kunt ook uw eigen gegevens, de invoergegevensset moet een *.tsv-bestandsindeling.
...
<table border="1" class="dataframe">
  ...
  <tbody>
    <tr>
      <th>0</th>
      <td>7</td>
      <td>Van: v064mb9k@ubvmsd.cc.buffalo.edu (NEIL B....</td>
    </tr>
    <tr>
      <th>1</th>
      <td>5</td>
      <td>Van: Rick Miller &lt; rick@ee.uwm.edu &gt;\nSubject:...</td>
    </tr>
    <tr>
      <th>2</th>
      <td>0</td>
    ...
    <tr>
      <th>4</th>
      <td>19</td>
      <td>Van: livesey@solntze.wpd.sgi.com (Jon Livesey...</td>
    </tr>
  </tbody>
</table>
</div>

Haal de correspondance tussen de categorieën en hun naam.
...
U kunt nu een histogram van de grafiek voorlopige verkennen van de frequentie van de klasse in training maken en testen van gegevenssets. 
...
Bij het uitvoeren van de notebook Jupypter, worden de grafieken worden weergegeven nadat de voorgaande codeblok wordt uitgevoerd.


## <a name="train-the-model"></a>Het model trainen

### <a name="specify-scikit-learn-algorithm-and-define-the-text-classifier"></a>Geef scikit-algoritme voor meer informatie en de classificatie van de tekst definiëren

Deze stap omvat het trainen van een scikit-meer tekst classificeringsmodel met behulp van een ten opzichte van Rest learning-algoritme voor logistieke regressie.

Voor de volledige lijst van de geleerde lessen, raadpleegt u de [Scikit cursisten](http://scikit-learn.org/stable/supervised_learning) documentatie.
...
Gebruik de standaardparameters van het pakket. Standaard haalt de classificatie van de tekst:
...
Tijdens de training, moet u zowel tekst als label kolommen hebben. Hoewel alleen de tekstkolom is nodig voor voorspellingen. 

### <a name="examine-and-set-the-parameters-of-the-different-pipeline-steps"></a>Bekijken en de parameters van de andere stappen instellen
...
Controleer vervolgens de parameterwaarden voor 'text_char_ngrams':
...
Indien nodig, kunt u de standaardparameters kunt wijzigen.  U kunt het bereik van de uitgepakte teken n-grammen uit (4,4) tot (3,4) om op te halen van zowel teken drie-g en 4 g wijzigen door de volgende code:
...
Indien nodig, kunt u de modelprestaties optimaliseren door opnieuw uit te voeren in het model die geschikt zijn voor de stap met gewijzigde parameters:
...
## <a name="apply-the-classifier"></a>De classificatie van toepassing

Het getrainde tekstclassificeerder van toepassing op de set met testgegevens voor het genereren van voorspellingen van klasse:
...
Zet de verwarring zonder normalisering matrix voor visualisatie.
...
Verwarringsmatrix zonder normalisering
    
Bij het uitvoeren van de notebook wordt de verwarringsmatrix weergegeven



Zet de genormaliseerde verwarringsmatrix voor visualisatie.
...
## <a name="save-the-pipeline"></a>Opslaan van de pijplijn
De pijplijn classificatie opslaan in een zip-bestand. Sla het word-ngrams en teken n-gram ook als tekstbestanden.
...
De classificatie pijplijn en de word-ngrams teken n-grammen voor inferentietaken laden:
...
Als u wilt een set met testgegevens evalueren, de pijplijn geladen tekst-classificatie van toepassing:
...
In deze sectie implementeert u de pijplijn van de indeling tekst als een Azure Machine Learning web service via [Azure Machine Learning CLI](https://docs.microsoft.com/azure/machine-learning/desktop-workbench/cli-for-azure-machine-learning). Vervolgens kunt u de webservice voor training en scoren gebruiken.

**Meld u aan bij uw Azure-abonnement met Azure CLI**
...
**Maken en instellen van uw implementatieomgeving**
...
1. Zorg ervoor dat uw Azure Machine Learning-omgeving, het Modelbeheer-account en de resourcegroep bevinden zich in dezelfde regio bevinden.
...
1. Update voor de implementatie-configuratiebestand dat u hebt gedownload om uw resources weer te geven:
...
1. Gezien het feit dat het getrainde model is geïmplementeerd, aanroepen van de scoring webservice op nieuwe gegevensset:
...
1. Laad de web-service op elk gewenst moment met behulp van de naam:
...
1. Test de webservice in de hoofdtekst van de twee e-mailberichten die zijn overgenomen uit de gegevensset 20 newsgrpoups:
...
Meer informatie over Azure Machine Learning-pakket voor Tekstanalyse overschrijd in deze artikelen:

+ Lees de [pakket overzicht](https://aka.ms/aml-packages/text).

+ Verken de [referentiedocumentatie](https://aka.ms/aml-packages/text) voor dit pakket.
