...
De **Microsoft Azure Machine Learning Algorithm Cheat blad** helpt bij het kiezen van het recht voor machine learning-algoritme voor uw predictive analytics-oplossingen uit de Azure Machine Learning Studio-bibliotheek met algoritmen.
...
Leren met supervisie is een populaire en nuttige van machine learning. Met één uitzondering, alle modules in Azure Machine Learning onder supervisie staan learning-algoritmen. Er zijn verschillende specifieke typen van leren met supervisie die worden weergegeven in Azure Machine Learning: classificatie, regressie en anomaliedetectie.
...
Het algoritme wijzigt op basis hiervan de strategie om te realiseren van de hoogste prijs. Er zijn momenteel geen leren met bekrachtigingen beschikt learning-algoritme modules in Azure Machine Learning. Bekrachtigingen is gebruikelijk in robotiek, waarbij de set sensorwaarden, serverlogs, op één punt in tijd een gegevenspunt, en het algoritme moet de volgende actie van de robot kiezen. Het is ook dat een natuurlijke geschikt voor Internet of Things toepassingen.
...
Er is ook een [parameter sweeping](algorithm-parameters-optimize.md) module blok in Azure Machine Learning die probeert automatisch alle combinaties van parameters aan de granulariteit die u kiest. Dit is een uitstekende manier om ervoor te zorgen dat u de parameter-ruimte hebt omspannen, verhoogt de tijd die nodig is voor een model te trainen exponentieel met het aantal parameters.
...
![Gegevens met een lineaire trends][3]
...
![Logistieke regressie tot gegevens van twee klassen met slechts één functie][4]
...
Als u beschikt over een zeer specifieke doel is het mogelijk dat de oplossing. In de verzameling van Azure Machine Learning zijn er algoritmen die zijn gespecialiseerd in:
...
![Gegevensset met behulp van K-means gegroepeerd][9]
...
![Twee klassen classificaties gecombineerd tot een classificatie van drie-klasse][10]

***Een combinatie van twee klassen classificaties combineren tot een classificatie van drie-klasse***

Azure Machine Learning bevat ook toegang tot een krachtige machine learning-frameworks onder de titel van [Vowpal Wabbit](https://msdn.microsoft.com/library/azure/8383eb49-c0a3-45db-95c8-eb56a1fef5bf).
